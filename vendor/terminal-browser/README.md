...
```json
{
  ...
  "selection": {
    ...
    "herdr": "/example/user/.local/share/terminal-browser-herdr",
    "piSettings": "/example/user/.pi/agent/settings.json",
    "piSource": "packages/pi-terminal-browser",
    "herdrRegistry": "/example/user/.config/herdr/plugins.json",
...
### Browser-only source copy

This copy includes terminal-browser and its pinned AgentCursor dependency, not
WebX search/read, the old research loader, retirement tooling or external release
publishing infrastructure. The original repository remains unchanged. See
[`copy-provenance.json`](copy-provenance.json) for source identity and file hashes.
Run commands in this directory; its pnpm workspace stays separate from the parent
repository's npm workspace. Copying this source does not activate another Pi
extension or replace an installed browser.
...
not a bundled Pi or Herdr application. This copy has no Pi Web research provider or automatic research loader.
