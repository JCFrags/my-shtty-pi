...
test('inventory has required header and columns', () => { const lines = capture().trimEnd().split('\n'); assert.equal(lines[0], 'product\tlifecycle\tprimary-npm-package\tsource-directory\trelease-unit-count\tinstall-recommendation\tpublication-permission\tpreferred-manifest-validation\tnext-gate'); assert.equal(lines.length, 8); for (const line of lines.slice(1)) assert.equal(line.split('\t').length, 9); });
