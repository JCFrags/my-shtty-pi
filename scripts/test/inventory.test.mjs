...
test('inventory has required header and columns', () => { const lines = capture().trimEnd().split('\n'); assert.equal(lines[0], 'product\tlifecycle\tprimary-npm-package\tsource-directory\trelease-unit-count\tinstall-recommendation\tpublication-permission\tpreferred-manifest-validation\tnext-gate'); assert.equal(lines.length, 10); for (const line of lines.slice(1)) assert.equal(line.split('\t').length, 9); });
test('inventory is stable, useful, and non-writing', () => { const before = fs.readFileSync(path.join(root, 'docs/package-catalog.json')); const first = capture(); const second = capture(); assert.equal(first, second); assert.match(first, /npm run (?:check|validate)/); assert.match(first, /Review the Pi 0\.84\.2 compatibility/); assert.deepEqual(fs.readFileSync(path.join(root, 'docs/package-catalog.json')), before); });
