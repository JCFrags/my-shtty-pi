...
rejects('missing checkout release comment', text => text.replace(' # v7.0.1', ''));
rejects('wrong setup-node release comment', text => text.replace('# v7.0.0', '# v7.0.1'));
rejects('unpinned action', text => text.replace(/@[0-9a-f]{40}/, '@main'));
rejects('unknown pinned action', text => text.replace('actions/setup-node@820762786026740c76f36085b0efc47a31fe5020', 'evil/setup-node@49933ea5288caeca8642d1e84afbd3f7d6820020'));
...
rejects('missing timeout', text => text.replace('    timeout-minutes: 10\n', ''));
rejects('wrong timeout', text => text.replace('timeout-minutes: 10', 'timeout-minutes: 11'));
rejects('missing package-manager-cache', text => text.replace('          package-manager-cache: false\n', ''));
rejects('true package-manager-cache', text => text.replace('package-manager-cache: false', 'package-manager-cache: true'));
rejects('extra checkout input', text => text.replace('persist-credentials: false', 'persist-credentials: false\n          extra-setting: false'));
rejects('cache dependency path', text => text.replace('package-manager-cache: false', 'package-manager-cache: false\n          cache-dependency-path: package-lock.json'));
rejects('setup scope', text => text.replace('package-manager-cache: false', 'package-manager-cache: false\n          scope: private'));
rejects('missing runtime assertion', text => text.replace(/^      - run: test .*$/m, ''));
rejects('wrong npm assertion', text => text.replace('10.9.3', '10.9.2'));
rejects('reordered run commands', text => text.replace('      - run: npm run check\n      - run: test -z "$(git status --porcelain=v1 --untracked-files=all)"', '      - run: test -z "$(git status --porcelain=v1 --untracked-files=all)"\n      - run: npm run check'));
