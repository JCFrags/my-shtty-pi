...
export function validateRoot(root = defaultRoot) {
  ...
  if (manifest.engines?.node !== '>=22.19.0' || manifest.packageManager !== 'npm@10.9.2') throw new Error('root policy: runtime metadata');
...
if (import.meta.url === `file://${process.argv[1]}`) { try { validateRoot(process.env.REPO_ROOT ? path.resolve(process.env.REPO_ROOT) : defaultRoot); console.log('root policy: ok'); } catch (error) { console.error(error.message); process.exitCode = 1; } }
