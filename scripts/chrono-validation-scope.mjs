...
function classify({ eventName, event, sha, root }) {
  ...
  if (paths.includes("package.json")) {
    ...
    if (!((oldProduct?.compiledCount === 118 && newProduct?.compiledCount === 121) ||
      (oldProduct?.compiledCount === 121 && newProduct?.compiledCount === 122) ||
      (oldProduct?.compiledCount === 122 && newProduct?.compiledCount === 124) ||
      (oldProduct?.compiledCount === 124 && newProduct?.compiledCount === 130))) throw new Error("unsupported-root-inventory-change");
