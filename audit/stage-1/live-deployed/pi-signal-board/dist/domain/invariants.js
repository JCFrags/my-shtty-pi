const ACTORS = Object.freeze({
    'update.upserted': Object.freeze(['agent']),
    'update.archived': Object.freeze(['agent', 'user']),
    'question.created': Object.freeze(['agent']),
    'question.revised': Object.freeze(['agent']),
    'question.cancelled': Object.freeze(['agent']),
    'question.escalated': Object.freeze(['system']),
    'question.staled': Object.freeze(['system']),
    'question.dismissed': Object.freeze(['user']),
    'question.answered': Object.freeze(['user']),
    'answer.delivery_queued': Object.freeze(['system']),
    'answer.delivery_failed': Object.freeze(['system']),
    'answer.acknowledged': Object.freeze(['agent']),
    'board.viewed': Object.freeze(['user']),
    'board.reset': Object.freeze(['user']),
});
const EVENT_TYPES = new Set(Object.keys(ACTORS));
...
export function isFiniteUtcTimestamp(value) {
...
export function hasValidEnvelope(event) {
    if (!isRecord(event))
        return false;
    const type = event.eventType;
    if (event.schemaVersion !== 1 || typeof type !== 'string' || !EVENT_TYPES.has(type))
        return false;
    if (typeof event.eventId !== 'string' ||
        ...
        return false;
    const allowed = ACTORS[type];
    return allowed.includes(event.actor);
}
...
export function canonicalize(value) {
    ...
    const visit = (item) => {
        ...
        if (typeof item === 'number')
            return Number.isFinite(item) ? JSON.stringify(item) : undefined;
...
export function commandFingerprint(event) {
    return canonicalize({ eventType: event.eventType, payload: event.payload });
}
export function validUpdateFields(fields, completedAt) {
    if (!isRecord(fields) ||
...
export function validQuestionSpec(spec) {
    if (!isRecord(spec) ||
    ...
    if (!spec.recommendedOptionIds.every((id) => optionIds.has(id)))
        return false;
    const single = spec.response.kind === 'single' || spec.response.kind === 'single_or_text';
    if (single && spec.recommendedOptionIds.length > 1)
        return false;
    const allowsText = spec.response.kind === 'text' || spec.response.kind.endsWith('_or_text');
    if (spec.recommendedText !== undefined &&
        (!allowsText || spec.recommendedText.trim().length === 0))
        return false;
    if (spec.temporaryDefault !== undefined) {
        if (spec.class !== 'reversible' || spec.response.kind === 'text')
            return false;
        if (!uniqueStrings(spec.temporaryDefault.optionIds) ||
            spec.temporaryDefault.optionIds.length === 0)
            return false;
        if (!spec.temporaryDefault.optionIds.every((id) => optionIds.has(id)))
            return false;
        if (single && spec.temporaryDefault.optionIds.length !== 1)
            return false;
        if (typeof spec.temporaryDefault.disclosure !== 'string' ||
...
export function validAnswerValue(value, spec) {
    if (!isRecord(value) || value.kind !== spec.response.kind)
        return false;
    const options = spec.response.options ?? [];
    const order = new Map(options.map((option, index) => [option.id, index]));
    const validText = (text) => typeof text === 'string' && text.trim() === text && text.length > 0;
    const validIds = (ids, requireOne) => {
        if (!Array.isArray(ids) || (requireOne && ids.length === 0))
            return false;
        if (!uniqueStrings(ids) || !ids.every((id) => order.has(id)))
            return false;
        return ids.every((id, index) => index === 0 || order.get(ids[index - 1]) < order.get(id));
    };
    ...
    switch (candidate.kind) {
        ...
        case 'single_or_text':
            return ((candidate.optionId === undefined ||
        ...
        case 'multiple_or_text':
            return (validIds(candidate.optionIds, false) &&
                (candidate.text === undefined || validText(candidate.text)) &&
...
export function answerMatchesRecommendation(value, spec) {
    if (spec.recommendedOptionIds.length === 0 && spec.recommendedText === undefined)
        return false;
    switch (value.kind) {
        case 'single':
            return (spec.recommendedOptionIds.length === 1 &&
                value.optionId === spec.recommendedOptionIds[0] &&
                spec.recommendedText === undefined);
        case 'multiple':
            return (sameStrings(value.optionIds, spec.recommendedOptionIds) &&
                spec.recommendedText === undefined);
        case 'text':
            return spec.recommendedOptionIds.length === 0 && value.text === spec.recommendedText;
        case 'single_or_text':
            return ((value.optionId ?? undefined) === spec.recommendedOptionIds[0] &&
...
export function validAttachments(attachments) {
...
export function sameSemanticValue(left, right) {
