...
const priority = StringEnum(['normal', 'high']);
const blockingPolicy = StringEnum(['never', 'when_agent_settles']);
const deliveryMode = StringEnum(['steer', 'followUp', 'nextTurn']);
...
const createParameters = Type.Object({ operation: StringEnum(['create']), ...editableProperties }, { additionalProperties: false });
const reviseParameters = Type.Object({
    operation: StringEnum(['revise']),
    id: questionLookupId,
...
export const QUESTION_TOOL_DESCRIPTION = 'Create, fully revise, or cancel a durable asynchronous Signals question.';
export const QUESTION_TOOL_PROMPT_SNIPPET = 'Queue a structured question only when useful independent work can continue.';
...
export function registerQuestionTool(pi, lifecycle, pending) {
    pi.registerTool({
        ...
        async execute(toolCallId, input) {
            try {
                ...
                const result = input.operation === 'create'
                    ? await service.createQuestion({ ...input, commandId })
                    : input.operation === 'revise'
                        ? await service.reviseQuestion({ ...input, commandId })
                        : await service.cancelQuestion({ ...input, commandId });
                if (!result.ok)
        ...
        renderCall(input, theme, context) {
            ...
            const identity = input.operation === 'create' ? safeInline(input.question, 28) : safeInline(input.id, 32);
            component.setText(truncateToWidth(`${theme.fg('toolTitle', theme.bold(QUESTION_TOOL_NAME))} ${theme.fg('muted', safeOperation(input))}${identity ? ` ${theme.fg('dim', identity)}` : ''}`, 240));
            return component;
        },
        renderResult(result, options, theme, context) {
            ...
            if (details === undefined) {
                ...
                const guidance = details.operation === 'cancel'
                    ? 'No answer was sent.'
...
function parseDetails(value) {
    ...
    const item = candidate.value?.item;
    if ((operation !== 'create' && operation !== 'revise' && operation !== 'cancel') ||
        typeof item !== 'object' ||
        item === null) {
        return undefined;
    }
...
function safeRevision(value) {
    return Number.isSafeInteger(value) && value >= 1 ? String(value) : '?';
}
