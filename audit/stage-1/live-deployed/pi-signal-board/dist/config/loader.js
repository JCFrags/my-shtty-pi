...
import { isConfigDocument } from './schema.js';
...
const nodeFixedConfigReader = {
    async readUtf8Capped(absoluteFixedPath) {
        try {
...
function mergeConfig(base, document) {
    return {
        schemaVersion: 1,
        ...
        widget: {
            ...
            showCompletedForMinutes: document.widget?.showCompletedForMinutes ?? base.widget.showCompletedForMinutes,
        ...
        notifications: {
            highPriorityQuestion: document.notifications?.highPriorityQuestion ?? base.notifications.highPriorityQuestion,
            questionEscalated: document.notifications?.questionEscalated ?? base.notifications.questionEscalated,
            ...
            updateCompleted: document.notifications?.updateCompleted ?? base.notifications.updateCompleted,
        },
        questions: {
            defaultDeliveryMode: document.questions?.defaultDeliveryMode ?? base.questions.defaultDeliveryMode,
            defaultBlockingPolicy: document.questions?.defaultBlockingPolicy ?? base.questions.defaultBlockingPolicy,
            recoveryDeliveryOnStart: document.questions?.recoveryDeliveryOnStart ?? base.questions.recoveryDeliveryOnStart,
        },
        limits: {
            maxActiveUpdates: document.limits?.maxActiveUpdates ?? base.limits.maxActiveUpdates,
            maxActionableQuestions: document.limits?.maxActionableQuestions ?? base.limits.maxActionableQuestions,
            visibleHistoryLimit: document.limits?.visibleHistoryLimit ?? base.limits.visibleHistoryLimit,
            maxUpdateMutationsPerTurn: document.limits?.maxUpdateMutationsPerTurn ?? base.limits.maxUpdateMutationsPerTurn,
            maxQuestionMutationsPerTurn: document.limits?.maxQuestionMutationsPerTurn ?? base.limits.maxQuestionMutationsPerTurn,
            maxAcknowledgementsPerTurn: document.limits?.maxAcknowledgementsPerTurn ?? base.limits.maxAcknowledgementsPerTurn,
        },
        ui: {
            wideLayoutMinimumColumns: document.ui?.wideLayoutMinimumColumns ?? base.ui.wideLayoutMinimumColumns,
...
function isSemanticallyValid(config) {
    return config.ui.minimumColumns <= config.ui.wideLayoutMinimumColumns;
}
async function applyDocument(source, path, base, reader) {
 ...
 */
export async function loadConfiguration(context, reader = nodeFixedConfigReader) {
    const warnings = [];
    let effective = mergeConfig(DEFAULT_CONFIG, { schemaVersion: 1 });
    const global = await applyDocument('global', join(getAgentDir(), CONFIG_FILE_NAME), effective, reader);
    ...
    const project = await applyDocument('project', join(context.cwd, CONFIG_DIR_NAME, CONFIG_FILE_NAME), effective, reader);
