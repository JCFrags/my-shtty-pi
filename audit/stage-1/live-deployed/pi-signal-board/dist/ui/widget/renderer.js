 ...
 */
export function renderWidgetLines(state, config, request) {
    const width = normalizeWidth(request.width);
    if (!config.enabled || !config.widget.enabled || width === 0)
        return Object.freeze([]);
    const candidates = selectWidgetCandidates(state, request.completedWindowCutoff);
    if (candidates.length === 0 && config.widget.hideWhenClear)
        return Object.freeze([]);
    ...
    const lines = [
        ...
        ...candidates
            ...
            .map((candidate) => candidate.entityType === 'question'
...
function renderHeader(questions, updates, unread, width) {
    if (questions === 0 && updates === 0)
        return 'SIGNALS · clear';
    if (width < LONG_HEADER_MINIMUM_WIDTH) {
        const counts = [`${questions}Q`, `${updates}U`];
        if (unread > 0)
            counts.push(`${unread} new`);
...
function renderHint(effectiveCommand, width) {
...
function countCandidates(candidates) {
    ...
    for (const candidate of candidates) {
        if (candidate.entityType === 'question')
            questions += 1;
        else
            updates += 1;
    }
...
function plainTruncate(line, width, ellipsis) {
...
function normalizeMaximumItems(maximumItems) {
