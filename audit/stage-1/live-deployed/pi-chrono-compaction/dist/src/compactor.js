...
import { measureTokenTelemetry, retentionSignalsFromFeedback } from "./telemetry.js";
...
export function resolveCompactorConfig(config = {}) {
    const merged = { ...DEFAULT_COMPACTOR_CONFIG, ...config };
...
function generationRelevantEntries(entries) {
    return entries.filter((entry) => entry.type === "message" || entry.type === "custom_message" || entry.type === "branch_summary");
}
export function computeGenerationHash(entries, config, retentionHints = "", futureEntries = [], pinnedMemoryText = "", retrievalFeedback) {
...
function shouldMergeEpisodes(units, rawTokens, targetTokens, config) {
    if (!config.mergeEpisodes)
        return false;
    if (units.length > config.maxIndividualUnits)
        return true;
    if (rawTokens > targetTokens * 5 && units.length > 60)
        return true;
    return false;
}
function appendIssues(report, additional) {
    const issues = [...report.issues, ...additional];
    return { ok: !issues.some((issue) => issue.severity === "error"), issues };
}
function buildDetails(entries, generationHash, result) {
...
export function capPlanToRecentSuffix(plan, generationHash, includeHeader, maximumTokens = HARD_REPLAY_CAP_TOKENS) {
    ...
    for (let index = 0; index < plan.units.length; index += 1) {
        for (const toolCallId of plan.units[index].toolCallIds) {
            ...
            if (interval)
                ...
                toolIntervals.set(toolCallId, { first: index, last: index });
        }
    }
    ...
    for (let start = 1; start <= plan.units.length; start += 1) {
        ...
        if (crossingInteractions === 0)
            safeStarts.push(start);
    }
    let low = 0;
    ...
    while (low < high) {
        ...
        if (renderCompressionPlan(candidate, generationHash, includeHeader).tokens > hardMaximum)
            low = middle + 1;
        else
            high = middle;
    }
...
function emptyResult(entries, config, retentionHints = "", futureEntries = []) {
...
export async function compactEntries(entries, options = {}) {
    ...
    if (blocks.length === 0)
        return emptyResult(entries, config, options.retentionHints, options.futureEntries);
    const hardOutputTokens = Math.min(HARD_REPLAY_CAP_TOKENS, Math.max(128, Math.floor(options.hardOutputTokens ?? HARD_REPLAY_CAP_TOKENS)));
    ...
    const analysisBlocks = options.futureEntries?.length
        ...
        : blocks;
    let candidateUnits = await buildCandidateUnits(blocks, config, options.semanticCompressor, options.signal, options.retentionHints ?? "", analysisBlocks, options.precomputedCandidates);
    ...
    const edited = await applyHistoryEditor(plan, blocks, generationHash, config.includeHeader, options.historyEditor, {
        maxInputTokens: Math.max(512, Math.floor(options.historyEditorMaxInputTokens ?? DEFAULT_HISTORY_EDITOR_MAX_INPUT_TOKENS)),
        maxOutputTokens: Math.min(hardOutputTokens, Math.max(256, Math.floor(options.historyEditorMaxOutputTokens ?? hardOutputTokens))),
    ...
    const additionalIssues = [
        ...initialPruned.rejectedIssues.map((issue) => ({ ...issue, severity: "warning" })),
        ...repeatPruned.rejectedIssues.map((issue) => ({ ...issue, severity: "warning" })),
    ];
    ...
    const retentionBands = [...gradient.assignments.values()].reduce((counts, assignment) => ({ ...counts, [assignment.band]: counts[assignment.band] + 1 }), { hot: 0, warm: 0, cold: 0 });
    const details = buildDetails(entries, generationHash, {
        rawTokens,
...
export function selectReplayTarget(input) {
