    ...
    try {
        for (;;) {
            ...
            for (const name of names) {
                ...
                if (value)
                    tickets.push({ ...value, name });
            }
            tickets.sort((a, b) => priorityValue(a.priority) - priorityValue(b.priority) || a.createdAtMs - b.createdAtMs || a.nonce.localeCompare(b.nonce));
            const position = Math.max(1, tickets.findIndex(x => x.nonce === nonce) + 1);
            ...
            if (available >= 0) {
                const earlier = tickets.slice(0, position - 1).length;
                if (earlier >= slots) {
                    const path = join(directory, `slot-${available}.json`);
                    ...
                    let released = false;
                    return { slot: available, slots, queueWaitMs: Date.now() - started, queuePosition: maximumPosition, release: async () => { if (released)
                            return; released = true; const path = join(directory, `slot-${available}.json`); const current = await readOwner(path); if (current?.nonce === nonce)
                            await rm(path, { force: true }); await rm(ticketPath, { force: true }); } };
                }
            }
