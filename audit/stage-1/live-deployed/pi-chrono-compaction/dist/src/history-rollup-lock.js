...
async function currentOwner(options) {
...
async function ownerIsDead(owner, options) {
    const readStart = options.readProcessStart ?? linuxProcessStart;
...
export async function acquireHistoryRollupLock(directory, options = {}) {
    ...
    try {
        ...
        if (!(await ownerIsDead(existing, options)))
            throw new Error("history-rollup-store-busy");
        const before = await stat(lockPath);
        const parsedAgain = JSON.parse(await readFile(lockPath, "utf8"));
        if (!validOwner(parsedAgain))
            throw new Error("history-rollup-lock-owner-unverifiable");
...
//# sourceMappingURL=history-rollup-lock.js.map