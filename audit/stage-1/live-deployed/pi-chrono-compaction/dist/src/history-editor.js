...
function sourceBlocks(unit, blocks) {
    return blocks.filter((block) => unit.sourceRefs.some((ref) => ref.entryId === block.entryId && (ref.blockIndex === undefined || ref.blockIndex === block.blockIndex)));
}
function requiredSnippets(unit) {
    if (unit.protectedExact || unit.kind === "user" || unit.kind === "custom_message")
        return [unit.selected.text];
    ...
    for (const candidate of unique(candidates)) {
        if (snippets.length >= 6 || tokens >= 160)
            break;
...
function goalRelevance(unit, blocks, retentionHints) {
    ...
    if (sources.some((block) => block.unresolved))
        relevance = Math.max(relevance, 0.9);
    if (sources.some((block) => block.isError || hasFailureLanguage(block.exactText)))
...
function treatmentFor(unit, age, relevance, importance) {
    if (unit.protectedExact)
        return "light";
...
export function selectHistoryEditorBudget(plan, baselineTokens, configuredMaximumTokens, hardMaximumTokens) {
    const hardBound = Math.max(256, Math.min(Math.floor(configuredMaximumTokens), Math.floor(hardMaximumTokens), Math.max(256, Math.floor(baselineTokens) - 1)));
    ...
    const expandedForHighValueHistory = baselineTokens > 10_000 && hardBound > 8_000 && (highValueCost >= 2_000 || highValueShare >= 0.3);
    const maxOutputTokens = expandedForHighValueHistory ? hardBound : Math.min(8_000, hardBound);
    const preferredOutputTokens = Math.min(maxOutputTokens, Math.max(512, Math.floor(baselineTokens * (expandedForHighValueHistory ? 0.6 : 0.45))));
    return { maxOutputTokens, preferredOutputTokens, expandedForHighValueHistory };
}
export function historyEditorStructuralHeadroom(maxOutputTokens, itemCount) {
    return Math.min(Math.max(0, maxOutputTokens - 128), Math.floor(maxOutputTokens * 0.4), 96 + Math.max(0, itemCount) * 56);
}
function boundedExcerpt(text, maximumCharacters = 900) {
...
function compressionCandidate(unit) {
    if (unit.protectedExact)
        return undefined;
...
function firstLargeDuplicateIds(units) {
    const counts = new Map();
    for (const unit of units) {
        if (estimateTokensFromText(unit.selected.text) < 128)
            continue;
    ...
    const seen = new Set();
    for (const unit of units) {
        if ((counts.get(unit.selected.text) ?? 0) < 2 || seen.has(unit.selected.text))
            continue;
...
function buildRequest(plan, blocks, budget, retentionHints) {
    ...
    return {
        ...
        maxOutputTokens,
        preferredOutputTokens: Math.min(maxOutputTokens, Math.max(128, budget.preferredOutputTokens - structuralHeadroom)),
        retentionHints,
        items,
    };
}
...
function parseDecisions(responseText, request) {
    ...
    if (value === null || typeof value !== "object" || Array.isArray(value))
        throw new Error("history editor output is not an object");
    const record = value;
    ...
    for (const raw of record.decisions) {
        ...
        if (!["critical", "high", "normal", "low"].includes(decision.importance)
            || typeof decision.confidence !== "number"
            || !Number.isFinite(decision.confidence)
            || decision.confidence < 0
            || decision.confidence > 1
            || !["keep", "compress"].includes(decision.action)
...
function editedPlan(plan, request, parsed) {
    ...
    const units = plan.units.map((unit) => {
        ...
        if (decision?.action !== "compress" || !item?.compress)
            return unit;
        const selected = unit.candidates.find((candidate) => candidate.id === item.compress?.candidateId);
        if (!selected || selected.level === "absent" || selected.tokens >= unit.selected.tokens)
            return unit;
        changedItems += 1;
...
export async function applyHistoryEditor(plan, blocks, generationHash, includeHeader, editor, options) {
    if (!editor)
        return { plan, observation: { status: "disabled", calls: 0, inputItems: 0 } };
    ...
    try {
        ...
        if (!response?.text.trim())
            throw new Error("history editor returned no text");
        const parsed = parseDecisions(response.text, request);
        const edited = editedPlan(plan, request, parsed);
        if (edited.changedItems === 0)
            throw new Error("history editor selected no eligible deterministic compression");
        const report = validatePlan(edited.plan, blocks, budget.maxOutputTokens);
        if (!report.ok)
            throw new Error(report.issues.filter((issue) => issue.severity === "error").map((issue) => issue.code).join(", "));
        const rendered = renderCompressionPlan(edited.plan, generationHash, includeHeader);
        if (rendered.tokens > budget.maxOutputTokens)
            throw new Error("rendered history editor output exceeds its adaptive hard bound");
...
export function historyEditorPrompt(request) {
    return [
...
export function createPiHistoryEditor(ctx) {
    let used = false;
    return {
        async edit(request, signal) {
            if (used)
                throw new Error("V1.1 history editor permits only one model job per compaction");
            used = true;
