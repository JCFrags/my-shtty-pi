import { directInstructionText, estimateTokensFromText, extractIdentifiers, getArray, getBoolean, getNumber, getRecord, getString, hasRestrictionLanguage, hasUnresolvedLanguage, stableStringify, } from "./utils.js";
function imageMarker(content) {
...
function renderContent(content) {
    ...
    if (!Array.isArray(content))
        return stableStringify(content, 2);
    const rendered = [];
    for (const rawBlock of content) {
        ...
        if (block.type === "text" && typeof block.text === "string")
            ...
            rendered.push(imageMarker(block));
        }
        else
            rendered.push(stableStringify(block, 2));
    }
    return rendered.join("\n");
}
function makeBlock(entry, entryIndex, kind, label, exactText, options = {}) {
    const entryId = entry.id;
    const sourceRef = options.blockIndex === undefined ? { entryId } : { entryId, blockIndex: options.blockIndex };
    const base = {
    ...
    if (options.blockIndex !== undefined)
        base.blockIndex = options.blockIndex;
    if (typeof entry.timestamp === "string")
        base.timestamp = entry.timestamp;
    if (options.toolCallId !== undefined)
        base.toolCallId = options.toolCallId;
    if (options.toolName !== undefined)
        base.toolName = options.toolName;
    if (options.toolArguments !== undefined)
        base.toolArguments = options.toolArguments;
    if (options.isError !== undefined)
        base.isError = options.isError;
    return base;
}
...
function parseMessageEntry(entry, entryIndex) {
        ...
        case "assistant": {
            ...
            content.forEach((rawBlock, blockIndex) => {
                ...
                if (!block || typeof block.type !== "string") {
                    blocks.push(makeBlock(entry, entryIndex, "assistant_text", "ASSISTANT TEXT", stableStringify(rawBlock, 2), {
                        blockIndex,
                ...
                if (block.type === "thinking" && typeof block.thinking === "string") {
                    blocks.push(makeBlock(entry, entryIndex, "assistant_reasoning", "ASSISTANT REASONING", block.thinking, {
                        blockIndex,
                ...
                if (block.type === "text" && typeof block.text === "string") {
                    const phase = toolCallIndexes.length === 0
                        ? "final"
                        : blockIndex < firstToolCallIndex
                            ...
                            : blockIndex > lastToolCallIndex
                                ? "post_tool_text"
                                : "interstitial";
                    blocks.push(makeBlock(entry, entryIndex, "assistant_text", "ASSISTANT TEXT", block.text, {
                        blockIndex,
                ...
                if (isToolCallBlock(block)) {
                    ...
                    blocks.push(makeBlock(entry, entryIndex, "tool_call", "TOOL CALL", `${block.name}(${stableStringify(args)})`, {
                        blockIndex,
                        toolCallId: block.id,
                        toolName: block.name,
                        toolArguments: args,
                ...
                if (block.type === "image" && typeof block.data === "string" && typeof block.mimeType === "string") {
                    blocks.push(makeBlock(entry, entryIndex, "assistant_text", "ASSISTANT IMAGE", imageMarker(block), {
                        blockIndex,
                        attributes: { image: true, containsImage: true },
                    }));
                    return;
                }
                blocks.push(makeBlock(entry, entryIndex, "assistant_text", "ASSISTANT CONTENT", stableStringify(block, 2), {
                    blockIndex,
...
function isReproducibleTool(toolName) {
    return /^(?:bash|read|grep|find|ls|git|search|rg|cat|head|tail)$/i.test(toolName);
}
export function parseHistoricalBlocks(entries, options = {}) {
    const blocks = [];
    entries.forEach((entry, entryIndex) => {
        ...
        switch (entry.type) {
            case "message":
                ...
                break;
            case "custom_message": {
                if (["chrono-compact-context-warning", "chrono-compact-resume"].includes(getString(entry.customType) ?? ""))
                    break;
                const text = renderContent(entry.content);
                blocks.push(makeBlock(entry, entryIndex, "custom_message", `CUSTOM MESSAGE ${getString(entry.customType) ?? ""}`.trim(), text, {
                    protectedExact: hasRestrictionLanguage(directInstructionText(text)),
            ...
            case "branch_summary": {
                blocks.push(makeBlock(entry, entryIndex, "branch_summary", "BRANCH SUMMARY", getString(entry.summary) ?? "", {
            ...
            case "compaction": {
                if (options.includeHistoricalCompactions) {
                    blocks.push(makeBlock(entry, entryIndex, "historical_compaction", "PRIOR COMPACTION CONTROL ENTRY", getString(entry.summary) ?? "", {
                        attributes: {
            ...
            case "model_change": {
                if (options.includeMetadata) {
                    blocks.push(makeBlock(entry, entryIndex, "model_change", "MODEL CHANGE", `${getString(entry.provider) ?? "unknown"}/${getString(entry.modelId) ?? "unknown"}`));
                }
                break;
            }
            case "thinking_level_change": {
                if (options.includeMetadata) {
                    blocks.push(makeBlock(entry, entryIndex, "thinking_level_change", "THINKING LEVEL CHANGE", getString(entry.thinkingLevel) ?? "unknown"));
                }
                break;
            }
            default:
    ...
    for (const block of blocks) {
        ...
        if (!block.toolName || block.toolName === "unknown")
...
export function messageContentBlocks(entry) {
    const message = getRecord(entry.message);
    const content = getArray(message?.content);
    return (content ?? []);
}
export function exactBlockContent(entry, blockIndex) {
...
export function textContentOnly(content) {
    return (getArray(content) ?? []).filter((block) => getRecord(block)?.type === "text" && typeof getRecord(block)?.text === "string");
}
