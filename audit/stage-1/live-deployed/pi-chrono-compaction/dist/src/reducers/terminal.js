import { byteCount, estimateTokensFromText, getBoolean, getNumber, getRecord, getString, lineCount, truncateToTokens, unique, } from "../utils.js";
...
function terminalMetadata(context) {
    ...
    const exitCode = getNumber(context.block.attributes.exitCode) ??
    ...
    const originallyTruncated = getBoolean(context.block.attributes.truncated) ?? getBoolean(details?.truncated) ?? getBoolean(details?.wasTruncated);
...
export function reduceTerminalOutput(context) {
    if (looksLikeTestOutput(context.block.exactText))
        return reduceTestOutput(context);
    ...
    if (originallyTruncated !== undefined)
        sections.push(`Originally truncated by command/tool: ${String(originallyTruncated)}`);
    if (fullOutputPath)
        sections.push(`Original tool spill file: ${fullOutputPath}`);
