...
import { historyDynamicValue } from "./history-value.js";
import { loadHistoryNode, readCurrentHistorySnapshot, } from "./history-rollup-store.js";
function hashTerm(salt, term) {
    return createHash("sha256").update(`${salt}:${term.toLowerCase()}`).digest("hex");
}
function contextTermHashes(runtime, context) {
...
function nodeScore(runtime, node, context, termHashes) {
...
function recordsFromNode(node) {
    if (node.nodeType === "leaf")
        return [...node.valueRecords];
    return [
...
export async function queryHistoryRollups(runtime, options = {}) {
    ...
    if (!root)
        return { records: [], nodesVisited: 0, nodeBytesRead: 0, targetLeavesLoaded: 0, sourceOrderValid: true };
    ...
    while (queue.length && nodesVisited < maximumNodes && runtime.nodeBytesRead - beforeBytes <= maximumNodeBytes) {
        ...
        nodesVisited++;
        for (const record of recordsFromNode(current.node))
        ...
        for (const childId of current.node.childNodeIds) {
            if (nodesVisited + queue.length >= maximumNodes)
                break;
            const child = await loadHistoryNode(runtime, childId);
            if (runtime.nodeBytesRead - beforeBytes > maximumNodeBytes)
                break;
...
//# sourceMappingURL=history-rollup-query.js.map