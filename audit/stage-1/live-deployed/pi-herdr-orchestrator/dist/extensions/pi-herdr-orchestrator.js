...
import { DEFAULT_PARENT_TOOL_NAMES, PARENT_TOOL_NAMES, } from "../src/pi/parent-tool-schema.js";
import { openPiHerd } from "../src/herdr/pi-herd-command.js";
import { createStateReporter, } from "../src/pi/state-reporter.js";
...
import { ToolAuditReporter, } from "../src/pi/tool-audit-reporter.js";
const MANAGED_TOOL_NAMES = [
    "orchestrator_result",
    "orchestrator_ask",
    "orchestrator_review_submit",
];
const ACTIVE_PARENT_TOOL_NAMES = process.env.PI_HERDR_ORCH_ADVANCED_TOOLS === "1"
...
function inactive(context) {
    context.ui.setStatus?.("pi-herdr-orchestrator", "Orchestrator inactive: Pi is outside a Herdr pane.");
}
export function reconcileActiveTools(pi, allowed) {
    ...
    const conflicts = [
        ...new Set([
            ...
            ...[...counts]
                ...
                .map(([name]) => name),
            ...allowed.filter((name, index) => !ORCHESTRATION_TOOLS.has(name) || allowed.indexOf(name) !== index),
        ]),
    ].sort();
    if (conflicts.length > 0) {
        pi.setActiveTools(current.filter((name) => !ORCHESTRATION_TOOLS.has(name) &&
            !LEGACY_ORCHESTRATION_TOOLS.has(name)));
        throw new Error(`ORCHESTRATION_TOOL_OWNERSHIP_CONFLICT:${conflicts.join(",")}`);
    }
    const next = [
...
export function piHerdrSessionReference(context) {
...
function readPersistedCorrelation(context) {
    ...
    for (let index = entries.length - 1; index >= 0; index--) {
        ...
        if (!entry || typeof entry !== "object" || Array.isArray(entry))
            continue;
        const source = entry;
        if (source.customType !== "pi-herdr-orchestrator-correlation" ||
        ...
        if (Object.keys(value).some((key) => !keys.includes(key)) ||
            ...
            !["accepted", "bound", "settled"].includes(kind) ||
        ...
        if (kind !== "accepted" &&
            (!bounded(value.agentCycleId, 256) ||
                ...
                value.firstTurnIndex < 0 ||
                value.firstTurnIndex > 1_000_000_000))
            continue;
        return value;
    }
    return undefined;
}
export function validateAssignment(value, state, expectedSession) {
    if (!value || typeof value !== "object" || Array.isArray(value))
        throw new Error("INVALID_REQUEST");
    const source = value;
    ...
    if (assignment.agentId !== state.agentId ||
    ...
    if (source.dependencyResults !== undefined) {
        if (!Array.isArray(source.dependencyResults) ||
        ...
        for (const item of source.dependencyResults) {
            if (!item ||
                typeof item !== "object" ||
                Array.isArray(item) ||
                Object.keys(item).some((key) => !["taskId", "resultId", "status", "summary"].includes(key)))
                throw new Error("INVALID_REQUEST");
            const projection = item;
            text(projection.taskId, 256);
            if (projection.resultId !== undefined)
                text(projection.resultId, 256);
            if (projection.status !== undefined)
                text(projection.status, 64);
            if (projection.summary !== undefined)
                text(projection.summary, 4096);
        }
    }
    if (source.resultContract !== undefined) {
        if (!source.resultContract ||
            typeof source.resultContract !== "object" ||
            Array.isArray(source.resultContract) ||
            Object.keys(source.resultContract).some((key) => !["schemaVersion", "required"].includes(key)))
            throw new Error("INVALID_REQUEST");
        const contract = source.resultContract;
...
export default async function piHerdrOrchestrator(api) {
    ...
    const auditReporter = new ToolAuditReporter({
        onGap: () => auditContext?.ui.setStatus?.("pi-herdr-orchestrator-audit", "Orchestration audit is degraded; tool behavior is unchanged."),
    });
    const terminalResultDelivery = new TerminalResultDelivery(pi);
    const providerCollector = new ProviderProjectionCollector(pi.events, async (projection) => {
        if (!client?.connected)
    ...
    const herdrPaneActive = process.env.HERDR_ENV === "1" && !!process.env.HERDR_PANE_ID;
    const herdrActive = herdrPaneActive && !!process.env.HERDR_SOCKET_PATH;
    const managedContext = herdrPaneActive &&
    ...
    const runtime = {
        async cleanup(reason) {
            const preserveCredential = ["reload", "new", "resume", "fork"].includes(reason ?? "");
            startEpoch++;
            if (heartbeat)
            ...
            heartbeat = undefined;
            if (adapter)
                binding.correlationState = adapter.correlationState();
    ...
    global[KEY] = runtime;
    const start = async (next, reconnect = false) => {
        auditContext = next;
        ...
        const scheduleAttachRetry = () => {
            if (epoch !== startEpoch || reconnectTimer)
                return;
        ...
        if (managed) {
            ...
            try {
                ...
                next.ui.setStatus?.("pi-herdr-orchestrator", error instanceof Error
        ...
        try {
            ...
            next.ui.setStatus?.("pi-herdr-orchestrator-audit", "Orchestration audit spool is unavailable; tool behavior is unchanged.");
        }
        if (epoch !== startEpoch)
            return;
        ...
        const reconcileAssignment = (assignment) => {
            if (persisted &&
                ...
                candidateAdapter.restorePersisted(persisted.kind, assignment, persisted.agentCycleId, persisted.firstTurnIndex);
            else if (binding.correlationState &&
        ...
        let scheduleReconnect = () => undefined;
        const handleControlRequest = async (request) => {
            if (!registered)
                throw new Error("PI_REGISTRATION_PENDING");
        ...
        const handleServerRequest = async (request) => {
            ...
            if (request.method === "question.deliver_answer") {
                if (Object.keys(request.params).some((key) => ![
                    "questionId",
                    "runId",
                    "toolCallId",
                    "state",
                    ...
                    !["questionId", "runId", "toolCallId", "state", "expected"].every((key) => Object.hasOwn(request.params, key)))
                    throw new Error("INVALID_REQUEST");
                ...
                if (deliveryState !== "answered" &&
                    ...
                    throw new Error("INVALID_REQUEST");
                if (!candidateClient.resolveQuestionDelivery(questionId, runId, toolCallId, {
                    state: deliveryState,
            ...
            if (request.method === "provider.files_action") {
                ...
                return await providerActions.files(action, fields);
            }
            if (request.method === "provider.agent_board_view") {
                ...
                if (selections !== undefined &&
                    ...
                    throw new Error("INVALID_REQUEST");
                return await providerActions.boardView(selections);
            }
            if (request.method === "provider.todo_action") {
                const p = request.params;
                if (Object.keys(p).some((key) => !["action", "taskId", "expected"].includes(key)) ||
                    !["start", "done", "clear_wait"].includes(p.action) ||
                ...
                if (!providerActions)
                    throw new Error("PROVIDER_UNAVAILABLE");
                return await providerActions.todo(p.action, p.taskId);
            }
            ...
            if (Object.keys(request.params).some((key) => key !== "assignment" && key !== "expected"))
                throw new Error("INVALID_REQUEST");
            const expected = request.params.expected;
            if (!expected || typeof expected !== "object" || Array.isArray(expected))
                throw new Error("INVALID_REQUEST");
            const guard = expected;
            if (Object.keys(guard).some((key) => !["piSessionId", "activity", "connectionGeneration"].includes(key)) ||
                text(guard.piSessionId, 256) !== state.sessionId ||
                guard.activity !== state.activity ||
                integer(guard.connectionGeneration) !== state.connectionGeneration)
                throw new Error("PI_IDENTITY_MISMATCH");
            const assignment = validateAssignment(request.params.assignment, state, state.sessionId);
        ...
        if (epoch !== startEpoch)
            return;
        if (managed && !token) {
            next.ui.setStatus?.("pi-herdr-orchestrator", "Managed token file unavailable; orchestration controls are disabled.");
            return;
        }
        ...
        if (epoch !== startEpoch)
            return;
        if (!managed && !secret) {
            next.ui.setStatus?.("pi-herdr-orchestrator", "Broker startup is pending; orchestration controls are disabled.");
            scheduleAttachRetry();
            return;
        }
        let candidateClient;
        const handleCandidateEvent = (event) => {
            terminalResultDelivery.handle(event, {
                ...
                request: (method, params, options) => candidateClient.request(method, params, options),
        ...
        try {
            const connected = (await candidateClient.connect());
            const subscriptionCursor = terminalResultDelivery.subscriptionCursor(Number.isSafeInteger(connected.broker?.lastEventSeq)
            ...
            const registration = await candidateClient.register(candidateAdapter.safeState(), piHerdrSessionReference(next));
            if (epoch !== startEpoch) {
                pendingClients.delete(candidateClient);
                candidateClient.close();
                return;
            ...
            pendingClients.delete(candidateClient);
            candidateAdapter.bindIdentity(registration.agentId, registration.generation, registration.connectionGeneration);
            if (registration.assignment !== undefined) {
                try {
                    const recoveredAssignment = validateAssignment(registration.assignment, candidateAdapter.safeState(), candidateAdapter.safeState().sessionId);
            ...
            pendingLifecycle = [];
            const recoveredPayload = candidateAdapter.recoveryLifecyclePayload(candidateClient.nextAdapterSeq());
            if (recoveredPayload)
            ...
            const parentAuthorized = registration.permissions.includes("delegate") ||
            ...
            if (parentAuthorized)
                await candidateClient.request("events.subscribe", {
            ...
            providerCollector.bind(registration.agentId, candidateAdapter.safeState().sessionId);
            providerCollector.request();
            scheduleReconnect = scheduleAttachRetry;
            reconnectNow = scheduleReconnect;
            stateReporter = createStateReporter({ heartbeat: (state) => candidateClient.heartbeat(state) }, {
            ...
            const flushLifecycle = () => {
                ...
                void candidateClient
                    ...
                    if (pendingLifecycle[0] === pending) {
                        ...
                        if (pending.settled)
                            pending.adapter.clearSettledCycle();
                    }
                })
                    .catch(() => {
            ...
            heartbeat = setInterval(() => {
                ...
                else if (client === candidateClient)
                    scheduleReconnect();
            }, Math.min(registration.heartbeatMs, 5_000));
            heartbeat.unref();
            next.ui.setStatus?.("pi-herdr-orchestrator", managed ? "Managed Pi connected" : "Adopted Pi connected");
        }
        catch {
            pendingClients.delete(candidateClient);
            candidateClient.close();
            if (epoch === startEpoch) {
                next.ui.setStatus?.("pi-herdr-orchestrator", "Broker unavailable; orchestration controls are disabled.");
                const delay = reconnectDelay;
                reconnectAttempts++;
                reconnectDelay = Math.min(30_000, reconnectDelay * 2);
                reconnectTimer = setTimeout(() => {
    ...
    const openAgentBoard = async (_args, raw) => {
        const context = raw;
        try {
    ...
    api.registerCommand("orchestrator-status", {
        ...
        handler: async (_args, next) => {
            ...
            if (!client?.connected)
                await start(context);
            providerCollector.request();
            next.ui.notify?.(client?.connected
                ? "Pi Herd Orchestrator connected."
                : "Pi Herd Orchestrator disconnected.", client?.connected ? "info" : "warning");
        },
    });
    api.on("session_start", async (_event, next) => {
        if (managed)
            reconcileActiveTools(pi, MANAGED_TOOL_NAMES);
        const context = next;
        terminalResultDelivery.restore(context.sessionManager.getEntries?.() ?? []);
        await start(context);
    });
    api.on("session_shutdown", (event) => runtime.cleanup(event && typeof event === "object" && "reason" in event
        ? String(event.reason)
        : "quit"));
    api.on("tool_execution_start", async (event) => {
        await auditReporter.captureStart(event);
    });
    api.on("tool_execution_end", async (event) => {
        await auditReporter.captureEnd(event);
    });
    for (const type of [
        "before_agent_start",
        "agent_start",
        ...
        api.on(type, (_event, raw) => {
            ...
            const incoming = (_event && typeof _event === "object" ? _event : {});
            const lifecycle = {
                type,
            ...
            if (currentClient && (result === "bound" || result === "settled")) {
                const payload = current.lifecyclePayload(result, lifecycle, currentClient.nextAdapterSeq());
                if (payload &&
                    !pendingLifecycle.some((item) => JSON.stringify(item.payload) === JSON.stringify(payload))) {
