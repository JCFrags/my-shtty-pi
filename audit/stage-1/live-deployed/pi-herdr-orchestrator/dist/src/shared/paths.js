...
export async function canonicalHerdrSocket(value = process.env.HERDR_SOCKET_PATH) {
    if (!value)
        throw new Error("HERDR_SOCKET_PATH is unavailable. Start Pi inside a supported Herdr pane.");
    ...
    if (canonical !== path)
        throw new Error("HERDR_SOCKET_PATH contains a symlink or noncanonical component.");
    ...
    if (!stat.isSocket() ||
        ...
        (stat.mode & 63n) !== 0n)
        throw new Error("HERDR_SOCKET_PATH must be an owner-only Unix socket owned by the current user.");
    return {
        path,
        dev: stat.dev,
        ino: stat.ino,
...
export async function revalidateHerdrSocket(expected) {
...
export function stateRootForSession(key) {
    if (!/^[A-Za-z0-9_-]{1,128}$/u.test(key))
        throw new Error("Orchestration session key is invalid.");
    const stateBase = process.env.PI_HERDR_ORCH_STATE_ROOT ??
        join(process.env.XDG_STATE_HOME ?? join(homedir(), ".local", "state"), "pi-herdr-orchestrator");
    return join(stateBase, key);
}
export function resolvePaths(herdrSocket = process.env.HERDR_SOCKET_PATH ?? "herdr") {
    const key = sessionKey(herdrSocket);
    const runtimeBase = process.env.PI_HERDR_ORCH_RUNTIME_ROOT ??
        join(process.env.XDG_RUNTIME_DIR ?? tmpdir(), `pi-herdr-orchestrator-${process.getuid?.() ?? 0}`);
    const root = stateRootForSession(key);
    const runtime = join(runtimeBase, key);
    const brokerSocket = join(runtime, "broker.sock");
    if (Buffer.byteLength(brokerSocket, "utf8") > 103)
        throw new Error("The canonical broker socket path exceeds the Linux limit.");
    return {
        root,
        runtime,
        events: join(root, "events-v1.jsonl"),
        toolAudit: join(root, "tool-audit-v1.jsonl"),
...
export async function resolveHerdrPaths(value = process.env.HERDR_SOCKET_PATH) {
