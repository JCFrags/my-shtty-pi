...
export function validateModelSelection(value) {
    if (!value || typeof value !== "object" || Array.isArray(value))
        throw new OrchestratorError("INVALID_REQUEST", "Model selection is invalid.");
    const record = value;
    if (Object.keys(record).some((key) => !["provider", "modelId", "thinkingLevel"].includes(key)) ||
        !bounded(record.provider, 128) ||
        !bounded(record.modelId, 256) ||
        !THINKING_LEVELS.includes(record.thinkingLevel))
        throw new OrchestratorError("INVALID_REQUEST", "Model selection is invalid.");
    return {
...
export function requiredModelSelections(config = {}) {
    const required = [];
    const add = (selection) => {
...
export function resolveSpawnPolicy(request, config = {}) {
    if (!bounded(request.taskProfileId, 64))
        throw new OrchestratorError("INVALID_REQUEST", "Task profile ID is invalid.");
    const placement = request.placement ?? "current-workspace";
    const defaultModelProfileId = placement === "new-workspace" ? "manager" : "subagent";
    const modelProfileId = request.modelProfileId ?? defaultModelProfileId;
    if (!["manager", "subagent"].includes(modelProfileId))
        throw new OrchestratorError("INVALID_REQUEST", "Model profile ID is invalid.");
    if ((placement === "current-workspace" && modelProfileId !== "subagent") ||
        (placement === "new-workspace" && modelProfileId !== "manager"))
        throw new OrchestratorError("PERMISSION_DENIED", "The model profile is not compatible with the requested placement.");
    const compatible = config.compatibility?.[request.taskProfileId];
    if (compatible && !compatible.includes(modelProfileId))
        throw new OrchestratorError("PERMISSION_DENIED", "The task profile is not compatible with the model profile.");
    if (!compatible && !SHIPPED_TASK_PROFILES.includes(request.taskProfileId))
        throw new OrchestratorError("INVALID_REQUEST", "The task profile has no model compatibility policy.");
    const scoped = config.defaults;
    const model = validateModelSelection(request.model ??
        (request.projectKey
    ...
    if (allowlist &&
        ...
        throw new OrchestratorError("PERMISSION_DENIED", "The effective model selection is outside the configured allowlist.");
    const effective = { placement, modelProfileId, model };
    return {
        requested: {
...
export function modelSelectionMatches(expected, actual) {
