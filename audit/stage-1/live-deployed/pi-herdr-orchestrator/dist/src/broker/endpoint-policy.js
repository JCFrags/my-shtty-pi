...
export function validConfiguredEndpointId(value) {
    return (typeof value === "string" &&
        ENDPOINT_ID_PATTERN.test(value) &&
        !value.startsWith(DERIVED_ENDPOINT_PREFIX));
}
export function derivedEndpointId(provider) {
    return `${DERIVED_ENDPOINT_PREFIX}${createHash("sha256")
        .update(provider)
        .digest("hex")
        .slice(0, 24)}`;
}
export function validateEndpointPolicyConfig(endpointsValue, modelIntelligenceValue) {
    let endpoints;
    if (endpointsValue !== undefined) {
        ...
        for (const [endpointId, rawLimit] of entries) {
            ...
            if (Object.keys(limit).some((key) => key !== "maxConcurrentAgents" && key !== "resourceClass") ||
                ...
                throw new Error(`scheduler.endpoints.${endpointId} is outside its safe bounds.`);
    ...
    if (modelIntelligenceValue !== undefined) {
        ...
        if (input.mappings.length > 0 && !endpoints)
            throw new Error("modelIntelligence mappings require scheduler.endpoints.");
        const mappings = [];
        const mappingKeys = new Set();
        for (const [index, rawMapping] of input.mappings.entries()) {
            const mapping = object(rawMapping, `modelIntelligence.mappings[${index}]`);
            if (Object.keys(mapping).some((key) => key !== "provider" &&
                key !== "modelId" &&
                key !== "endpointId" &&
                ...
                (mapping.canonicalModelId !== undefined &&
                    (typeof mapping.canonicalModelId !== "string" ||
                        !/^[a-z0-9][a-z0-9._/-]{0,255}$/u.test(mapping.canonicalModelId))) ||
        ...
        if (input.profiles !== undefined) {
            ...
            for (const [profileId, rawProfile] of entries) {
                if (!/^[a-z][a-z0-9_-]{0,63}$/u.test(profileId))
                    throw new Error("modelIntelligence.profiles is invalid.");
                const profile = object(rawProfile, `modelIntelligence.profiles.${profileId}`);
                const weights = object(profile.weightsPpm, `modelIntelligence.profiles.${profileId}.weightsPpm`);
                const weightKeys = [
                    "taskCapability",
                    "protocolReliability",
                    "speed",
                ...
                if (Object.keys(profile).some((key) => !["weightsPpm", "uncertaintyPenaltyPpm", "tieBandPpm"].includes(key)) ||
                    Object.keys(weights).length !== weightKeys.length ||
                    Object.keys(weights).some((key) => !weightKeys.includes(key)) ||
                    !weightKeys.every((key) => Number.isSafeInteger(weights[key]) &&
                    ...
                    throw new Error(`modelIntelligence.profiles.${profileId} is invalid.`);
                profiles[profileId] = {
                    weightsPpm: {
        ...
        if (input.sources !== undefined) {
            ...
            if (sourceInput.artificialAnalysis !== undefined) {
                const raw = object(sourceInput.artificialAnalysis, "modelIntelligence.sources.artificialAnalysis");
                if (Object.keys(raw).some((key) => ![
                    ...
                    throw new Error("modelIntelligence.sources.artificialAnalysis is invalid.");
                const profileInput = object(raw.profileMetrics, "modelIntelligence.sources.artificialAnalysis.profileMetrics");
                ...
                if (profileEntries.length < 1 ||
                    ...
                    throw new Error("modelIntelligence.sources.artificialAnalysis.profileMetrics is invalid.");
                const profileMetrics = {};
                for (const [profile, metric] of profileEntries) {
                    if (!/^[a-z][a-z0-9_-]{0,63}$/u.test(profile) ||
                        !["coding", "intelligence", "agentic"].includes(String(metric)))
                        throw new Error("modelIntelligence.sources.artificialAnalysis.profileMetrics is invalid.");
                    profileMetrics[profile] = metric;
                }
                if (!Array.isArray(raw.models) ||
                    raw.models.length < 1 ||
                    raw.models.length > MAX_FOUNDATION_MODELS)
                    throw new Error("modelIntelligence.sources.artificialAnalysis.models is invalid.");
                const models = [];
                const canonicalIds = new Set();
                const slugs = new Set();
                const configuredCanonicalIds = new Set(mappings
                    .map((mapping) => mapping.canonicalModelId)
                    .filter((value) => value !== undefined));
                for (const [index, rawModel] of raw.models.entries()) {
                    const model = object(rawModel, `modelIntelligence.sources.artificialAnalysis.models[${index}]`);
                    if (Object.keys(model).some((key) => key !== "canonicalModelId" && key !== "slug") ||
                        ...
                        throw new Error(`modelIntelligence.sources.artificialAnalysis.models[${index}] is invalid.`);
        ...
        modelIntelligence = {
            schemaVersion: 1,
            routingMode: (input.routingMode ?? "current_default"),
            mappings,
            ...(profiles ? { profiles } : {}),
            ...(sources ? { sources } : {}),
        };
...
export function resolveEndpoint(model, policy, fallbackLimit) {
    const exact = policy.mappings?.find((mapping) => mapping.provider === model.provider && mapping.modelId === model.modelId);
    const provider = policy.mappings?.find((mapping) => mapping.provider === model.provider && mapping.modelId === undefined);
    ...
    if (!limit)
        throw new Error(`Endpoint mapping references unknown endpoint: ${mapping.endpointId}.`);
    return {
        endpointId: mapping.endpointId,
