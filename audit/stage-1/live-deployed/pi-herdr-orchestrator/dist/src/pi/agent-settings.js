import { Container, SettingsList, Text, } from "@pi-herdr-deck/tui";
import { SHIPPED_TASK_PROFILES, THINKING_LEVELS, requiredModelSelections, } from "../broker/model-policy.js";
...
function modelFromScoped(value) {
...
function currentPiModels(context) {
...
function parseCapabilities(value) {
    ...
    const models = value.models.map((candidate) => {
        if (!isRecord(candidate) ||
            ...
            candidate.thinkingLevels.some((level) => !THINKING_LEVELS.includes(level)))
            throw new Error("The broker returned an invalid model catalog.");
        return {
    ...
    return {
        models,
        thinkingLevels: THINKING_LEVELS.filter((level) => models.some((model) => model.thinkingLevels.includes(level))),
    };
}
...
function buildCatalog(capabilities, context, policy) {
    const current = new Set(currentPiModels(context).map((model) => modelKey(model.provider, model.id)));
    const retained = new Set([...(policy.allowlist ?? []), ...requiredModelSelections(policy)].map((selection) => modelKey(selection.provider, selection.modelId)));
...
function selectedByModel(selections) {
    const selected = new Map();
    for (const selection of selections) {
        const key = modelKey(selection.provider, selection.modelId);
        const levels = selected.get(key) ?? new Set();
...
function selectionCount(selected) {
    let count = 0;
    for (const levels of selected.values())
        count += levels.size;
    return count;
}
function summarizeLevels(model, levels) {
    const ordered = model.thinkingLevels.filter((level) => levels?.has(level));
    return ordered.length > 0 ? ordered.join(", ") : "not allowed";
}
function flattenSelections(catalog, selected) {
    const selections = [];
...
function addRequiredSelections(selected, required) {
...
function createLevelSubmenu(model, selected, requiredKeys, routingMode, ui, listTheme, done) {
    const key = modelKey(model.provider, model.modelId);
    const original = selected.get(key) ?? new Set();
    const draft = new Set(original);
    const items = [
        ...model.thinkingLevels.map((level) => {
            const required = requiredKeys.has(selectionKey({
                provider: model.provider,
                modelId: model.modelId,
                thinkingLevel: level,
            }));
            return {
                ...
                ...(required
                    ? {
                        description: routingMode === "explicit_required"
    ...
    return new SettingsList(items, Math.min(items.length + 2, 12), listTheme, (id, value) => {
        if (id !== SAVE_ID) {
            const level = id;
            if (value === "allowed")
                draft.add(level);
        ...
        if (nextCount > MAX_ALLOWLIST_ENTRIES) {
            ui.notify(`A restricted model scope can contain at most ${MAX_ALLOWLIST_ENTRIES} model-level pairs.`, "warning");
            return;
        }
        if (draft.size > 0)
            selected.set(key, new Set(draft));
        else
            selected.delete(key);
        done(summarizeLevels(model, draft));
    }, () => done(undefined));
}
function createEndpointSubmenu(endpoints, listTheme, done) {
        ...
        ...(entries.length > 0
            ? entries.map((endpointId) => ({
                ...
                values: [
                    ...
                    .map(String),
                description: "Lowering a limit blocks new admission. It does not stop active agents.",
            }))
            : [
                {
                    id: "none",
                    ...
                    currentValue: "none",
                    values: ["none"],
                    description: "Add endpoint IDs in the owner config before assigning model mappings.",
                },
            ]),
        {
            id: SAVE_ID,
...
function createMappingSubmenu(catalog, endpoints, mappings, listTheme, done) {
    ...
    const items = [
        ...catalog.map((model, index) => ({
            ...
            currentValue: mappings.find((mapping) => mapping.provider === model.provider &&
                mapping.modelId === model.modelId)?.endpointId ?? "derived",
            values,
            description: "An exact model mapping overrides a provider mapping. Derived uses the provider fallback.",
        })),
        {
            id: SAVE_ID,
    ...
    return new SettingsList(items, Math.min(items.length + 2, 12), listTheme, (id, value) => {
        ...
        if (!model)
            return;
        const existing = mappings.findIndex((mapping) => mapping.provider === model.provider &&
...
function createProfileSubmenu(profileId, profile, profiles, ui, listTheme, done) {
    ...
    return new SettingsList(items, Math.min(items.length + 2, 12), listTheme, (id, value) => {
        if (id === SAVE_ID) {
            const total = Object.values(draft.weightsPpm).reduce((sum, item) => sum + item, 0);
            if (total !== 1_000_000) {
        ...
        if (id === "uncertaintyPenaltyPpm" || id === "tieBandPpm")
            ...
            mutableDraft.weightsPpm[id] = Number(value);
    }, () => done(undefined));
}
function createProfilesSubmenu(profiles, ui, listTheme, done) {
    ...
    const items = [
        ...profileIds.map((profileId) => ({
            ...
            description: "Order: capability, protocol, speed, cost, human preference.",
            submenu: (_currentValue, close) => createProfileSubmenu(profileId, profiles[profileId], profiles, ui, listTheme, close),
        })),
        {
            id: SAVE_ID,
    ...
    return new SettingsList(items, Math.min(items.length + 2, 12), listTheme, (id) => {
        if (id === SAVE_ID)
            done(`${String(profileIds.length)} profiles`);
    }, () => done(undefined));
}
async function editPolicy(context, catalog, policy, operatorSettings) {
    if (!("custom" in context.ui) || typeof context.ui.custom !== "function") {
        context.ui.notify?.("Agent settings require Pi's interactive UI.", "warning");
        return undefined;
    }
    const ui = context.ui;
    ...
    let routingMode = operatorSettings.modelIntelligence.routingMode ?? "current_default";
    const selected = selectedByModel(policy.allowlist ?? (routingMode === "explicit_required" ? [] : required));
    if (routingMode !== "explicit_required")
        addRequiredSelections(selected, required);
    const endpoints = structuredClone(operatorSettings.endpoints);
    const mappings = structuredClone(operatorSettings.modelIntelligence.mappings);
    const configuredProfileIds = Object.keys(operatorSettings.modelIntelligence.profiles ?? {});
    const profileIds = [
        ...new Set([...SHIPPED_TASK_PROFILES, ...configuredProfileIds]),
    ];
    const profiles = Object.fromEntries(profileIds.map((profileId) => [
        profileId,
        structuredClone(modelRankingProfile(operatorSettings.modelIntelligence, profileId)),
    ]));
    let refreshFoundation = false;
    const foundationState = String(operatorSettings.foundationStatus?.state ?? "disabled");
    const foundationObservedAt = operatorSettings.foundationStatus?.lastGoodObservedAt;
    const foundationSummary = typeof foundationObservedAt === "string"
        ? `${foundationState} · ${foundationObservedAt}`
        : foundationState;
    return await ui.custom((_tui, theme, _kb, done) => {
        const listTheme = settingsTheme(theme);
        const container = new Container();
        container.addChild(new Text(theme.fg("accent", theme.bold("Agent settings")), 1, 1));
        const items = [
            {
                id: ROUTING_ID,
                label: "Model routing",
                ...
                description: "explicit_required rejects a direct agent_spawn without a model. rated_auto selects only with sufficient evidence.",
            },
            {
                id: MODE_ID,
                label: "Scope mode",
                ...
                description: "Restricted mode permits only the selected model and thinking-level pairs.",
            },
            {
                id: ENDPOINTS_ID,
                ...
                submenu: (_currentValue, close) => createEndpointSubmenu(endpoints, listTheme, close),
            },
            {
                id: MAPPINGS_ID,
                ...
                submenu: (_currentValue, close) => createMappingSubmenu(catalog, endpoints, mappings, listTheme, close),
            },
            {
                id: PROFILES_ID,
                label: "Task-profile scoring",
                currentValue: `${String(Object.keys(profiles).length)} profiles`,
                description: "Edit capability, reliability, speed, cost, preference, uncertainty, and tie weights.",
                submenu: (_currentValue, close) => createProfilesSubmenu(profiles, ui, listTheme, close),
            },
            {
                id: FOUNDATION_ID,
                ...
                description: "Source status is read-only here. Select refresh to request one bounded update.",
            },
            ...catalog.map((model, index) => ({
                id: `model-${index}`,
                label: `${model.provider}/${model.modelId}`,
                currentValue: summarizeLevels(model, selected.get(modelKey(model.provider, model.modelId))),
                ...
                submenu: (_currentValue, close) => createLevelSubmenu(model, selected, routingMode === "explicit_required" ? new Set() : requiredKeys, routingMode, ui, listTheme, close),
            })),
            {
                id: SAVE_ID,
        ...
        let settingsList;
        settingsList = new SettingsList(items, 15, listTheme, (id, value) => {
            if (id === ROUTING_ID) {
                routingMode = value;
                if (routingMode !== "explicit_required")
                    addRequiredSelections(selected, required);
            ...
            if (id !== SAVE_ID)
                return;
            const allowlist = mode === "unrestricted"
                ? null
                : flattenSelections(catalog, selected);
            if (allowlist !== null && allowlist.length < 1) {
                ui.notify("Select at least one model and thinking level.", "warning");
                return;
            }
            const sortedMappings = [...mappings].sort((left, right) => left.provider.localeCompare(right.provider) ||
                (left.modelId ?? "").localeCompare(right.modelId ?? ""));
            done({
                allowlist,
...
export async function openAgentSettings(client, context) {
    if (!client?.connected) {
    ...
    try {
        ...
        if (catalog.length < 1) {
            context.ui.notify?.("No broker-attested model is in the current Pi model scope.", "warning");
            return;
        }
        ...
        context.ui.notify?.(persisted
            ? "Agent settings were saved for new agents."
            : "Agent settings were not persisted.", persisted ? "info" : "warning");
        if (result.refreshFoundation) {
            try {
                ...
                context.ui.notify?.(started
                    ? "Foundation refresh was scheduled."
                    : "Foundation refresh is not enabled.", started ? "info" : "warning");
            }
            catch (error) {
                context.ui.notify?.(`Agent settings were saved, but foundation refresh failed: ${error instanceof Error ? error.message : String(error)}`, "warning");
            }
        }
    }
    catch (error) {
