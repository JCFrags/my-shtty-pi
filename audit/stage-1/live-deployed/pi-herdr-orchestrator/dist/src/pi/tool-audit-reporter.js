...
import { createPrivateExclusive, readPrivateRegular, replacePrivateRegular, verifyPrivatePath, } from "../shared/private-fs.js";
import { ensurePrivateDirectory, stateRootForSession, } from "../shared/paths.js";
import { stat, unlink } from "node:fs/promises";
import { join } from "node:path";
import { DEFAULT_PARENT_TOOL_NAMES, PARENT_TOOL_NAMES, } from "./parent-tool-schema.js";
...
export const AUDITED_ORCHESTRATION_TOOLS = new Set([
    ...MANAGED_TOOL_NAMES,
...
function validateEnvelope(value) {
    ...
    const item = value;
    const parsedAt = typeof item.observedAt === "string" ? Date.parse(item.observedAt) : NaN;
    if (!safeText(item.toolCallId, 256) ||
    ...
    if (item.phase === "started") {
        ...
        let encoded;
        try {
            if (item.inputUnavailable === undefined &&
...
export function toolAuditSpoolPath(sessionKey, piSessionId) {
    if (!safeText(piSessionId, 256))
        throw new Error("Pi session ID is invalid for audit spooling.");
    return join(stateRootForSession(sessionKey), "tool-audit-spool-v1", `${sha256(piSessionId)}.json`);
}
export class ToolAuditReporter {
    ...
    async configure(sessionKey, piSessionId) {
        const path = toolAuditSpoolPath(sessionKey, piSessionId);
        if (this.#spoolPath === path && this.#loaded)
            return;
        if (this.#loaded)
            await this.interrupt("PI_SESSION_REPLACED");
        this.#loaded = false;
        this.#spoolPath = path;
        await ensurePrivateDirectory(join(stateRootForSession(sessionKey), "tool-audit-spool-v1"));
        this.#queue = [];
    ...
    async captureStart(event) {
        if (!safeText(event.toolCallId, 256) ||
        ...
        const bytes = canonical === undefined
            ? undefined
            : Buffer.byteLength(canonical, "utf8");
        const envelope = canonical === undefined
            ? {
                phase: "started",
                observedAt,
        ...
        if (this.#pending.has(event.toolCallId) ||
            ...
            this.#onGap("Tool audit pending-call limit was reached; completion duration may be unavailable.");
        await this.#enqueue(envelope);
    }
    async captureEnd(event) {
        if (!safeText(event.toolCallId, 256) ||
            !safeText(event.toolName, 128) ||
            !AUDITED_ORCHESTRATION_TOOLS.has(event.toolName))
            return;
    ...
    async interrupt(reason = "PI_SESSION_SHUTDOWN") {
        const observedAtMs = this.#now();
    ...
    async flush() {
        ...
        const operation = this.#drain().catch((error) => {
            this.#onGap(error instanceof Error
    ...
    async #drain() {
        ...
        if (!client?.connected || !this.#loaded)
            return;
        while (this.#queue.length > 0 &&
            ...
            try {
                await client.request("tool_audit.ingest", envelope, {
                    timeoutMs: 5_000,
                });
            }
            catch {
    ...
    async #persist() {
        ...
        if (await pathExists(path))
            await replacePrivateRegular(path, content);
        else
            await createPrivateExclusive(path, content);
    }
}
