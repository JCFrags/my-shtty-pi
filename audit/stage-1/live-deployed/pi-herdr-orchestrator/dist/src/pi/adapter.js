        ...
        const text = typeof content === "string"
            ...
            : Array.isArray(content)
                ? content
                    ...
                    .join("\n")
                : undefined;
        if (text && Buffer.byteLength(text, "utf8") <= 65_536)
            return text;
    }
    return undefined;
}
export function capabilities(api, context) {
    return {
        core: typeof api.sendUserMessage === "function" &&
...
export class PiAdapter {
    correlator = new LifecycleCorrelator();
    #api;
    #context;
    #agentId;
    #generation;
    ...
    #activeCycleId;
    #peerAsk;
    constructor(api, context, agentId, generation) {
        this.#api = api;
    ...
    bindIdentity(agentId, generation, connectionGeneration) {
    ...
    onLifecycle(event) {
        if (event.type === "agent_end" && this.#peerAsk) {
            ...
            if (answer)
                ...
                pending.reject(new Error("PEER_ANSWER_UNAVAILABLE"));
        }
        const cycle = event.type === "agent_start"
            ? (this.#activeCycleId = `cyc_${createId("evt").slice(4)}`)
            : (event.agentCycleId ?? this.#activeCycleId);
        const result = this.correlator.lifecycle(cycle ? { ...event, agentCycleId: cycle } : event);
        if (result === "bound" || result === "settled")
            this.persistCorrelation();
        return result;
    }
    recoveryLifecyclePayload(adapterSeq) {
        const state = this.correlator.state;
        if (state.kind === "bound")
            return this.lifecyclePayload("bound", {
                type: "turn_start",
        ...
        if (state.kind === "settled")
            return this.lifecyclePayload("settled", {
                type: "agent_settled",
                agentId: state.assignment.agentId,
                generation: state.assignment.generation,
                piSessionId: state.piSessionId,
    ...
    lifecyclePayload(result, event, adapterSeq) {
        const current = this.correlator.state;
        if ((current.kind !== "bound" && current.kind !== "settled") ||
            this.#connectionGeneration === undefined ||
            !current.agentCycleId)
            return undefined;
        const turnIndex = result === "bound" ? event.turnIndex : current.firstTurnIndex;
        ...
        return {
            ...
            safeData: {
                toolName: typeof state.currentTool === "string" &&
                    Buffer.byteLength(state.currentTool, "utf8") <= 256 &&
                    !/[\u0000-\u001f\u007f]/u.test(state.currentTool)
                    ? state.currentTool
                    : null,
                contextPercent: typeof state.contextPercent === "number" &&
    ...
    restorePersisted(kind, assignment, agentCycleId, firstTurnIndex) {
        this.correlator.restorePersisted(kind, assignment, this.safeState(), agentCycleId, firstTurnIndex);
    }
    persistCorrelation() {
        const state = this.correlator.state;
        if (state.kind === "none" || state.kind === "pending")
            return;
        this.#api.appendEntry?.("pi-herdr-orchestrator-correlation", {
            ...
            generation: state.assignment.generation,
            assignmentGeneration: state.assignment.assignmentGeneration,
            piSessionId: state.kind === "accepted"
    ...
    async handleControl(method, params) {
        const state = this.safeState();
        const identity = [
            "agentId",
            "generation",
        ...
        if (identity.some((key) => params[key] !==
            (key === "agentId"
                ? state.agentId
                : key === "generation"
                    ? state.generation
        ...
        if (method === "control.prompt" ||
        ...
        else if (method === "control.abort")
            await this.abort();
        else if (method === "control.compact")
            await this.compact();
            ...
            if (!Array.isArray(params.tools) ||
                ...
                params.tools.some((name) => typeof name !== "string" || name.length === 0 || name.length > 128))
                throw new Error("INVALID_REQUEST");
        ...
        try {
            ...
            if (rebind) {
                ...
                try {
                    ...
                    throw new AggregateError([error, restoreError], "Prompt delivery and settled-assignment restoration failed.");
                }
            }
            throw error;
        }
    ...
    async steer(message) {
        ...
        if (this.#context.isIdle())
            throw new Error("AGENT_NOT_WORKING");
    ...
    async compact() {
        ...
        await new Promise((resolve, reject) => this.#context.compact({ onComplete: resolve, onError: reject }));
    }
    async setModel(provider, modelId) {
        ...
        if (accepted === false)
            throw new Error("PI_COMMAND_REJECTED");
    }
    async setThinking(level) {
        this.require("thinking");
        const choices = this.#api.getAllowedThinkingLevels?.() ??
...
export function renderAssignment(assignment) {
    const constraints = assignment.constraints.map((item) => `- ${item}`).join("\n") || "- None";
...
export function piSessionId(context) {
