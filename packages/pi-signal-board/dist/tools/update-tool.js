...
const upsertParameters = Type.Object({
    ...
    detail: Type.Optional(oneOf([Type.String({ minLength: 1, maxLength: 4000 }), Type.Null()])),
    stage: Type.Optional(oneOf([updateStage, Type.Null()])),
    progress: Type.Optional(oneOf([progress, Type.Null()])),
    attachments: Type.Optional(Type.Array(attachment, { maxItems: 10 })),
}, { additionalProperties: false });
const archiveObject = Type.Object({
    operation: StringEnum(['archive']),
    id: Type.Optional(lookupId),
    key: Type.Optional(key),
    expectedRevision: Type.Optional(Type.Integer({ minimum: 1 })),
}, { additionalProperties: false });
const archiveParameters = Type.Unsafe({
    ...archiveObject,
    anyOf: [{ required: ['id'] }, { required: ['key'] }],
});
export const UPDATE_TOOL_PARAMETERS = oneOf([upsertParameters, archiveParameters]);
export const UPDATE_TOOL_DESCRIPTION = 'create, revise, or archive a durable significant board update.';
export const UPDATE_TOOL_PROMPT_SNIPPET = 'Post or revise a durable Signals milestone, finding, warning, blocker, failure, or completion.';
...
export function registerUpdateTool(pi, lifecycle, pending) {
    pi.registerTool({
        ...
        async execute(toolCallId, input) {
            try {
                ...
                const result = input.operation === 'upsert'
                    ? await service.upsertUpdate({ ...input, commandId })
                    : await service.archiveUpdate({ ...input, commandId });
                if (!result.ok)
        ...
        renderCall(input, theme, context) {
            ...
            const identity = safeInline(input.id ?? input.key ?? ('title' in input ? input.title : undefined), 18);
            component.setText(truncateToWidth(`${theme.fg('toolTitle', theme.bold(UPDATE_TOOL_NAME))} ${theme.fg('muted', input.operation)}${identity ? ` ${theme.fg('dim', identity)}` : ''}`, 240));
            return component;
        },
        renderResult(result, options, theme, context) {
            ...
            if (!details || typeof details !== 'object' || !('ok' in details)) {
                component.setText(theme.fg('error', 'ERROR SB_INTERNAL Signals result details are unavailable.'));
            }
            else if (!details.ok) {
                component.setText(theme.fg('error', `ERROR ${details.error.code} ${safeInline(details.error.message, 65)}`));
            }
            else {
...
export function patchPendingToolFailure(event, pending) {
    if (event.toolName !== UPDATE_TOOL_NAME &&
    ...
    return {
        content: [
            { type: 'text', text: `ERROR ${failure.error.code}: ${failure.error.message}` },
        ],
        details: failure,
        isError: true,
    };
...
function successDetails(operation, result) {
