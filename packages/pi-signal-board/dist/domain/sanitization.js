 ...
 */
export function validatePersistedText(value, mode, maxCodePoints) {
...
function policy(mode, maxCodePoints) {
    return Object.freeze({ mode, maxCodePoints });
}
function sanitize(value, mode, maxCodePoints) {
...
function stripTerminalEffects(value, mode) {
    let output = '';
    let index = 0;
...
function consumeEscape(value, escapeIndex) {
...
function consumeCsi(value, contentIndex) {
...
function consumeControlString(value, contentIndex, bellTerminates) {
    let index = contentIndex;
    while (index < value.length) {
        const codeUnit = value.charCodeAt(index);
...
function failure(reason, codePointLength, maxCodePoints) {
