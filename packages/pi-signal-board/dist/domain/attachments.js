...
import { fail, fieldError, signalBoardError, succeed, } from './errors.js';
import { sanitizeText, TEXT_FIELD_POLICIES } from './sanitization.js';
 ...
 */
export function normalizeAttachments(input, cwd) {
...
function normalizeAttachment(input, index, cwd) {
    ...
    const label = sanitizedField(input, 'label', `${base}.label`, TEXT_FIELD_POLICIES.attachmentLabel);
    if (!label.ok) {
        return label;
    }
    switch (input.kind) {
        ...
        case 'command': {
            ...
            if (!keys.ok)
                return keys;
            const reference = sanitizedField(input, 'reference', `${base}.reference`, TEXT_FIELD_POLICIES.attachmentReference);
            if (!reference.ok)
                return reference;
            return succeed(Object.freeze({ kind: input.kind, label: label.value, reference: reference.value }));
        }
        case 'url':
        ...
        case 'note': {
            ...
            if (!keys.ok)
                return keys;
            const text = sanitizedField(input, 'text', `${base}.text`, TEXT_FIELD_POLICIES.attachmentNote);
            if (!text.ok)
                return text;
...
function normalizeFile(input, base, label, cwd, lineRange) {
    ...
    if (!lineRange) {
        return succeed(Object.freeze({
            ...
            ...(pathResult.value.external ? { external: true } : {}),
        }));
    }
...
function normalizeUrl(input, base, label) {
...
function sanitizedField(input, key, path, policy) {
...
function exactKeys(input, allowed, base) {
