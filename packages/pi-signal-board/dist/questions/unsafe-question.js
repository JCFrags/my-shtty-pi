 ...
 */
export function guardUnsafeQuestion(spec) {
    if (spec.class === 'authorization')
        return unsafe();
    ...
    if (!tokens.some((token) => HIGH_RISK_WORDS.has(token)))
        return succeed(spec);
...
function isLocalCodeRepresentationException(spec, tokens, questionTokens, reasonTokens) {
    if (spec.class !== 'reversible')
        return false;
    if (!hasAny(tokens, LOCAL_CONTEXT))
        return false;
    if (!hasAny(tokens, CODE_CONTEXT))
        return false;
    if (!hasAny(tokens, REPRESENTATION_CONTEXT))
        return false;
    if (!hasAny(tokens, REVERSIBLE_CONTEXT))
        return false;
    if (hasAny(tokens, NON_EXCEPTION_RISK))
        return false;
    if (hasAny(tokens, EXTERNAL_OR_EFFECT_CONTEXT))
        return false;
    return (highRiskTermsAreNamedRepresentations(questionTokens) &&
        highRiskTermsAreNamedRepresentations(reasonTokens));
}
function highRiskTermsAreNamedRepresentations(tokens) {
    for (let index = 0; index < tokens.length; index += 1) {
        if (!HIGH_RISK_WORDS.has(tokens[index] ?? ''))
            continue;
        const previous = tokens[index - 1];
        if (previous === undefined || !REPRESENTATION_BINDERS.has(previous))
            return false;
    }
    return true;
}
