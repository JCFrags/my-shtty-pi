...
import { ERROR_DEFINITIONS, type SignalBoardError } from './domain/errors.js';
import { RuntimeIdGenerator } from './domain/ids.js';
import {
  type AskUserDeferredProviderRegistration,
  registerAskUserDeferredProviderV1,
} from './integration/ask-user-deferred-provider.js';
...
export function createSignalBoardExtension(
  ...
  return (pi: ExtensionAPI): void => {
    ...
    let askUserProviderRegistration: AskUserDeferredProviderRegistration | undefined;
    ...
    askUserProviderRegistration = registerAskUserDeferredProviderV1(pi.events, {
      createQuestion: async (command) => {
        ...
        if (!access.ok) return { ok: false, error: runtimeProviderError(access.error.code) };
...
function runtimeProviderError(
  code: 'SB_NOT_INITIALIZED' | 'SB_DISABLED' | 'SB_UNSUPPORTED_HOST' | 'SB_INTERNAL',
): SignalBoardError {
