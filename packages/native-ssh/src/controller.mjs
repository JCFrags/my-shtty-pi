...
export class Controller {
  ...
  #render() { if (!this.context) return; let text = "LOCAL"; if (this.state.mode === "remote") text = this.health === "error" ? `REMOTE: ${this.state.target.displayName} [stale/error: ${this.errorCode}]` : this.leases.size ? `REMOTE: ${this.state.target.displayName} [busy:${this.leases.size}]` : `REMOTE: ${this.state.target.displayName} [native SSH]`; this.context.ui.setStatus("native-ssh", text); this.context.ui.setWidget("native-ssh", this.state.mode === "remote" ? ["REMOTE NATIVE SSH: built-in read/write/edit/bash/search tools use the selected configured host.", "OpenSSH config, host-key checks, and the configured remote account remain authoritative. User ! commands stay LOCAL."] : undefined, { placement: "belowEditor" }); }
}
