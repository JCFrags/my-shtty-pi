...
test("partial usage summaries and independent details failures preserve last-good banked data", async (t) => {
	...
	await startAfterInitialPoll(coordinator); await waitFor(() => latest?.bankedDetails !== undefined); const summaryAt = latest!.bankedSummary!.observedAt; const detailsAt = latest!.bankedDetails!.observedAt; round = 1; now += 101; await coordinator.tick();
