...
Example: `context_recall({query:"rollover", providers:["memory","todo","notes","workplan"]})`. Empty query means browse. Native adapters use up to 16 case-insensitive terms formed from Unicode letters, numbers, underscores, and hyphens. They match any term and rank results, not literal phrases. Categories are optional. Memory proposals appear only when category `proposal` is explicit. A provider's native read tool must already be active, including `memory_get` for Memory. Recall never calls `setActiveTools`, invokes tools, bypasses Progressive Tools, or enables missing tools. Use the ordinary tool-help route before a later query if needed.
...
`complete` is true only when every requested provider is ready, reports complete scanning, excludes no matching records, and reports no incomplete card fields. This is the provider's bounded current-state claim, not an independent proof of historical or semantic completeness. Memory recovery pins the card's exact record revision. Other native recovery can return a newer current revision. Logical Memory visibility and branch-local tool state remain distinct. Re-query with a narrower query/category/provider or use the supplied native recovery instead of assuming a global cursor exists.
...
Recall uses protocol V2 with four providers and eight categories. The fixed registry is not dynamic provider discovery. The library retains V1 for older three-provider clients. Query text is at most 512 UTF-8 bytes. See [the protocol](../protocol/README.md) for field and transport bounds.
...
Providers share Pi's JavaScript process. This supports cooperative exception/timeout isolation, not protection from synchronous hangs or process crashes. A timeout does not cancel arbitrary provider work. Native read tools and state stores remain independent of Recall. Chrono's V4 compiler uses this same pure collector, not a second Recall implementation or a native tool invocation.
