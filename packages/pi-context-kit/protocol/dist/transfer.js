...
import { PROVIDER_IDS, sameScope, validateScope } from "./index.js";
...
function refuse(code) { throw new StateTransferError(code); }
...
export function boundedTransferJson(value, maxBytes) {
    if (!Number.isSafeInteger(maxBytes) || maxBytes < 1 || maxBytes > STATE_TRANSFER_LIMITS.aggregateBytes)
        refuse("state-checkpoint-budget");
    let bytes = 0, nodes = 0;
    const ancestors = new Set();
    const count = (amount) => { bytes += amount; if (bytes > maxBytes)
        refuse("state-checkpoint-budget"); };
    const text = (value) => {
    ...
    const visit = (item, depth) => {
        ...
        if (typeof item === "number") {
            if (!Number.isFinite(item) || (Number.isInteger(item) && !Number.isSafeInteger(item)))
                refuse("state-checkpoint-corrupt");
            count(JSON.stringify(item).length);
            return;
        }
        if (typeof item !== "object" || ancestors.has(item))
            refuse("state-checkpoint-corrupt");
        if (Object.getPrototypeOf(item) !== (Array.isArray(item) ? Array.prototype : Object.prototype))
            refuse("state-checkpoint-corrupt");
        ancestors.add(item);
        count(2);
        if (Array.isArray(item)) {
            if (item.length > 200_000 - nodes)
                refuse("state-checkpoint-budget");
...
function requestData(value) {
    const data = JSON.parse(boundedTransferJson(value, 4096));
    if (!exact(data, ["version", "requestId", "scope", "providers", "maxBytes", "deadlineMs"]) || data.version !== 2 || !identifier(data.requestId))
        refuse("state-checkpoint-corrupt");
    ...
    if (!Array.isArray(data.providers) || data.providers.length < 1 || data.providers.length > PROVIDER_IDS.length
        || new Set(data.providers).size !== data.providers.length || data.providers.some((id) => !PROVIDER_IDS.includes(id)))
        refuse("state-checkpoint-corrupt");
...
export function validateTransferEntries(values, scope, provider, maxBytes = STATE_TRANSFER_LIMITS.aggregateBytes) {
    const data = JSON.parse(boundedTransferJson(values, maxBytes));
    if (!Array.isArray(data) || data.length > 7)
        refuse("state-checkpoint-corrupt");
    const seen = new Set();
    const sizes = new Map();
    for (const entry of data) {
        if (!exact(entry, ["customType", "data"]))
            refuse("state-checkpoint-corrupt");
        const body = entry.data;
        const native = entry.customType === NATIVE_CHECKPOINT_ENTRY;
        if (!native && entry.customType !== OWNER_BINDING_ENTRY)
            refuse("state-checkpoint-corrupt");
        if (!exact(body, ["version", "provider", "sourceSessionId", "sourceLeafId", native ? "state" : "binding"])
            || body.version !== 1 || !PROVIDER_IDS.includes(body.provider)
            || (provider !== undefined && body.provider !== provider) || (native && body.provider === "memory")
            || !identifier(body.sourceSessionId) || !(body.sourceLeafId === null || identifier(body.sourceLeafId)))
            refuse("state-checkpoint-corrupt");
        if (scope && !sameScope(scope, { sessionId: body.sourceSessionId, leafId: body.sourceLeafId }))
            refuse("state-checkpoint-scope");
        const key = `${body.provider}:${entry.customType}`;
        if (seen.has(key))
            refuse("state-checkpoint-corrupt");
        seen.add(key);
        const bytes = (sizes.get(body.provider) ?? 0) + Buffer.byteLength(JSON.stringify(entry));
        if (bytes > STATE_TRANSFER_LIMITS.providerBytes)
            refuse("state-checkpoint-budget");
        sizes.set(body.provider, bytes);
    }
    for (const id of provider ? [provider] : sizes.keys()) {
        if (!seen.has(`${id}:${id === "memory" ? OWNER_BINDING_ENTRY : NATIVE_CHECKPOINT_ENTRY}`))
            refuse("state-checkpoint-corrupt");
    }
    return data;
}
export function registerStateTransferProvider(events, provider, capture, getScope) {
    ...
    const remove = events.on(STATE_TRANSFER_REQUEST, (raw) => {
        let request;
        try {
            request = requestData(raw);
        }
        catch {
            return;
        }
        const remaining = request.deadlineMs - Date.now();
        if (!active || !request.providers.includes(provider) || remaining < 0 || remaining > STATE_TRANSFER_LIMITS.maxWaitMs)
            return;
        Object.freeze(request.scope);
        Object.freeze(request.providers);
        Object.freeze(request);
        const envelope = { version: 2, requestId: request.requestId, provider, scope: request.scope };
        const emit = (reply) => {
            if (active && Date.now() <= request.deadlineMs) {
                try {
                    events.emit(STATE_TRANSFER_RESPONSE, reply);
                }
                catch { /* Optional peer transport cannot fail native use. */ }
            }
        };
        if (jobs.size >= 1) {
            emit({ ...envelope, ok: false, code: "state-checkpoint-pending" });
            return;
        }
        ...
        const verify = () => {
            ...
            if (!active || controller.signal.aborted)
                refuse("state-checkpoint-pending");
            if (!current || !sameScope(validateScope(current), request.scope))
                refuse("state-checkpoint-scope");
        };
        void Promise.resolve().then(async () => {
            ...
            verify();
            const reply = { ...envelope, ok: true, entries };
    ...
    return () => { active = false; for (const job of jobs)
        job.abort(); remove(); };
}
...
export async function captureStateTransfer(events, getScope, options) {
    const providers = [...options.providers];
    if (providers.length > PROVIDER_IDS.length || new Set(providers).size !== providers.length
        || providers.some((id) => !PROVIDER_IDS.includes(id)))
        refuse("state-checkpoint-corrupt");
    if (!providers.length)
        return [];
    const scope = validateScope(getScope()), requestId = randomUUID();
    const waitMs = options.waitMs ?? STATE_TRANSFER_LIMITS.waitMs;
    if (!Number.isSafeInteger(waitMs) || waitMs < 1 || waitMs > STATE_TRANSFER_LIMITS.maxWaitMs)
        refuse("state-checkpoint-budget");
    const request = { version: 2, requestId, scope, providers, maxBytes: STATE_TRANSFER_LIMITS.providerBytes, deadlineMs: Date.now() + waitMs };
    ...
    await new Promise((resolve, reject) => {
        ...
        const finish = (error) => {
            ...
            catch { /* Settlement still completes. */ }
        ...
        try {
            remove = events.on(STATE_TRANSFER_RESPONSE, (raw) => {
                if (settled || !raw || typeof raw !== "object")
                    return;
                ...
                try {
                    ...
                    if (!exact(reply, ["version", "requestId", "provider", "scope", "ok", reply.ok ? "entries" : "code"])
                        || reply.version !== 2 || !sameScope(validateScope(reply.scope), scope))
                        refuse("state-checkpoint-corrupt");
                    // Other installed owners can answer the same transport. Do not import them.
                    if (!providers.includes(reply.provider))
                        return;
                    if (replies.has(reply.provider))
                        refuse("state-checkpoint-corrupt");
                    if (reply.ok !== true) {
                        const codes = ["state-checkpoint-corrupt", "state-checkpoint-pending", "state-checkpoint-scope", "state-checkpoint-budget"];
                    ...
                    if (replies.size === providers.length)
                        finish();
                }
                catch (error) {
                    finish(error instanceof StateTransferError ? error : new StateTransferError("state-checkpoint-corrupt"));
                }
            });
            ...
            finish(new StateTransferError("state-checkpoint-corrupt"));
        }
    });
    if (options.signal?.aborted || !sameScope(validateScope(getScope()), scope))
        refuse("state-checkpoint-scope");
