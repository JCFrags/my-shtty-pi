...
import type { CandidateUnit, RepresentationCandidate, SessionEntryLike } from "./types.js";
import {
...
export type ToolResultProjectionMode = "off" | "safe" | "aggressive";
...
export interface ProjectionSourceBinding {
...
export interface ToolResultProjectionOptions {
  ...
  readonly minimumSourceTokens?: number;
  readonly minimumSavedTokens?: number;
...
export interface ToolResultProjectionMetrics {
  ...
  readonly exactRecoveryCovered: number;
  readonly keptRecent: number;
  readonly keptFirstConsumption: number;
  readonly protectedResults: number;
  readonly tooSmallResults: number;
  readonly refusedResults: number;
  readonly reducerFamilies: Readonly<Record<string, { readonly results: number; readonly removedTokens: number }>>;
  readonly refusalReason?: string;
}

export interface ToolResultProjectionResult<T extends ContextMessageLike = ContextMessageLike> {
...
function sourceFingerprint(message: ContextMessageLike): string {
...
function contentText(content: unknown): { text: string; hasImage: boolean; hasUnsupportedContent: boolean } {
  if (typeof content === "string") return { text: content, hasImage: false, hasUnsupportedContent: false };
  const blocks = getArray(content);
  if (!blocks) return { text: "", hasImage: false, hasUnsupportedContent: content !== undefined };
  const text: string[] = [];
  let hasImage = false;
  ...
  for (const item of blocks) {
    ...
    else hasUnsupportedContent = true;
  }
  return { text: text.join("\n"), hasImage, hasUnsupportedContent };
}

function toolCalls(message: ContextMessageLike): Array<{ id: string; name: string }> {
...
function detailRecords(value: unknown, depth = 0): Readonly<Record<string, unknown>>[] {
  if (depth > 3) return [];
...
function terminalOutcomeKnownSuccessful(info: ResultInfo<ContextMessageLike>): boolean {
  if (!TERMINAL_TOOL.test(info.toolName)) return true;
...
function hasStructuredFailure(message: ContextMessageLike): boolean {
  ...
  for (const record of detailRecords(message.details)) {
    ...
    for (const key of ["cancelled", "canceled", "aborted"] as const) if (record[key] === true) return true;
    const status = getString(record.status) ?? getString(record.state);
    if (status && /^(?:error|fail(?:ed|ure)?|abort(?:ed)?|cancel(?:led|ed)?)$/i.test(status.trim())) return true;
  }
  return false;
}

function laterUserCites(messages: readonly ContextMessageLike[], info: ResultInfo<ContextMessageLike>): boolean {
...
function isProtectedProjectionResult(
  messages: readonly ContextMessageLike[],
  info: ResultInfo<ContextMessageLike>,
...
function safeDetails(value: unknown): Record<string, unknown> {
  ...
  for (const [key, item] of Object.entries(record).sort(([left], [right]) => left.localeCompare(right))) {
    if (!IMPORTANT_DETAIL_KEY.test(key) || Object.keys(result).length >= 32) continue;
    if (item === null || typeof item === "boolean" || typeof item === "number") result[key] = item;
    else if (typeof item === "string" && item.length <= 512) result[key] = item;
  }
  return result;
}

function syntheticEntries<T extends ContextMessageLike>(
  messages: readonly T[],
  resultEntryIds: ReadonlyMap<number, string>,
): SessionEntryLike[] {
...
function candidateForProjection(
  unit: CandidateUnit | undefined,
  mode: ToolResultProjectionMode,
  minimumSavedTokens: number,
): RepresentationCandidate | undefined {
  if (!unit) return undefined;
  const eligible = unit.candidates.filter((candidate) => {
    if (!candidate.lossy || candidate.tokens + minimumSavedTokens > unit.rawTokens) return false;
    if (candidate.level === "raw" || candidate.level === "normalized" || candidate.level === "absent") return false;
    if (mode === "safe" && (candidate.level === "marker" || candidate.reducer === "historical-marker")) return false;
    return true;
  });
  if (mode === "aggressive") {
...
function emptyMetrics(mode: ToolResultProjectionMode, sourceTokens: number, refusalReason?: string): ToolResultProjectionMetrics {
  return {
    mode,
    sourceTokens,
...
function requestRefusal<T extends ContextMessageLike>(
  messages: readonly T[],
  mode: ToolResultProjectionMode,
  sourceTokens: number,
  resultCount: number,
  newlySeenToolCallIds: ReadonlySet<string>,
  refusalReason: string,
): ToolResultProjectionResult<T> {
  return {
    messages,
    metrics: {
...
export async function projectToolResultContext<T extends ContextMessageLike>(
  messages: readonly T[],
  options: ToolResultProjectionOptions = {},
  ...
  let pairRefusal: string | undefined;
  messages.forEach((message, index) => {
    for (const call of toolCalls(message)) {
      if (calls.has(call.id)) pairRefusal = `duplicate tool call ID ${call.id}`;
      else calls.set(call.id, { index, name: call.name });
    }
  });

  const results: Array<ResultInfo<T>> = [];
  ...
  messages.forEach((message, index) => {
    ...
    results.push(info);
    if (!options.seenToolCallIds?.has(toolCallId)) newlySeen.add(toolCallId);
  });
  if (pairRefusal) {
    return {
      messages,
  ...
  const validatedSources = new Map<string, ProjectionSourceBinding>();
  ...
  for (const info of results) {
    ...
    if (!source) {
      return requestRefusal(
        ...
        results.length,
        newlySeen,
        `missing authoritative source binding for call ID ${info.toolCallId}`,
      );
    }
    if (typeof source.entryId !== "string" || source.entryId.length === 0) {
      return requestRefusal(
        ...
        results.length,
        newlySeen,
        `invalid authoritative source entry identity for call ID ${info.toolCallId}`,
      );
    }
    if (source.toolCallId !== info.toolCallId) {
      return requestRefusal(
        ...
        results.length,
        newlySeen,
        `authoritative source call ID mismatch for call ID ${info.toolCallId}`,
      );
    }
    if (source.sourceFingerprint !== sourceFingerprint(info.message)) {
      return requestRefusal(
        ...
        results.length,
        newlySeen,
        `authoritative source fingerprint mismatch for call ID ${info.toolCallId}`,
      );
    }
    ...
    if (entryOwner !== undefined && entryOwner !== info.toolCallId) {
      return requestRefusal(
        ...
        results.length,
        newlySeen,
        `authoritative source entry identity ${source.entryId} is shared by call IDs ${entryOwner} and ${info.toolCallId}`,
      );
    }
  ...
  if (unsupported) {
    return requestRefusal(
      ...
      results.length,
      newlySeen,
      `unsupported tool result content for call ID ${unsupported.toolCallId}`,
    );
  }
  ...
  for (const indexes of indexesByContent.values()) {
    if (indexes.length > 1 && indexes[0] !== undefined) canonicalIndexes.add(indexes[0]);
  }
...
export function projectionSourcesFromBranch(entries: readonly SessionEntryLike[]): ReadonlyMap<string, ProjectionSourceBinding> {
  const result = new Map<string, ProjectionSourceBinding>();
  ...
  for (const entry of entries) {
    ...
    result.set(toolCallId, {
      entryId: entry.id,
      toolCallId,
      sourceFingerprint: sourceFingerprint(message as ContextMessageLike),
    });
  }
  return result;
}

export function validateProjectedToolPairs(messages: readonly ContextMessageLike[]): { readonly ok: boolean; readonly reason: string } {
  const calls = new Map<string, number>();
  const results = new Set<string>();
  for (let index = 0; index < messages.length; index += 1) {
    const message = messages[index]!;
    for (const call of toolCalls(message)) {
      if (calls.has(call.id)) return { ok: false, reason: `duplicate call ${call.id}` };
      calls.set(call.id, index);
    }
    if (message.role !== "toolResult") continue;
    const id = getString(message.toolCallId);
    if (!id || !calls.has(id) || (calls.get(id) ?? index) >= index) return { ok: false, reason: `orphan result at ${index}` };
    if (results.has(id)) return { ok: false, reason: `duplicate result ${id}` };
    results.add(id);
  }
