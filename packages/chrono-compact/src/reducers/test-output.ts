...
export function looksLikeTestOutput(text: string): boolean {
  return /(?:\bTests?:|\bTest Files?:|\btest result:|\bpytest\b|\bvitest\b|\bjest\b|\bmocha\b|\bcargo test\b|\bgo test\b|\d+\s+(?:passed|failed|skipped))/i.test(
    text,
  );
}

function frameworkFor(text: string): string | undefined {
  if (/vitest/i.test(text)) return "Vitest";
  if (/jest/i.test(text)) return "Jest";
  if (/pytest|short test summary info/i.test(text)) return "pytest";
  if (/test result:\s*(?:ok|FAILED)/i.test(text)) return "Rust test harness";
  if (/---\s+(?:PASS|FAIL):|go test/i.test(text)) return "Go test";
  if (/mocha|\d+ passing/i.test(text)) return "Mocha";
  if (/TAP version|not ok \d+/i.test(text)) return "TAP";
  return undefined;
}
...
function parseTotals(text: string): Totals {
  const totals: Totals = {};
  const patterns: Array<[keyof Totals, RegExp[]]> = [
...
function pairedCommand(context: ReducerContext): string | undefined {
  const args = context.block.toolArguments;
  return getString(args?.command) ?? getString(context.block.attributes.command);
}

function exitCode(context: ReducerContext): number | undefined {
...
function collectFailureSections(lines: readonly string[]): string[] {
  ...
  lines.forEach((line, index) => {
    ...
    if (PASSING_NOISE.test(line) && !/(?:FAIL(?:ED)?|ERROR|AssertionError|expected|received|actual|snapshot|timed out)/i.test(line)) return;
...
export function reduceTestOutput(context: ReducerContext): ReducerResult {
  ...
  if (code !== undefined) sections.push(`Exit code: ${code}`);
  if (framework) sections.push(`Framework: ${framework}`);
  const totalParts: string[] = [];
  if (totals.passed !== undefined) totalParts.push(`${totals.passed} passed`);
  if (totals.failed !== undefined) totalParts.push(`${totals.failed} failed`);
  if (totals.skipped !== undefined) totalParts.push(`${totals.skipped} skipped`);
  if (totals.total !== undefined) totalParts.push(`${totals.total} total`);
  if (totalParts.length > 0) sections.push(`Tests: ${totalParts.join(", ")}`);
  if (namedFailures.length > 0) sections.push(`\nFailing tests:\n${namedFailures.map((line) => `- ${line}`).join("\n")}`);
  if (failureLines.length > 0) sections.push(`\nFailure evidence (exact excerpts):\n${failureLines.join("\n")}`);
