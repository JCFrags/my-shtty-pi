...
`summary-rebase.ts` decides when the periodic or recursive-summary threshold fires. On that generation, `pi-extension.ts` calls Pi's regular summary path with original raw source messages and no previous summary. It does not use ChronoCompact replay or prior combined output as summary input. Chronological replay always rebuilds from original source on every generation.
