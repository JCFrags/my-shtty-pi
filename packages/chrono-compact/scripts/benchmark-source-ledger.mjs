...
const LIMITS = { "final-tasks": 5000, batches: 100 };
const HELP = "Usage: benchmark-source-ledger.mjs --final-tasks N --batches N";
let runtime;
...
export function parseSourceLedgerArguments(argv) {
  ...
  for (let index = 0; index < argv.length; index += 2) {
    ...
    if (!Object.hasOwn(LIMITS, name)) throw new Error(`Unknown argument: ${argument}.`);
    if (Object.hasOwn(values, name)) throw new Error(`Duplicate argument: ${argument}.`);
    values[name] = integer(name, argv[index + 1]);
  }
  for (const name of Object.keys(LIMITS)) if (!Object.hasOwn(values, name)) throw new Error(`Missing value for --${name}.`);
  return values;
}
...
export async function main(argv = process.argv.slice(2)) {
  ...
  console.log(JSON.stringify(await runSourceLedgerBenchmark(args["final-tasks"], args.batches)));
}
