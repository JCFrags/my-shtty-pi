#!/usr/bin/env node
// @ts-nocheck
import{appendFile,mkdtemp,readFile,rm,stat,writeFile}from'node:fs/promises';import{tmpdir}from'node:os';import{join}from'node:path';import{pathToFileURL}from'node:url';import{performance}from'node:perf_hooks';import{syntheticEntries}from'./benchmark-v2.mjs';
const limits = {
  "final-tasks": [1, 10000],
  tasks: [1, 10000],
  batches: [1, 100],
...
export async function runHistoryRollupBenchmark(arguments_) {
  ...
  const runtime = await import(pathToFileURL(join(process.cwd(), "dist/src/index.js")));
  try {
