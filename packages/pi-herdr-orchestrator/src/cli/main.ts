...
function auditFilters(
  ...
  options: { includeStatus?: boolean; includeLimit?: boolean } = {},
...
export async function main(argv = process.argv.slice(2)): Promise<void> {
  ...
  if (command === "audit") {
    const method =
      subcommand === "list" || subcommand === "failures"
        ? "tool_audit.list"
        : subcommand === "show"
          ? "tool_audit.get"
          : subcommand === "stats"
            ...
            : subcommand === "verify"
              ...
              : undefined;
    if (!method)
      throw new Error("Usage: audit list|show|failures|stats|verify.");
    let params: Record<string, unknown>;
    if (subcommand === "show") {
      const invocationId = argv[2];
      if (!invocationId)
        throw new Error("Usage: audit show INVOCATION_SHA256.");
      params = { invocationId };
    } else if (subcommand === "verify") params = {};
    else if (subcommand === "stats") {
      params = auditFilters(argv, { includeLimit: false });
      const groupBy = option(argv, "--group-by");
      if (groupBy !== undefined)
        params.groupBy = groupBy.split(",").filter(Boolean);
    } else {
      params = auditFilters(argv);
      if (subcommand === "failures") params.status = "failed";
    }
    await ensureBroker();
    console.log(
      JSON.stringify(
  ...
  console.error(
    "Usage: doctor [--json] | broker start|stop|restart|status | audit list|show|failures|stats|verify | events verify | config validate PATH | recovery plan|export|adopted-lineage --plan PRIVATE_JSON | retention plan|policy-plan | ops plan|verify|apply | export --output DIR | version",
  );
  process.exitCode = 2;
}
