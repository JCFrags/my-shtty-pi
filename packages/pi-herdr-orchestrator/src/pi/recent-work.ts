...
/**
 * Use the bundled deterministic ChronoCompact runtime on already-safe
 * recent-work items. Raw user messages, reasoning, tool arguments, and
 * tool-result bodies never enter this boundary. Any load, reduction, planning,
 * or validation failure returns the deterministic built-in projection.
 */
export async function projectRecentWorkWithChrono(options: {
  ...
  try {
    ...
    const maxBytes = options.maxBytes ?? DEFAULT_RECENT_WORK_BYTES;
    const safeSourceBytes = items.reduce((total, item) => {
      const text =
        item.kind === "assistant"
          ? item.text
          : item.kind === "tool"
    ...
    const estimatedSourceTokens = Math.ceil(safeSourceBytes / 4);
    if (
      maxBytes >= 16_384 &&
      items.length >= 4 &&
      estimatedSourceTokens >= 384
    ) {
      const outputTokenCap = Math.min(
        25_000,
        Math.floor((maxBytes - 2_048) / 4),
      );
      const targetTokens = Math.max(
        256,
        Math.min(outputTokenCap, Math.floor(estimatedSourceTokens * 0.7)),
      );
      try {
        ...
        // Keep the valid Chrono-reduced items when richer planning is rejected.
      }
    }
