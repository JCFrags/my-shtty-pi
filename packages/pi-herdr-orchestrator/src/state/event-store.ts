import { isHistoricalActorKind } from "./historical-actor.js";
...
const CURRENT_ACTOR_KINDS = new Set([
  "human",
  "cli",
  "pi_parent",
  "pi_child",
...
export class EventStore {
      ...
      for await (const line of readPrivateLines(this.path, (stat) => {
        ...
        const verifiedPrevious =
          snapshot && index === snapshot.lastEventSeq
            ...
            : previous;
        this.verifyEvent(event, verifiedPrevious, true);
        previous = event;
  ...
  private verifyEvent(event: StoredEvent, previous?: StoredEvent, historical = false): void {
    ...
    const baseValid =
      ...
      (CURRENT_ACTOR_KINDS.has(actor.kind) ||
        (historical && isHistoricalActorKind(actor.kind))) &&
      !!refs &&
      typeof refs === "object" &&
      !Array.isArray(refs) &&
      !!payload &&
