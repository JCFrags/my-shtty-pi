...
export function projectAgentLifecycle(
  ...
  const managedResourceState = state.herdrResources?.[agent.id]?.state;
  if (managedResourceState === "closed")
    return {
      lifecycleClass,
      keepForReuse,
...
export function agentWithLifecycle(
  ...
  return {
    ...
    ...(managedResourceState ? { managedResourceState } : {}),
    ...(progress ? { progress } : {}),
  };
}
