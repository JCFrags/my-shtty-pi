...
assert.equal(
  ...
  "file:vendor/pi-chrono-compact-2.0.0.tgz",
);
...
try {
  ...
  const bundledFiles = files.filter((file) =>
    file.startsWith("package/node_modules/"),
  );
  assert.ok(
    bundledFiles.includes(
      "package/node_modules/pi-chrono-compact/package.json",
    ),
    "package is missing bundled pi-chrono-compact",
  );
  assert.equal(
    bundledFiles.every((file) =>
      file.startsWith("package/node_modules/pi-chrono-compact/"),
    ),
    true,
    "package includes an unexpected bundled dependency",
  );
