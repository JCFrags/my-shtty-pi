...
async function runCase(kind: "cancel" | "deadline"): Promise<void> {
  ...
  try {
    ...
    let controlDelivered = expectControl();
    ...
    assert.equal(broker.store.state.tasks[taskId]?.state, "running");
    ...
    const steerResult = await parent.request(
      "task.steer",
      {
        taskId,
        message: "Inspect the current failure before editing.",
        timeoutMs: 2_000,
      },
      { idempotencyKey: `steer-${kind}`, timeoutMs: 5_000 },
    );
    assert.equal((steerResult as any).state, "delivered");
    const steeringControl = await bounded(
      controlDelivered,
      "steering control delivery",
    );
    assert.equal(steeringControl.request.method, "control.steer");
    assert.equal(steeringControl.request.params.taskId, taskId);
    assert.equal(steeringControl.request.params.runId, runId);
    assert.equal(
      steeringControl.request.params.assignmentGeneration,
      assignmentGeneration,
    );
    assert.match(steeringControl.request.params.commandId, /^cmd_/u);
    assert.equal(
      childBoundary.messages.at(-1)?.message,
      "Inspect the current failure before editing.",
    );
    assert.deepEqual(childBoundary.messages.at(-1)?.options, {
      deliverAs: "steer",
    });
    const replay = await parent.request(
      "task.steer",
      {
        taskId,
        message: "Inspect the current failure before editing.",
    ...
    assert.equal((replay as any).commandId, (steerResult as any).commandId);
    assert.equal(capturedControls.length, 1);
    controlDelivered = expectControl();
    ...
    const terminalReceipt = ownReceipt(
      ...
      (event) =>
        (kind === "cancel" &&
          event.type === "run.state_changed" &&
          event.entityRefs?.runId === runId &&
          (event.payload as any)?.state === "cancelled") ||
        (kind === "deadline" &&
          event.type === "run.state_changed" &&
          event.entityRefs?.runId === runId &&
    ...
    if (kind === "cancel") {
      ...
      assert.deepEqual(cancelResult, { taskId, state: "cancelling" });
      assert.equal(broker.store.state.tasks[taskId]?.state, "cancelling");
      assert.equal(broker.store.state.runs[runId]?.state, "working");
    } else {
    ...
    assert.equal(capturedControls.length, 2);
    ...
    if (kind === "cancel") {
      assert.equal(terminalEvidence[0]?.type, "task.cancel_requested");
      assert.deepEqual(terminalEvidence[0]?.entityRefs, { taskId });
      assert.deepEqual(terminalEvidence[0]?.payload, {
        taskId,
        reason: "controlled cancellation",
        cascade: true,
        completionMode: "quiescent",
      });
      const settled = childAdapter.onLifecycle({
        type: "agent_settled",
        agentId,
        generation: 1,
        piSessionId: "child-session",
    ...
    assert.equal(terminalEvidence.at(-1), terminalEvent);
    assert.equal(
  ...
  await orderedCleanup(primaryError, [
    ...
    async () => {
      assert.equal(terminalEvidence?.length, kind === "cancel" ? 2 : 1);
      if (terminalEvent) assert.equal(terminalEvidence?.at(-1), terminalEvent);
