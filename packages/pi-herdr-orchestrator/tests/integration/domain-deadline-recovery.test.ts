...
test("broker restart preserves a completion-driven task past the legacy wall limit", async () => {
  const root = await mkdtemp(join(tmpdir(), "completion-restart-"));
  const runtime = await mkdtemp(join(tmpdir(), "completion-restart-runtime-"));
  const paths = makePaths(root, runtime);
  const now = Date.parse("2026-01-01T00:00:00.000Z");
  ...
  try {
    await broker.stop();
    const taskId = "tsk_01J1Q0Q0Q0Q0Q0Q0Q0Q0Q0Q0Q2";
    await broker.store.append({
      type: "task.created_m3",
      actor,
      entityRefs: { taskId },
      payload: {
        ...
        objective: "continue until terminal",
        createdAt: new Date(now).toISOString(),
        completionPolicy: "until_terminal",
      },
    });
    broker = new Broker(paths, { now: () => now + 48 * 60 * 60_000 });
    await broker.start();
    assert.equal(broker.store.state.tasks[taskId]?.state, "queued");
    assert.equal(
    ...
    assert.equal(
      Object.values(broker.store.events).some(
        (event) =>
...
test("production restart rejects question delivery before one exact fallback stop", async () => {
  ...
  const stops: unknown[] = [];
  const stopAgentIds: Array<string | undefined> = [];
  const make = () =>
    new Broker(paths, {
      now: () => now,
      herdrFactory: async (store) =>
        ({
          store,
          startupReconcile: async () => [],
          stop: async (identity: unknown, expectedAgentId?: string) => {
            stops.push(identity);
            stopAgentIds.push(expectedAgentId);
            stopResolve();
          },
        }) as never,
    });
  ...
  try {
    ...
    assert.deepEqual(stopAgentIds, [agentId]);
    assert.equal(
