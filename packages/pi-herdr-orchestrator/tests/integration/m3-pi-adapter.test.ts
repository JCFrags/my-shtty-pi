...
test("monitor snapshot returns bounded assignment-scoped recent work after settlement", async () => {
  const fake = new FakePi(
    assignment.agentId,
    assignment.generation,
    assignment.piSessionId,
  ...
  const entries: unknown[] = [
    ...
    },
    {
      ...
      message: {
        role: "assistant",
        content: [
          { type: "thinking", thinking: "not exported" },
          { type: "text", text: "Verified the adapter projection." },
        ],
      },
    },
  ];
  fake.context.sessionManager.getBranch = () => entries;
  ...
  const result = await fake.adapter.handleControl("control.monitor_snapshot", {
    agentId: assignment.agentId,
    generation: assignment.generation,
    piSessionId: assignment.piSessionId,
    taskId: assignment.taskId,
  ...
  assert.equal(result.recentWork?.taskId, assignment.taskId);
  assert.deepEqual(result.recentWork?.items, [
    {
      kind: "assistant",
      text: "Verified the adapter projection.",
    },
  ]);
  assert.equal(JSON.stringify(result).includes("not exported"), false);

  await assert.rejects(
    fake.adapter.handleControl("control.monitor_snapshot", {
      agentId: assignment.agentId,
      generation: assignment.generation,
      piSessionId: assignment.piSessionId,
      taskId: assignment.taskId,
      runId: "run_01J00000000000000000000099",
      assignmentId: assignment.id,
      assignmentGeneration: assignment.assignmentGeneration,
      maxItems: 12,
      maxBytes: 8_192,
    }),
    /PI_IDENTITY_MISMATCH/u,
  );
});
