...
class BoundaryHerdr {
  ...
  managedIdentities: Array<{
    paneId: string;
    terminalId?: string;
    sessionReference?: Record<string, string>;
  }> = [];
  failMismatch = false;
  enrichManaged = false;
  ...
  async verifyManagedPane(
    ...
    this.managedCalls++;
    this.managedIdentities.push(identity);
    if (!identity.sessionReference)
      throw new Error("MANAGED_SESSION_REFERENCE_DROPPED");
    if (this.enrichManaged && this.managedCalls === 1)
      return {
        paneId: identity.paneId,
    ...
    return {
      ...
      tabId: "tab-1",
      cwd:
        this.enrichManaged || this.managedCalls < 3
          ? "/checkout"
          : "/replacement",
...
async function managedCase(
  failMismatch: boolean,
  enrichManaged = false,
): Promise<{
  root: string;
  broker: Broker;
  ...
  try {
    ...
    herdr.enrichManaged = enrichManaged;
    ...
    if (enrichManaged) await registration();
    else
      await assert.rejects(
        registration,
...
test("managed registration accepts canonical identity enrichment at the response boundary", async () => {
  const { root, broker, herdr, clients } = await managedCase(false, true);
  try {
    assert.equal(herdr.managedCalls, 3);
    assert.equal(herdr.mismatchCalls, 0);
    assert.deepEqual(
      herdr.managedIdentities.map((identity) => identity.sessionReference),
      Array.from({ length: 3 }, () => ({
        source: "herdr:pi",
        agent: "pi",
        kind: "id",
    ...
    const resources = Object.values(broker.store.state.herdrResources ?? {});
    assert.equal(resources.length, 1);
    assert.equal(resources[0]?.state, "registered");
    assert.equal(
