...
test("reconciliation recognizes a live agent listed separately from its pane", () => {
  const id = agentId();
  const results = reconcileAgents(
      ...
      panes: [
        {
          id: "p1",
          terminalId: "t1",
          occupant: { terminalId: "stale-terminal" },
        },
      ],
      tabs: [],
      workspaces: [],
      agents: [{ id, kind: "pi", paneId: "p1", terminalId: "t1" }],
      worktrees: [],
    },
  );
  assert.equal(results[0]?.kind, "present");
  assert.equal(results[0]?.terminalId, "t1");
});
test("reconciliation rebinds a restored terminal only through the same durable Pi session", () => {
  const id = agentId();
  const results = reconcileAgents(
    [
      {
  ...
  assert.equal(results[0]?.terminalId, "t2");
});
test("reconciliation refuses terminal rebind when the durable Pi session is duplicated", () => {
  const id = agentId();
  const results = reconcileAgents(
    [
      {
...
test("reconciliation rejects a different canonical agent even with matching pane and session", () => {
  const id = agentId();
  const results = reconcileAgents(
    [
      {
...
test("reconciliation rejects an ambiguously occupied modern pane", () => {
  const id = agentId();
  const results = reconcileAgents(
    [
      {
...
test("reconciliation rejects a different canonical legacy occupant", () => {
  const id = agentId();
  const results = reconcileAgents(
    [{ id, state: "idle", generation: 1, paneId: "p1", terminalId: "t1" }],
    {
      panes: [
        {
          id: "p1",
          terminalId: "t1",
          occupant: { id: agentId(), terminalId: "t1", generation: 1 },
        },
      ],
      tabs: [],
      workspaces: [],
...
test("reconciliation rejects a legacy generation mismatch or omission", () => {
  const id = agentId();
  const resource = {
    [id]: {
      agentId: id,
  ...
  for (const occupant of [
    { agentId: id, terminalId: "t1", generation: 2 },
    { agentId: id, terminalId: "t1" },
  ]) {
    const results = reconcileAgents(
      [{ id, state: "idle", generation: 1, paneId: "p1", terminalId: "t1" }],
      {
        panes: [{ id: "p1", terminalId: "t1", occupant }],
        tabs: [],
        workspaces: [],
        agents: [],
        worktrees: [],
...
test("reconciliation rejects a modern generation mismatch", () => {
  const id = agentId();
  const results = reconcileAgents(
    [
      {
...
test("reconciliation rejects canonical omission without durable session proof", () => {
  const id = agentId();
  const results = reconcileAgents(
    [{ id, state: "idle", generation: 1, paneId: "p1", terminalId: "t1" }],
    {
      ...
      tabs: [],
      workspaces: [],
      agents: [{ kind: "pi", paneId: "p1", terminalId: "t1" }],
      worktrees: [],
    },
  );
  assert.equal(results[0]?.kind, "replaced");
