...
test("parent-bound broker vertical path provisions, assigns, correlates, bounds, replays, and denies cross-parent access", async () => {
  ...
  try {
    ...
    const storedTask = broker.store.state.tasks[taskId]!;
    assert.ok(storedTask.project?.advisoryModelReceipt);
    const unavailableMonitor = ok(
      await send(p1.socket, "task.get", {
        taskId,
        include: ["recentWork"],
      }),
    );
    assert.deepEqual(unavailableMonitor.recentWorkUnavailable, {
      reason: "agent_unavailable",
    });
    assert.equal("recentWork" in unavailableMonitor, false);
    assert.equal("objective" in unavailableMonitor, false);
    assert.equal("constraints" in unavailableMonitor, false);
    assert.equal("project" in unavailableMonitor, false);
    assert.ok(Buffer.byteLength(JSON.stringify(unavailableMonitor)) <= 8_192);
    const detailedTask = ok(
      await send(p1.socket, "task.get", {
        taskId,
        include: ["definition", "lineage", "project"],
        maxBytes: 32_768,
      }),
    );
    assert.equal(detailedTask.objective, storedTask.objective);
    assert.deepEqual(detailedTask.constraints, storedTask.constraints);
    assert.equal(detailedTask.parentAgentId, storedTask.parentAgentId);
    assert.equal(detailedTask.workflowId, storedTask.workflowId);
    assert.deepEqual(detailedTask.project, storedTask.project);
    ...
    let monitorSnapshotParams: Record<string, unknown> | undefined;
    const child = await connect(
      ...
      (frame, socket) => {
        ...
        if (frame.method === "control.monitor_snapshot") {
          monitorSnapshotParams = frame.params;
          socket.write(
            encodeFrame({
              v: 1,
              type: "server_response",
              ...
              result: {
                ...
                recentWork: {
                  ...
                  items: [
                    {
                      kind: "assistant",
                      text: "Verified the vertical monitor route.",
                    },
                  ],
                  sourceEntryCount: 2,
                  omittedEntryCount: 0,
    ...
    const monitoredTask = ok(
      await send(p1.socket, "task.get", {
        taskId,
        include: ["recentWork"],
      }),
    );
    assert.deepEqual(monitoredTask.recentWork, {
      schemaVersion: 2,
      taskId,
      runId,
      source: "live_child_session",
    ...
    assert.deepEqual(monitorSnapshotParams, {
      taskId,
      runId,
      assignmentId,
    ...
    assert.ok(Buffer.byteLength(JSON.stringify(expandedMonitor)) <= 32_768);
    assert.deepEqual(monitorSnapshotParams, {
      taskId,
      runId,
      assignmentId,
    ...
    const ownTaskWait = ok(
      await send(p1.socket, "coordination.wait", {
        kind: "task",
        targetId: taskId,
        until: ["blocked"],
        timeoutMs: 1_000,
    ...
    assert.equal(ownTaskWait.value.id, taskId);
    const deniedTaskWait = await send(p2.socket, "coordination.wait", {
      kind: "task",
      targetId: taskId,
      until: ["blocked"],
      timeoutMs: 1_000,
    ...
    assert.equal(deniedTaskWait.ok, false);
    assert.equal(deniedTaskWait.error?.code, "PERMISSION_DENIED");
    ok(
    ...
    assert.equal(inspected.id, taskId);
    assert.equal(inspected.state, "succeeded");
    assert.equal("objective" in inspected, false);
    assert.equal("constraints" in inspected, false);
    assert.equal("project" in inspected, false);
    assert.deepEqual(
      inspected.runs.map((run: { id: string; state: string }) => ({
        id: run.id,
        state: run.state,
      })),
      [{ id: runId, state: "succeeded" }],
    );
    assert.ok(Buffer.byteLength(JSON.stringify(inspected)) <= 3_000);
    const listed = ok(
...
test("a pending foundation request does not block broker-owned agent creation", async () => {
  ...
  try {
    ...
    const taskId = spawned.tasks?.[0]?.taskId as string;
    assert.equal(
      broker.store.state.tasks[taskId]?.completionPolicy,
      "until_terminal",
    );
    assert.equal(broker.store.state.tasks[taskId]?.timeoutAt, undefined);
    const runId = broker.store.state.tasks[taskId]?.currentRunId;
    assert.equal(
      runId ? broker.store.state.runs[runId]?.timeoutAt : undefined,
      undefined,
    );
  } finally {
    resolveSource(
