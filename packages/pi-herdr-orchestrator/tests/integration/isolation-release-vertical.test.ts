...
test("direct spawn waits for endpoint capacity until cancellation releases it", async () => {
  ...
  try {
    ...
    assert.deepEqual(
      ...
      { taskId: firstTaskId, state: "cancelling" },
    );
    const secondProvision = await h.nextProvision();
    secondProvision.release();
    assert.equal(h.provisions.length, 2);
...
test("spawn and read responses expose derived startup progress", async () => {
  ...
  try {
    ...
    assert.notEqual(child.progress?.deadlineKind, "task");
    assert.equal(
      h.broker.store.state.tasks[child.taskId]?.completionPolicy,
      "until_terminal",
    );
    assert.equal(
      h.broker.store.state.tasks[child.taskId]?.timeoutAt,
      undefined,
    );
