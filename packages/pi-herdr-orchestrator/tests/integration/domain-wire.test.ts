...
class DomainHerdr {
  ...
  readonly stops: any[] = [];
  readonly stopAgentIds: Array<string | undefined> = [];
  ...
  async stop(guard: any, expectedAgentId?: string) {
    this.stops.push(guard);
    this.stopAgentIds.push(expectedAgentId);
    this.onStop?.();
    this.#resolveStop();
  }
...
test("adopted root preserves lineage when Herdr replaces its terminal", async () => {
  const root = await mkdtemp(join(tmpdir(), "domain-adopted-terminal-"));
  const runtime = await mkdtemp(
    join(tmpdir(), "domain-adopted-terminal-runtime-"),
  );
  const paths = {
    sessionKey: sessionKey(join(runtime, "broker.sock")),
    root,
  ...
  let first: Socket | undefined;
  let replacement: Socket | undefined;
  const registration = (terminalId: string, sessionId = "pi-session-a") => ({
    adapterVersion: "0.1.0",
    herdr: {
      paneId: "adopted-pane",
      terminalId,
  ...
  try {
    ...
    const initial = resultOf(
      await request(
        first,
        "agent.register_adopted",
        registration("adopted-terminal-a"),
      ),
    );
    const childAgentId = createId("agt");
    await broker.store.append({
      ...
      entityRefs: { agentId: childAgentId },
      payload: {
        agentId: childAgentId,
        managed: true,
        generation: 1,
        parentAgentId: initial.agentId,
        paneId: "child-pane",
    ...
    const task = resultOf(
      await request(first, "task.create_m3", {
        title: "lineage task",
        objective: "remain visible after terminal replacement",
        parentAgentId: initial.agentId,
      }),
    );

    const activeReplacement = await connect(paths, secret);
    const activeAttempt = await request(
      activeReplacement,
      "agent.register_adopted",
      registration("adopted-terminal-b"),
    );
    assert.equal(activeAttempt.ok, false);
    assert.equal(activeAttempt.error?.code, "AGENT_REPLACED");
    activeReplacement.destroy();
    ...
    assert.equal(reloaded.agentId, initial.agentId);
    assert.equal(
      broker.store.state.agents[initial.agentId]?.terminalId,
      "adopted-terminal-b",
    );
    assert.equal(
      reloaded.connectionGeneration,
    ...
    assert.deepEqual(
      resultOf(
        await request(replacement, "agent.list", {
          ids: [childAgentId],
          limit: 10,
        }),
    ...
    assert.deepEqual(
      resultOf(
        await request(replacement, "task.list", {
          limit: 10,
        }),
      ).items.map((item: any) => item.id),
      [task.taskId],
    ...
    assert.equal(
      Object.values(broker.store.state.agents).filter(
        (agent) =>
          !agent.managed &&
          agent.paneId === "adopted-pane" &&
          agent.piSessionId === "pi-session-a",
      ).length,
      1,
    );
  } finally {
...
test("terminal replacement does not cross durable sessions and rejects ambiguous lineage", async () => {
  const root = await mkdtemp(join(tmpdir(), "domain-adopted-terminal-safe-"));
  const runtime = await mkdtemp(
    join(tmpdir(), "domain-adopted-terminal-safe-runtime-"),
  );
  const paths = {
    sessionKey: sessionKey(join(runtime, "broker.sock")),
    root,
  ...
  const closeSocket = async () => {
    ...
    socket.destroy();
    await bounded(closed, "adopted client close timeout");
    socket = undefined;
  };
  try {
    const secret = (await readFile(paths.secret, "utf8")).trim();
    ...
    const initial = resultOf(
      await request(
        socket,
        "agent.register_adopted",
        registration("terminal-a", "pi-session-a"),
      ),
    );
    const childAgentId = createId("agt");
    await broker.store.append({
    ...
    socket = await connect(paths, secret);
    const mismatchedClaim = resultOf(
      await request(
        socket,
        "agent.register_adopted",
        registration("terminal-mismatch", "pi-session-a", "pi-session-b"),
      ),
    );
    assert.notEqual(mismatchedClaim.agentId, initial.agentId);
    assert.deepEqual(
      resultOf(
        await request(socket, "agent.list", {
          ids: [childAgentId],
          limit: 10,
        }),
      ).items,
    ...
    const ambiguous = await request(
      socket,
      "agent.register_adopted",
      registration("terminal-c", "pi-session-a"),
    );
    assert.equal(ambiguous.ok, false);
    assert.equal(ambiguous.error?.code, "AGENT_REPLACED");
    assert.equal(
      Object.values(broker.store.state.agents).filter(
        (agent) =>
          !agent.managed &&
...
test("production A to B queue races do not resurrect or provision cancelled work", async () => {
  for (const order of ["cancel-first", "advance-first"] as const) {
    ...
    try {
      ...
      if (order === "cancel-first") {
        ...
        assert.equal(broker.store.state.tasks[taskB]?.state, "cancelling");
        assert.equal(broker.store.state.runs[runBId]?.state, "created");
        assert.equal(broker.store.state.runs[runBId]?.cancelled, true);
        assert.equal(bCancelRequested, 1);
  ...
  for (const order of ["cancel-first", "run-first"] as const) {
    ...
    try {
      ...
      if (order === "cancel-first") {
        ...
        assert.equal(
          broker.store.state.tasks[task.taskId]?.state,
          "cancelling",
        );
...
test("production strict managed child task.cancel-versus-timeout race", async () => {
  ...
  try {
    ...
    if (
      broker.store.state.questions?.[questionAck.questionId]?.state ===
      "cancelled"
    ) {
      assert.equal(broker.store.state.tasks[item.taskId]?.state, "cancelling");
      assert.equal(broker.store.state.runs[item.runId]?.state, "working");
      assert.equal(broker.store.state.runs[item.runId]?.cancelled, true);
    }
...
test("production strict delegation is completion-driven without a task wall deadline", async () => {
  const root = await mkdtemp(join(tmpdir(), "domain-completion-policy-"));
  const runtime = await mkdtemp(
    join(tmpdir(), "domain-completion-policy-runtime-"),
  );
  const paths = {
    sessionKey: sessionKey(join(runtime, "broker.sock")),
    root,
  ...
  const broker = new Broker(paths, {
    setTimeout: (callback, delay) => {
      ...
      return setTimeout(callback, 2_000_000);
    },
    herdrFactory: async (store) => new DomainHerdr(store) as any,
  });
  await broker.start();
  ...
  try {
    ...
    const item = delegated.tasks[0];
    const task = broker.store.state.tasks[item.taskId]!;
    const run = broker.store.state.runs[item.runId]!;
    assert.equal(task.completionPolicy, "until_terminal");
    assert.equal(task.timeoutAt, undefined);
    assert.equal(run.timeoutAt, undefined);
    assert.equal(scheduledDelays.includes(10_000), false);
  } finally {
    parent?.destroy();
    await broker.stop().catch(() => undefined);
    await rm(root, { recursive: true, force: true });
  ...
  for (const resourceCase of ["absent", "stale"] as const) {
    ...
    try {
      ...
      assert.deepEqual(
        ...
        { taskId: item.taskId, state: "cancelling" },
      );
...
test("production managed cancellation falls back to one exact Herdr stop", async () => {
  for (const failureMode of [
    ...
    try {
      ...
      const removeRunCancelled = broker.store.onAppend((event) => {
        if (
          event.type === "run.state_changed" &&
          event.entityRefs?.runId === item.runId &&
      ...
      assert.deepEqual(herdr.stopAgentIds, [item.agentId]);
...
test("authorized M4 evidence producers bind immutable results, independent identity, human correction, and restart recovery", async () => {
  ...
  try {
    ...
    if (originalExpiresAt === undefined) delete independentContract.expiresAt;
    else independentContract.expiresAt = originalExpiresAt;
