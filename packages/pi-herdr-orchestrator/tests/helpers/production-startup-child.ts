...
import { DEFAULT_PARENT_TOOL_NAMES } from "../../src/pi/parent-tool-schema.js";
...
async function extensionClient(): Promise<void> {
  ...
  assert.deepEqual(new Set(tools.keys()), new Set(DEFAULT_PARENT_TOOL_NAMES));
  assert.equal(tools.size, DEFAULT_PARENT_TOOL_NAMES.length);
  ...
  const orchestrate = tools.get("orchestrate");
  assert.ok(orchestrate);
  const spawnResult = await orchestrate.execute(
  ...
  const agentOption = (name: string): string => {
    const index = provision.agentArgs.indexOf(name);
    assert.notEqual(index, -1, `missing managed agent option ${name}`);
    const value = provision.agentArgs[index + 1];
    assert.ok(value, `missing managed agent value for ${name}`);
    return value;
  };
  const selectedProvider = agentOption("--provider");
  const selectedModel = agentOption("--model");
  const selectedThinking = agentOption("--thinking");
  assert.equal(selectedModel.startsWith(`${selectedProvider}/`), true);
  ...
  const registered = await managed.register({
    ...
    model: {
      ...
      id: selectedModel.slice(selectedProvider.length + 1),
      name: selectedModel.slice(selectedProvider.length + 1),
    },
