...
test("terminal result delivery ignores blocked, wakes for success, and deduplicates replay", async () => {
  ...
  assert.equal(
    ...
    'Managed task tsk_one (Focused check) reached succeeded for agent agt_one. Use orchestrate with action "collect" and taskIds ["tsk_one"] to record its structured result if available. If agent agt_one remains open and is no longer needed, use orchestrate with action "close" and taskId tsk_one. Continue the remaining work without waiting for a user prompt.',
  );
  assert.deepEqual(
...
test("terminal result delivery binds the parent and supports failure without a result", async () => {
  ...
  assert.equal(
    fake.messages[0]?.message.content,
    'Managed task tsk_failed (Failed check) reached failed. Use orchestrate with action "collect" and taskIds ["tsk_failed"] to record its structured result if available. Continue the remaining work without waiting for a user prompt.',
  );
  assert.doesNotMatch(
    fake.messages[0]?.message.content ?? "",
    /action "close"/,
