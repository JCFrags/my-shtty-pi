...
test("closed managed resources project as historical tombstones", () => {
  const candidate = agent({ lifecycleClass: "temporary", state: "stopped" });
  const current = state(candidate);
  current.herdrResources = {
    [candidate.id]: {
  ...
  assert.deepEqual(projectAgentLifecycle(candidate, current), {
    lifecycleClass: "temporary",
    keepForReuse: false,
    closeRecommendation: "keep",
    closeReason: "The managed resource is closed; this row is historical.",
  });
  assert.equal(
    agentWithLifecycle(candidate, current).managedResourceState,
    "closed",
  );
});
