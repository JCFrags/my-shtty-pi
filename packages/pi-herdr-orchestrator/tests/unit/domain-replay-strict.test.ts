...
test("task lifecycle follows accepted, working, collecting, and quiescent cancellation", async () => {
  const { root, store } = await fresh();
  try {
    ...
    assert.equal(store.state.tasks[taskId]?.state, "assigned");
    await store.append({
      type: "run.pi_started",
      actor,
      entityRefs: { taskId, runId },
    ...
    assert.equal(store.state.tasks[taskId]?.state, "running");
    await store.append({
    ...
    assert.equal(store.state.tasks[taskId]?.state, "cancelling");
    assert.equal(store.state.runs[runId]?.state, "working");
    assert.equal(store.state.runs[runId]?.cancelled, true);
    await store.append({
      type: "run.pi_settled",
      actor,
      entityRefs: { taskId, runId },
    ...
    await store.append({
      ...
      actor,
      entityRefs: { taskId, runId },
      payload: { runId, state: "cancelled" },
    });
    assert.equal(store.state.tasks[taskId]?.state, "cancelled");

    const normal = await seed(store);
    await store.append({
      type: "assignment.accepted",
      actor,
      entityRefs: { taskId: normal.taskId, runId: normal.runId },
      payload: { runId: normal.runId },
    });
    await store.append({
      type: "run.pi_started",
      actor,
      entityRefs: { taskId: normal.taskId, runId: normal.runId },
      payload: { runId: normal.runId, state: "working" },
    });
    await store.append({
      type: "run.pi_settled",
      actor,
      entityRefs: { taskId: normal.taskId, runId: normal.runId },
      payload: { runId: normal.runId, state: "settled" },
    });
    assert.equal(store.state.tasks[normal.taskId]?.state, "collecting");

    const replay = new EventStore(join(root, "events.jsonl"), actor);
    await replay.open();
    assert.equal(replay.readOnly, false);
    assert.equal(replay.state.tasks[taskId]?.state, "cancelled");
    assert.equal(replay.state.runs[runId]?.state, "cancelled");
    assert.equal(replay.state.tasks[normal.taskId]?.state, "collecting");
  } finally {
    await rm(root, { recursive: true, force: true });
  }
});

test("legacy cancellation events remain terminal on replay", async () => {
  const { root, store } = await fresh();
  try {
    const { taskId, runId } = await seed(store);
    await store.append({
      ...
      actor,
      entityRefs: { taskId },
      payload: { taskId, reason: "legacy cancellation", cascade: false },
    });
    assert.equal(store.state.tasks[taskId]?.state, "cancelled");
    assert.equal(store.state.runs[runId]?.state, "cancelled");
    const replay = new EventStore(join(root, "events.jsonl"), actor);
    await replay.open();
    assert.equal(replay.state.tasks[taskId]?.state, "cancelled");
    assert.equal(replay.state.runs[runId]?.state, "cancelled");
...
test("explicit completion policy persists without deriving a deadline", async () => {
  const { root, store } = await fresh();
  try {
    const taskId = createId("tsk"),
      runId = createId("run"),
    ...
    assert.equal(store.state.tasks[taskId]?.completionPolicy, "until_terminal");
    assert.equal(store.state.tasks[taskId]?.timeoutAt, undefined);
    assert.equal(store.state.runs[runId]?.timeoutAt, undefined);

    const replay = new EventStore(join(root, "events.jsonl"), actor);
    await replay.open();
    assert.equal(replay.readOnly, false);
    assert.equal(
      replay.state.tasks[taskId]?.completionPolicy,
      "until_terminal",
    );
    assert.equal(replay.state.tasks[taskId]?.timeoutAt, undefined);
    assert.equal(replay.state.runs[runId]?.timeoutAt, undefined);

    const invalidTaskId = createId("tsk");
    await assert.rejects(
      store.append({
        type: "task.created_m3",
        actor,
        entityRefs: { taskId: invalidTaskId },
        payload: {
          ...taskPayload(invalidTaskId),
          completionPolicy: "until_terminal",
          timeoutAt: timeout,
        },
      }),
