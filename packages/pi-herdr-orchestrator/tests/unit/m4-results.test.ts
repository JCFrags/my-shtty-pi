...
test("M4 result and question validators preserve bounded application payload text", () => {
  validateResult(body);
  validateQuestion(question);
  validateResult({
    ...body,
    summary:
      "Use api_key=example-value and Authorization: Bearer example-value.",
  });
  validateQuestion({
    ...question,
    prompt: "Which private_key reference should the worker use?",
    context: "The parent supplied token=example-value for this task.",
  });
