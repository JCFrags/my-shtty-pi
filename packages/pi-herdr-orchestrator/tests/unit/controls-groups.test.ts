...
import { registerAdvancedParentTools } from "../../src/pi/tools.js";
...
test("requirement 11: peer ask preserves bounded application payload text", async () => {
  const calls: Array<{ method: string; params: Record<string, unknown> }> = [];
  const answer = "Use Authorization: Bearer example-value.";
  ...
  const response = await service.execute(
    {
      ...
      input: {
        agentId: "agt_child",
        message: "Check api_key=example-value.",
  ...
  assert.equal((response.result as { answer: string }).answer, answer);
