...
test("inspect, list, collect, and cancel map to bounded broker operations", async () => {
  ...
  assert.deepEqual(calls, [
    {
      method: "task.get",
      params: { taskId, include: ["runs"], maxBytes: 8_192 },
    },
    {
      method: "agent.list",
      params: {
    ...
    {
      method: "task.cancel",
      params: { taskId, reason: "No longer required.", cascade: false },
    },
  ]);
});
...
test("repeated multi-parent facade use does not lose or cross close identities", async () => {
  ...
  await Promise.all(
    [...tasks.entries()].map(async ([taskId, value]) => {
      ...
      await tool.execute(
        `wait-${taskId}`,
        { action: "wait", taskId, timeoutMs: 1_000 },
...
test("wait needs only taskId and close derives exact agent identity", async () => {
  ...
  const { tool } = fixture(async (method, params) => {
    ...
    if (method === "coordination.wait")
      return {
        ...
        state: "succeeded",
        ready: true,
        value: { id: taskId, advisoryModelReceipt: { privateScore: 123 } },
      };
    if (method === "task.get")
  ...
  await tool.execute("call-close", {
    action: "close",
    taskId,
    reason: "Temporary scout is complete.",
    confirm: true,
  });
  assert.deepEqual(calls, [
    {
