...
const agentSchemaUrl = new URL(
  "../../../schemas/m3-agent.schema.json",
  import.meta.url,
);
const assignmentSchemaUrl = new URL(
  "../../../schemas/m3-assignment.schema.json",
  import.meta.url,
);
...
test("M3 schemas are namespaced additive draft schemas", async () => {
  const [agent, assignment] = await Promise.all([
    readSchema(agentSchemaUrl),
    readSchema(assignmentSchemaUrl),
  ]);
  assert.equal(
    agent["$schema"],
...
test("M3 assignment schema covers exact task/run/agent correlation", async () => {
  ...
  >;
  assert.ok(properties["deadline"]);
  assert.equal(required.includes("deadline"), false);
...
test("M3 schema fixtures reject stale identity and generation values", async () => {
  ...
  const agentIdPattern = new RegExp(
    String(
      (agent["properties"] as Record<string, Record<string, string>>)[
        "agentId"
      ]!["pattern"],
    ),
  );
  const assignmentIdPattern = new RegExp(
    String(
      (assignment["properties"] as Record<string, Record<string, string>>)[
        "assignmentId"
      ]!["pattern"],
    ),
  );
  ...
  assert.equal(
    assignmentIdPattern.test("asg_01J00000000000000000000000"),
    true,
  );
  assert.equal(assignmentIdPattern.test("asg_0"), false);
  assert.equal(
    (
      assignment["properties"] as Record<
        string,
