...
test("M3 assignment rendering excludes secrets and transcript text", () => {
  ...
  assert.match(message, /Deadline: 2030-01-01T00:00:00.000Z/);
  assert.doesNotMatch(message, /token|session-1/);
});
test("M3 completion-driven assignment renders no synthetic deadline", () => {
  const { deadline: _deadline, ...completionDriven } = assignment;
  const message = renderAssignment(completionDriven);
  assert.match(message, /continue until the task reaches a terminal result/i);
  assert.doesNotMatch(message, /Deadline:/);
});
