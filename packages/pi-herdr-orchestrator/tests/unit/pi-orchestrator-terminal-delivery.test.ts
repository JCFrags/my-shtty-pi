...
test("parent extension wakes on terminal result and replays a disconnected suffix", async (t) => {
  ...
  assert.equal(
    ...
    'Managed task tsk_first (First result) reached succeeded for agent agt_first. Use orchestrate with action "collect" and taskIds ["tsk_first"] to record its structured result if available. If agent agt_first remains open and is no longer needed, use orchestrate with action "close" and taskId tsk_first. Continue the remaining work without waiting for a user prompt.',
  );
