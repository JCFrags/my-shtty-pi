...
test("ChronoCompact calls receive only safe sections and honor a larger bound", async () => {
  const entries: Record<string, unknown>[] = [
      ...
      entry(assistantId, parent, {
        ...
        message: {
          ...
          content: [
            ...
            {
              type: "text",
              text: `Verified public step ${index}. ${"Safe public detail. ".repeat(20)}`,
            },
            {
              type: "toolCall",
              id: callId,
...
test("the bundled deterministic ChronoCompact runtime loads without Pi peers", async () => {
  const entries: Record<string, unknown>[] = [
    entry("bundled-marker", null, {
      type: "custom",
      customType: "pi-herdr-orchestrator-assignment",
      data: { taskId, runId, assignmentId },
    }),
  ];
  let parent = "bundled-marker";
  for (let index = 0; index < 40; index++) {
    const id = `bundled-assistant-${index}`;
    entries.push(
      entry(id, parent, {
        type: "message",
        message: {
          ...
          content: [
            {
              type: "text",
              text: `Completed phase ${index}. Verified the bounded inspection behavior and focused tests passed. ${"Safe implementation detail. ".repeat(8)}`,
            },
          ],
        },
      }),
  ...
  assert.equal(snapshot.reducer, "chrono-compact");
  assert.equal(snapshot.items.length, 40);
  assert.ok(snapshot.replay);
  assert.equal(
    snapshot.replay.rawTokens > snapshot.replay.renderedTokens,
    true,
  );
  assert.equal(validateRecentWorkSnapshot(snapshot, { taskId, runId }), true);
});
