...
test("orchestrator extension reload transfers runtime credential and registers tools once", async (t) => {
  ...
  const previous = {
    ...
    stateRoot: process.env.PI_HERDR_ORCH_STATE_ROOT,
  };
  const envKeys = {
    herdr: "HERDR_ENV",
    ...
    stateRoot: "PI_HERDR_ORCH_STATE_ROOT",
  } as const;
        ...
        if (frame.type === "hello")
          ...
          const count =
            frame.method === "agent.register_managed"
              ? ++registrationCount
              : registrationCount;
          if (frame.method === "tool_audit.ingest")
            auditRequests.push(
              structuredClone(frame.params as Record<string, unknown>),
            );
          if (frame.method === "agent.heartbeat") {
            heartbeatCount++;
            if (rejectHeartbeats) {
              writeResponse(socket, {
                v: 1,
                ...
                error: {
                  code: "RUN_MISMATCH",
                  message: "Heartbeat sequence is stale.",
                  retryable: false,
                },
              });
              continue;
  ...
  await emit(first, "tool_execution_start", firstHarness.context, {
    toolCallId: "call_malformed",
    toolName: "orchestrator_result",
    args: { taskId: "bad", exactInput: "operator-visible" },
  });
  await emit(first, "tool_execution_end", firstHarness.context, {
    toolCallId: "call_malformed",
    toolName: "orchestrator_result",
    isError: true,
    result: {
      content: [
        {
          type: "text",
          text: "INVALID_REQUEST: malformed result arguments",
        },
      ],
    },
  });
  for (let attempt = 0; attempt < 100 && auditRequests.length < 2; attempt++)
    await new Promise((resolve) => setTimeout(resolve, 10));
  assert.equal(auditRequests.length, 2);
  assert.deepEqual(auditRequests[0]?.input, {
    taskId: "bad",
    exactInput: "operator-visible",
  });
  assert.equal(auditRequests[1]?.status, "failed");
  assert.equal(auditRequests[1]?.errorCode, "INVALID_REQUEST");
  assert.equal(Object.hasOwn(auditRequests[1] ?? {}, "result"), false);
  const heartbeatBaseline = heartbeatCount;
  rejectHeartbeats = true;
  for (
    let attempt = 0;
    attempt < 100 && heartbeatCount < heartbeatBaseline + 2;
    attempt++
  )
    await new Promise((resolve) => setTimeout(resolve, 10));
  assert.ok(
    heartbeatCount >= heartbeatBaseline + 2,
    "the connected adapter continues heartbeating after a semantic rejection",
  );
  assert.equal(
    registrationCount,
    1,
    "a semantic heartbeat rejection must not re-register a healthy socket",
  );
  assert.equal(
    connections.filter((socket) => !socket.destroyed).length,
    1,
    "a semantic heartbeat rejection must not replace the connected socket",
  );
