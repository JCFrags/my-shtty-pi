...
import { resolveEndpoint, } from "../broker/endpoint-policy.js";
import { resolveSpawnPolicy, validateModelSelection, } from "../broker/model-policy.js";
import { projectModelEvidence, } from "./model-evidence.js";
...
function fiveStarDisplay(valuePpm) {
...
export function availableModelOptionsView(options, endpointPolicy, limit = MODEL_RANKING_POLICY.maxReturnedCandidates) {
    ...
    const presentLevels = new Set(returned.flatMap((model) => model.thinkingLevels.map((thinking) => thinking.thinkingLevel)));
    return {
...
function exactMapping(modelIntelligence, selection) {
    return modelIntelligence?.mappings.find((mapping) => mapping.provider === selection.provider &&
        mapping.modelId === selection.modelId);
}
export function modelCapacityView(endpointId, limit, scheduler) {
    const active = scheduler.active.filter((task) => task.endpointId === endpointId).length;
    const queued = scheduler.queued.filter((task) => task.endpointId === endpointId).length;
    return {
        endpointId,
        limit,
        active,
...
function sourceDates(state, taskProfile) {
    if (!state)
        return [];
    const values = Object.values(state.records)
        .map((stored) => stored.record)
        .filter((record) => record.taskProfile === taskProfile &&
...
export function modelRankingProfile(modelIntelligence, taskProfile) {
    return (modelIntelligence?.profiles?.[taskProfile] ?? {
        weightsPpm: {
            taskCapability: MODEL_RANKING_POLICY.weightsPpm.task_capability,
            protocolReliability: MODEL_RANKING_POLICY.weightsPpm.protocol_reliability,
...
function scoreProjection(projection, endpoint, rankingProfile) {
    const values = {
    ...
    const utilityPpm = divideHalfUp(BigInt(values.taskCapability) * BigInt(weights.taskCapability) +
        ...
        BigInt(values.preference) * BigInt(weights.humanPreference), BigInt(PPM));
    const uncertaintyPenaltyPpm = divideHalfUp(BigInt(rankingProfile.uncertaintyPenaltyPpm) *
        BigInt(PPM - projection.overallConfidencePpm), BigInt(PPM));
    return {
...
export function buildModelOptions(input) {
    ...
    const rankingProfile = modelRankingProfile(input.modelIntelligence, input.taskProfile);
    if (!Number.isSafeInteger(limit) ||
        limit < 1 ||
        limit > MODEL_RANKING_POLICY.maxEligibleCandidates)
        throw new Error("MODEL_OPTIONS_LIMIT_INVALID");
    const routingMode = input.modelIntelligence?.routingMode ?? "current_default";
    const policySelection = input.selectedModel ??
    ...
    const installed = [
        ...new Map(input.capabilities.models
            .flatMap((model) => model.thinkingLevels.map((thinkingLevel) => validateModelSelection({
            provider: model.provider,
            modelId: model.modelId,
            thinkingLevel,
        })))
            .map((selection) => [selectionKey(selection), selection])).values(),
    ];
    const eligible = [];
    const excluded = [];
    ...
    eligible.sort((left, right) => selectionKey(left).localeCompare(selectionKey(right)));
    const candidates = eligible.map((selection) => {
        const endpoint = resolveEndpoint(selection, input.endpointPolicy, input.fallbackEndpointLimit);
    ...
    const projection = projectModelEvidence(input.evidence, candidates, input.taskProfile, input.asOf);
    const identityOrder = (left, right) => candidateKey({
        ...left.selection,
    ...
    const scored = projection.candidates
        .map((candidate) => {
        const endpoint = resolveEndpoint(candidate.candidate, input.endpointPolicy, input.fallbackEndpointLimit);
        return scoreProjection(candidate, modelCapacityView(endpoint.endpointId, endpoint.maxConcurrentAgents, input.scheduler), rankingProfile);
    })
        .sort((left, right) => right.scorePpm - left.scorePpm || identityOrder(left, right));
    const ordered = [];
    const tieStarts = new Set();
...
export function ratedAutomaticCandidate(options) {
    if (options.mode !== "rated_auto")
        return undefined;
...
export function createAdvisoryModelReceipt(input) {
    ...
    const selected = input.options.candidates.find((candidate) => selectionKey(candidate.selection) === selectionKey(selectedModel));
    ...
    if (!selectedEndpoint)
        throw new Error("MODEL_SELECTED_ENDPOINT_REQUIRED");
    const withoutDigest = {
        schemaVersion: 1,
        scorerVersion: 1,
        ...
        recommendedMatchesSelection: selectionKey(recommended.selection) === selectionKey(selectedModel),
        ...
        selectedEndpoint,
        alternatives: input.options.candidates.slice(0, MODEL_RANKING_POLICY.maxReceiptAlternatives),
...
function validSelection(value) {
    ...
    try {
        return (exactKeys(selection, ["provider", "modelId", "thinkingLevel"]) &&
...
function validPpm(value, allowNegative = false) {
...
function validRankedOption(value) {
    if (!value || typeof value !== "object" || Array.isArray(value))
        return false;
    const option = value;
...
export function validateAdvisoryModelReceipt(value) {
    if (!value || typeof value !== "object" || Array.isArray(value))
    ...
    const selectedRank = receipt.selectedRank;
    const selectedAlternative = selectedRank !== null && selectedRank <= alternatives.length
        ? alternatives[selectedRank - 1]
        : undefined;
    if (alternatives.some((option, index) => option.rank !== index + 1 ||
