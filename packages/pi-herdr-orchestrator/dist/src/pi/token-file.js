...
export function siblingSecretPath(socketPath) {
    if (!socketPath ||
...
export async function readBrokerSecretFile(path) {
...
export async function readManagedTokenFile(path) {
    if (!path ||
    ...
    if (Buffer.byteLength(value, "utf8") > MAX_TOKEN_FILE_BYTES ||
        !/^[A-Za-z0-9_-]{32,2048}\n?$/u.test(value) ||
