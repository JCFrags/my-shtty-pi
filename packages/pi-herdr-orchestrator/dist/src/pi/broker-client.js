...
function validateHelloResult(value, expectedId) {
    ...
    if (!exactKeys(broker, ["version", "status", "lastEventSeq"]) ||
    ...
    if (!Number.isSafeInteger(broker.lastEventSeq) ||
        broker.lastEventSeq < 0)
        throw new Error("BROKER_HELLO_INVALID");
    ...
    if (!Array.isArray(principal.permissions) ||
        ...
        principal.permissions.some((permission) => typeof permission !== "string" ||
            permission.length === 0 ||
            Buffer.byteLength(permission, "utf8") > 256))
        throw new Error("BROKER_HELLO_INVALID");
    const limits = record(frame.limits);
    if (!exactKeys(limits, ["maxLineBytes"]) ||
        !Number.isSafeInteger(limits.maxLineBytes) ||
        limits.maxLineBytes < 1 ||
        limits.maxLineBytes > 16_777_216)
        throw new Error("BROKER_HELLO_INVALID");
    const agentId = principal.agentId === undefined ? undefined : safeString(principal.agentId);
    const piSessionId = principal.piSessionId === undefined
        ? undefined
    ...
    return {
        ...
        id: expectedId,
        ok: true,
        broker: { version, status, lastEventSeq: broker.lastEventSeq },
        principal: {
        ...
        limits: { maxLineBytes: limits.maxLineBytes },
    };
}
...
function safeErrorDetails(value, depth = 0) {
    ...
    if (typeof value === "object") {
        ...
        return (entries.length <= 64 &&
            entries.every(([key, item]) => Buffer.byteLength(key, "utf8") <= 256 &&
                !/(?:token|secret|password|cookie|credential|private.?key|api.?key|socket.?path)/iu.test(key) &&
...
function validResponse(value) {
    ...
    if (!error || typeof error !== "object" || Array.isArray(error))
        return false;
    const e = error;
    return (exactKeys(e, ["code", "message", "retryable", "details", "remediation"]) &&
...
export class PiBrokerClient {
    #socket;
    #options;
    #questionBindings = new Map();
    #questionWaiters = new Map();
    #registrationReady = false;
    #queuedServerRequests = [];
    #pending = new Map();
    #connected = false;
    #adapterSeq = 0;
    #helloId;
    ...
    bindIdentity(agentId, generation) {
        if (!/^[\x21-\x7e]{1,256}$/u.test(agentId) ||
    ...
    async connect() {
        ...
        const decoder = new NdjsonDecoder((value) => value);
        socket.on("data", (data) => {
            if (this.#socket !== socket)
                return;
                ...
                if (frame.type === "event") {
                    try {
                        if (!exactKeys(frame, [
                            ...
                            Object.values(frame.refs).some((value) => typeof value !== "string" ||
                        ...
                        this.failClosed(error instanceof Error
                            ? error
        ...
        const hello = await this.raw({
            ...
            id: helloId,
            client: {
                kind: this.#options.token && this.#options.agentId !== undefined
                    ? "pi_child"
                    : "pi_parent",
    ...
    markRegistrationReady() {
        ...
        for (const frame of queued) {
            if (typeof frame.id === "string")
                this.#serverRequestIds.delete(frame.id);
            void this.handleServerRequest(frame);
        }
    }
    async register(state, sessionReference = {
        source: "herdr:pi",
        agent: "pi",
        kind: "id",
        ...
        const result = await this.request(this.#options.token ? "agent.register_managed" : "agent.register_adopted", {
        ...
        if (typeof value.agentId !== "string" ||
            ...
            heartbeatMsValue < 100 ||
            heartbeatMsValue > 60_000 ||
        ...
        const connectionGeneration = connectionGenerationValue;
        const heartbeatMs = heartbeatMsValue;
    ...
    registerQuestionWaiter(toolCallId, runId, timeoutMs, signal) {
        if (!/^[\x21-\x7e]{1,256}$/u.test(toolCallId) ||
        ...
        if (this.#questionWaiters.size >= 16 ||
            this.#questionBindings.has(toolCallId) ||
            [...this.#questionWaiters.values()].some((waiter) => waiter.toolCallId === toolCallId || waiter.runId === runId))
    ...
    discardQuestionWaiter(toolCallId) {
        const entry = [...this.#questionWaiters.entries()].find(([, waiter]) => waiter.toolCallId === toolCallId);
        this.#questionBindings.delete(toolCallId);
        if (!entry)
            return;
    ...
    cancelQuestionWaiter(toolCallId, error = new Error("CANCELLED")) {
        const entry = [...this.#questionWaiters.entries()].find(([, waiter]) => waiter.toolCallId === toolCallId);
        this.#questionBindings.delete(toolCallId);
        if (!entry)
            return;
    ...
    questionIdForToolCall(toolCallId) {
        return this.#questionBindings.get(toolCallId);
    }
    bindQuestionWaiter(toolCallId, questionId) {
        ...
        const waiter = [...this.#questionWaiters.entries()].find(([, item]) => item.toolCallId === toolCallId)?.[1];
        if (!waiter)
            throw new Error("QUESTION_WAITER_MISSING");
        waiter.questionId = questionId;
        this.#questionBindings.set(toolCallId, questionId);
    }
    resolveQuestionDelivery(questionId, runId, toolCallId, value) {
        const entry = [...this.#questionWaiters.entries()].find(([, waiter]) => waiter.toolCallId === toolCallId &&
            waiter.runId === runId &&
    ...
    failClosed(error = new Error("BROKER_PROTOCOL_INVALID")) {
    ...
    async handleServerRequest(frame) {
        ...
        let id;
        try {
            if (!exactKeys(frame, ["v", "type", "id", "method", "params"]) ||
                frame.v !== 1 ||
        ...
        try {
            ...
            if (this.#socket === socket)
                socket.write(encodeFrame({ v: 1, type: "server_response", id, ok: true, result }));
        }
        catch (error) {
            const failure = error;
    ...
    async raw(frame, timeoutMs) {
        ...
        return await new Promise((resolve, reject) => {
            ...
            timer.unref?.();
            this.#pending.set(id, {
                method: String(frame.method ?? "hello"),
                resolve: (value) => {
                    clearTimeout(timer);
                    resolve(value);
                },
            ...
            try {
                ...
                this.#pending.delete(id);
                clearTimeout(timer);
                reject(error instanceof Error ? error : new Error("BROKER_REQUEST_FAILED"));
            }
        });
    }
}
