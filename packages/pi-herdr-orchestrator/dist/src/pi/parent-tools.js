export const DEFAULT_PARENT_TOOL_LIMITS = Object.freeze({
...
function truncateUtf8(value, maxBytes) {
    if (Buffer.byteLength(value, "utf8") <= maxBytes)
        return value;
    const marker = "…";
    let end = Math.min(value.length, Math.max(0, maxBytes - Buffer.byteLength(marker, "utf8")));
    while (end > 0 &&
...
function safeProjection(value, limits) {
...
export class ParentToolService {
    #broker;
    #limits;
    constructor(broker, limitsOrLegacy = {}, legacyLimits = {}) {
        this.#broker = broker;
        const limits = limitsOrLegacy instanceof Map ? legacyLimits : limitsOrLegacy;
        this.#limits = { ...DEFAULT_PARENT_TOOL_LIMITS, ...limits };
    }
    async execute(request, principal, signal) {
        if (signal?.aborted)
            return {
                ok: false,
                error: { code: "CANCELLED", message: "The request was cancelled." },
            };
        if ((request.tool === "delegate" || request.tool === "delegate_compact") &&
        ...
        try {
            const result = safeProjection(await this.#broker.invoke(methodForTool(request.tool), mapParentToolInput(request.tool, request.input), principal, request.idempotencyKey), this.#limits);
            ...
            const candidate = result && typeof result === "object" && !Array.isArray(result)
                ? result.retrieval
                : undefined;
            const retrieval = candidate && typeof candidate === "object" && !Array.isArray(candidate)
                ? candidate
                : undefined;
            const safeRetrieval = retrieval &&
                ...
                ? {
                    ...
                    id: retrieval.id,
                    nextCursor: typeof retrieval.nextCursor === "string" ||
