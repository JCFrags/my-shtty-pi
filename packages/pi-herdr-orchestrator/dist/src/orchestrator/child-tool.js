...
const id = (v, k) => typeof v[k] === "string" ? v[k] : undefined;
function env(name) {
    const v = process.env[name];
    if (!v)
        throw new ChildError("CHILD_CONTEXT_INCOMPLETE");
    return v;
}
function publicError(e) {
    if (e instanceof ChildError)
        return e;
...
async function context(p) {
    ...
    const domainId = env("PI_HERDR_DOMAIN_ID"), agentId = env("PI_HERDR_AGENT_ID");
    if (env("PI_HERDR_AGENT_GENERATION") !== "1")
        throw new ChildError("GENERATION_MISMATCH");
    const runId = value(p.runId, 128), generation = p.assignmentGeneration;
    if (!Number.isSafeInteger(generation) || Number(generation) < 1)
        throw new ChildError("INVALID_REQUEST");
    const registry = await readRegistryByDomain(domainId), agent = registry.agents.find((a) => a.agentId === agentId);
    ...
    const run = agent.runs.find((r) => r.runId === runId && r.assignmentGeneration === generation);
    if (!run)
        throw new ChildError("STALE_ASSIGNMENT");
    if (agent.workspaceId !== env("HERDR_WORKSPACE_ID") ||
        agent.tabId !== env("HERDR_TAB_ID") ||
        agent.paneId !== env("HERDR_PANE_ID"))
        throw new ChildError("CHILD_IDENTITY_MISMATCH");
    const cli = new HerdrCli(), [current, herdrAgent, pane] = await Promise.all([
        cli.paneCurrent(),
        cli.agentGet(agent.herdrAgentName),
        cli.paneGet(agent.paneId),
    ]);
    for (const candidate of [current, herdrAgent, pane])
        if (id(candidate, "workspace_id") !== agent.workspaceId ||
...
async function validateTarget(cli, target) {
    const [a, p] = await Promise.all([
        cli.agentGet(target.herdrAgentName),
        cli.paneGet(target.paneId),
    ]);
    for (const v of [a, p])
        if (id(v, "workspace_id") !== target.workspaceId ||
...
async function append(c, kind, target, summary) {
...
async function execute(raw) {
    const p = object(raw) ?? {}, action = p.action;
    if (action !== "progress" &&
        action !== "send" &&
    ...
    if (action === "progress") {
        ...
        const e = await append(c, "progress", "parent", value(p.summary, MAX_SUMMARY));
        return {
            ok: true,
            action,
            runId: e.runId,
    ...
    if (action === "send") {
        ...
        if (target !== "parent") {
            const registry = await readRegistryByDomain(c.agent.domainId), recipient = registry.agents.find((a) => a.agentId === target);
            if (!recipient ||
            ...
            await c.cli.agentPrompt(recipient.herdrAgentName, `[subagent message from ${c.agent.agentId}; run ${c.run.runId}; assignment ${c.run.assignmentGeneration}] ${message}`);
        }
    ...
    const completed = await c.store.complete({
        ...
        status,
        summary: value(p.summary, 4096),
        finalResult: p.finalResult === undefined ? null : value(p.finalResult, MAX_RESULT),
    });
    return {
        ok: true,
        action,
...
export function registerSubagentChannel(api) {
    const tool = {
        name: "subagent_channel",
        label: "Subagent Channel",
        parameters: SCHEMA,
        description: "Report progress, send a message, acknowledge a requested cancellation, or explicitly complete the exact assigned run. Every call requires the current run ID and assignment generation; stale assignments are rejected.",
        promptSnippet: "Use subagent_channel with the exact runId and assignmentGeneration from the latest assignment prompt; acknowledge cancellation only after the parent requests it.",
