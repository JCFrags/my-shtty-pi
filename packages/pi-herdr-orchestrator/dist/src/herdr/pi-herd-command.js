...
function registryPath(env) {
    return (env.HERDR_PI_HERD_REGISTRY ??
...
function invoke(binary, argv, environment) {
...
export async function openPiHerd(env = process.env) {
    const binary = env.HERDR_BIN_PATH;
    const targetPaneId = env.HERDR_PANE_ID?.trim();
    if (!binary || !binary.startsWith("/"))
        throw new Error("Agent Board is available only inside Herdr (missing absolute HERDR_BIN_PATH).");
    if (!targetPaneId)
        throw new Error("Agent Board is available only inside Herdr (missing HERDR_PANE_ID).");
    try {
        await access(binary, constants.X_OK);
    }
    catch {
        throw new Error("Agent Board cannot open: HERDR_BIN_PATH is not executable.");
    }
    const path = registryPath(env);
    const registry = await loadRegistry(path);
    const entry = registry[targetPaneId];
    if (entry?.paneId) {
        const focused = await invoke(binary, ["plugin", "pane", "focus", entry.paneId].join("\u0000"), env);
        if (focused.code === 0) {
    ...
    const open = await invoke(binary, [
