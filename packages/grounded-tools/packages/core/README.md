...
The `ask-user-v1` export freezes the store-free facade/provider message types, four same-process channel names, strict runtime guards, correlation rules, and normalized results. Dialog and a later Signals provider can share this contract without importing each other.
