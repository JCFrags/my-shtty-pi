...
const UMBRELLA_ENTRIES = ["package/LICENSE", "package/README.md", "package/package.json", "package/docs/artifacts.md", "package/docs/ask-user-v1.md", "package/docs/benchmark.md", "package/docs/local-search.md", "package/docs/security.md", "package/packages/core/LICENSE", "package/packages/core/README.md", "package/packages/core/package.json", "package/packages/core/src/anchors.ts", "package/packages/core/src/ask-user-v1.ts", "package/packages/core/src/atomic.ts", "package/packages/core/src/exec.ts", "package/packages/core/src/local-session.ts", "package/packages/core/src/lsp-client.ts", "package/packages/core/src/notes.ts", "package/packages/core/src/output.ts", "package/packages/core/src/paths.ts", "package/packages/core/src/process-manager.ts", "package/packages/core/src/pty_bridge.py", "package/packages/core/src/search.ts", "package/packages/core/src/session_pty_bridge.py", "package/packages/core/src/session-contract.ts", "package/packages/core/src/session-framing.ts", "package/packages/core/src/session-logs.ts", "package/packages/core/src/session-registry.ts", "package/packages/core/src/state.ts", "package/packages/core/src/syntax.ts", "package/packages/core/src/tasks.ts", "package/packages/core/src/text.ts", "package/packages/core/src/workplan.ts", ...["dialog", "files", "lsp", "notes", "process", "tasks", "workplan"].flatMap(n => [`package/packages/${n}/LICENSE`, `package/packages/${n}/README.md`, `package/packages/${n}/index.ts`, `package/packages/${n}/package.json`]), "package/packages/dialog/ask-user-facade.ts", "package/packages/dialog/blocking-provider.ts", "package/packages/tasks/settings.ts", ...["LICENSE", "README.md", "package.json", "src/anchors.ts", "src/ask-user-v1.ts", "src/atomic.ts", "src/exec.ts", "src/local-session.ts", "src/lsp-client.ts", "src/notes.ts", "src/output.ts", "src/paths.ts", "src/process-manager.ts", "src/pty_bridge.py", "src/search.ts", "src/session-contract.ts", "src/session_pty_bridge.py", "src/session-framing.ts", "src/session-logs.ts", "src/session-registry.ts", "src/state.ts", "src/syntax.ts", "src/tasks.ts", "src/text.ts", "src/workplan.ts"].map(p => `package/node_modules/@grounded/pi-core/${p}`)].sort();
...
test("feature source packs keep bundled core and load independently", async () => {
    for (const name of ["notes", "workplan"] as const) {
        ...
        try {
            ...
            const bundledCore = safeChild(root, "stage/node_modules/@grounded/pi-core");
            await cp(join(repo, "packages", "core"), bundledCore, { recursive: true });
...
test("source packs use strict reviewed payloads and clean controlled loading", async () => {
    ...
    try {
        ...
        const extraction = await extract(root, held.entries, newEvidence());
        await cleanLoad(root, extraction, "pi-grounded-tools", manifest.pi.extensions, ["read", "edit", "write", "local_search", "bash", "process", "session", "lsp", "ask_user_question", "todo", "notes", "workplan"]);
    }
    finally {
        await rm(root, { recursive: true, force: true });
    }
...
test("exact release verification binds digest, identity, custody, and manifest", async () => {
    ...
    try {
        ...
        await verifyReleaseArtifact({ artifactPath, sha256: sha256(await readFile(artifactPath)), provenance: { source: "public source-pack fixture", version: "0.1.0", commit: "fixture-commit" }, temporaryRoot: root, packageName: "pi-grounded-tools", entries: ["./packages/files/index.ts", "./packages/process/index.ts", "./packages/lsp/index.ts", "./packages/dialog/index.ts", "./packages/tasks/index.ts", "./packages/notes/index.ts", "./packages/workplan/index.ts"], expectedTools: ["read", "edit", "write", "local_search", "bash", "process", "session", "lsp", "ask_user_question", "todo", "notes", "workplan"] });
    }
    finally {
        await rm(root, { recursive: true, force: true });
    }
