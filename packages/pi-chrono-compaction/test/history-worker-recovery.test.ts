// Persisted compatibility coverage through the explicit synthetic-only transport.
// These tests verify semantics; production OS isolation requires shared-runtime tests.
...
test("extension responses page, repeat deterministically, and recover exact oversized matches", async () => {
  ...
  try {
    ...
    extension(pi as unknown as ExtensionAPI, { historyTransport: { isolation: "os-bounded-child-v1", run: async (wire) => handleHistoryWorkerRequest(wire) } });
    const context = {
      hasUI: false,
      model: undefined,
