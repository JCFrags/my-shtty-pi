...
test("small worker compare and queue modes report exactness and host limits",async()=>{const compare=await runWorkerBenchmark(parseWorkerBenchmarkArgs(["--mode","compare","--tasks","2"]));assert.equal(compare.outputEquivalent,true);assert.equal(compare.modelCalls,0);assert.equal(compare.networkCalls,0);assert.ok(compare.responseBytes<8*1024*1024);const queue=await runWorkerBenchmark(parseWorkerBenchmarkArgs(["--mode","queue","--slots","2","--jobs","5"]));/* Equivalent benchmark requests now coalesce even with distinct job IDs. */assert.equal(queue.maximumActiveWorkers,1);assert.deepEqual(queue.artifacts,{tickets:0,slots:0});});
