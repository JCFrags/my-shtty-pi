...
interface Owner { readonly schemaVersion: 1; readonly pid: number; readonly processStartIdentity: string; readonly nonce: string; readonly createdAtMs: number; readonly priority: WorkerPriority; readonly jobType: WorkerJobType; readonly key?: string; }
...
export interface SchedulerOptions { readonly slots?: number; readonly timeoutMs?: number; readonly pollMs?: number; readonly priority: WorkerPriority; readonly jobType: WorkerJobType; readonly signal?: AbortSignal; readonly directory?: string; readonly malformedStableMs?: number; readonly sessionKey?: string; readonly enforcePolicy?: boolean; }
function startIdentity(text: string): string | undefined { const end = text.lastIndexOf(") "); if (end < 0) return undefined; const token = text.slice(end + 2).trim().split(/\s+/)[19]; return token && /^\d+$/.test(token) ? token : undefined; }
export function linuxProcessStartIdentity(pid=process.pid): string | undefined { try { return startIdentity(readFileSync(`/proc/${pid}/stat`, "utf8")); } catch { return undefined; } }
...
export async function acquireHostWorkerSlot(options:SchedulerOptions):Promise<SchedulerLease>{
 ...
 if(!Number.isInteger(slots)||slots<WORKER_LIMITS.slots.min||slots>WORKER_LIMITS.slots.max)throw new Error("host worker slots must be from 1 through 4");
 ...
 const transaction=<T>(action:()=>Promise<T>)=>withRuntimeMutex(join(directory,"queue.lock"),action);
 ...
 for(;;){
   ...
   const acquired=await transaction(async()=>{
     await cleanup(directory,malformedStableMs);
     const tickets:Owner[]=[];
     for(const name of (await readdir(directory)).filter(x=>x.startsWith("ticket-"))){const value=await readOwner(join(directory,name));if(value)tickets.push(value);}
     let turns:Record<string,number>={};
     const turnsPath=join(directory,"turns.json");
     try{const metadata=await lstat(turnsPath);if(metadata.isFile()&&!metadata.isSymbolicLink()&&metadata.uid===process.getuid?.()&&(metadata.mode&0o077)===0&&metadata.size<=16384){const value=JSON.parse(await readFile(turnsPath,"utf8"));if(value&&typeof value==="object"&&!Array.isArray(value)&&Object.keys(value).length<=128&&Object.entries(value).every(([k,v])=>/^[a-f0-9]{64}$/.test(k)&&Number.isSafeInteger(v)))turns=value;}}catch{}
     const now=Date.now();const rank=(ticket:Owner)=>now-ticket.createdAtMs>=WORKER_LIMITS.starvationMs?-1:priorityValue(ticket.priority);
     ...
     const index=tickets.findIndex(x=>x.nonce===nonce);maximumPosition=Math.max(maximumPosition,index+1);
