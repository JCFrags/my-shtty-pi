...
export function captureLogicalStateCheckpoints(pi: ExtensionAPI, ctx: ExtensionCommandContext): LogicalStateCheckpoint[] {
  ...
  if (!sourceLeafId) return fail();
  const requestId = randomUUID(), entries: unknown[] = [], responded = new Set<string>();
  let refused = false;
  const unsubscribe = pi.events.on("grounded-state:checkpoint-response-v1", (reply: unknown) => {
    if (!object(reply) || reply.requestId !== requestId) return;
    ...
    try { entries.push(JSON.parse(JSON.stringify(reply.entry))); }
    catch { refused = true; }
  });
  try { pi.events.emit("grounded-state:checkpoint-request-v1", { version: 1, requestId, sourceSessionId, sourceLeafId,
  ...
  if (refused || responded.size !== installed.size || ctx.sessionManager.getSessionId() !== sourceSessionId
    || ctx.sessionManager.getLeafId() !== sourceLeafId) return fail();
