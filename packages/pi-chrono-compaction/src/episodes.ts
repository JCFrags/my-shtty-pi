...
function turnRanges(units: readonly CandidateUnit[], blocksById: ReadonlyMap<string, HistoricalBlock>): TurnRange[] {
  const starts: number[] = [];
  units.forEach((unit, index) => {
    if (unit.kind === "user" || unit.kind === "custom_message") starts.push(index);
  });
  if (starts.length === 0 && units.length > 0) starts.push(0);
  ...
  starts.forEach((start, index) => {
    ...
    const completed = slice.some((unit) => {
      if (unit.kind !== "assistant_text") return false;
...
function bestCompactCandidate(unit: CandidateUnit): RepresentationCandidate {
  const candidates = [...unit.candidates].filter((candidate) => candidate.level !== "absent");
  return (
    candidates.find((candidate) => candidate.level === "reduced" || candidate.level === "semantic") ??
...
function episodeBody(units: readonly CandidateUnit[], config: CompactorConfig): string {
  ...
  for (const unit of units) {
    ...
    if (unit.kind === "user" || unit.kind === "custom_message") continue;
    ...
    if (sequence.length >= 22) break;
  }

  const parts: string[] = [];
  if (request) parts.push(`Request:\n${truncateToTokens(bestCompactCandidate(request).text, 120, "…")}`);
  if (sequence.length > 0) parts.push(`Sequence:\n${sequence.join("\n")}`);
  if (final) parts.push(`Result / last assistant state:\n${truncateToTokens(bestCompactCandidate(final).text, 120, "…")}`);
  return truncateToTokens(parts.join("\n\n"), config.maxEpisodeTokens, "\n…[additional episode steps omitted]…\n");
}

function makeEpisode(units: readonly CandidateUnit[], config: CompactorConfig): CandidateUnit {
  const first = units[0]!;
  const last = units[units.length - 1]!;
  const sourceRefs: SourceRef[] = units.flatMap((unit) => unit.sourceRefs);
  ...
  const merged: RepresentationCandidate = {
  ...
  const markerText = `Completed historical task episode spanning ${first.sourceRefs[0]?.entryId ?? "?"}–${
    last.sourceRefs[last.sourceRefs.length - 1]?.entryId ?? "?"
  }. Detailed chronological steps omitted; exact range remains recoverable.`;
  const marker: RepresentationCandidate = {
...
function routineSegmentProtected(unit: CandidateUnit): boolean {
  if (unit.protectedExact || unit.kind === "user" || unit.kind === "custom_message" || unit.kind === "assistant_text" || unit.kind === "episode") return true;
  return false;
}

function makeRoutineSegment(
  units: readonly CandidateUnit[],
  config: CompactorConfig,
  blocksByEntry: ReadonlyMap<string, readonly HistoricalBlock[]>,
): CandidateUnit {
  const first = units[0]!;
  const last = units[units.length - 1]!;
 ...
 */
export function mergeRoutineActivitySegments(
  units: readonly CandidateUnit[],
  ...
  if (units.length <= desiredUnits) return [...units];
  const blocksByEntry = new Map<string, HistoricalBlock[]>();
  ...
  const mergeableCount = units.reduce(
    (count, unit, index) => count + (index < recentStart && !routineSegmentProtected(unit) ? 1 : 0),
    0,
  );
  ...
  const output: CandidateUnit[] = [];
  let pending: CandidateUnit[] = [];
  const flush = (): void => {
    if (pending.length >= 2) output.push(makeRoutineSegment(pending, config, blocksByEntry));
...
export function mergeOldCompletedEpisodes(
  units: readonly CandidateUnit[],
  blocks: readonly HistoricalBlock[],
  config: CompactorConfig,
): CandidateUnit[] {
  if (!config.mergeEpisodes || units.length === 0) return [...units];
  ...
  while (rangeIndex < ranges.length) {
    ...
    while (cursor < range.start) output.push(units[cursor++]!);
    if (!range.completed || !range.mergeable || range.end >= eligibleLimit) {
      while (cursor <= range.end) output.push(units[cursor++]!);
      rangeIndex += 1;
      continue;
    }
    ...
    while (groupEndRange < ranges.length && groupEndRange < rangeIndex + 6) {
      ...
      groupEndRange += 1;
      if (rawTokens >= config.minEpisodeRawTokens && unitCount >= 6) break;
    }
  ...
  while (cursor < units.length) output.push(units[cursor++]!);
  return output;
}
