...
export interface SessionRolloutIdentity { sessionId: string; sourcePath: string }
const digest = (value: string): string => createHash("sha256").update(value).digest("hex");
const fail = (): never => { throw Object.assign(new Error("search-v3-rollout-unsafe"), { code: "search-v3-rollout-unsafe" }); };
function identity(value: SessionRolloutIdentity): { sessionKey: string; sourceKey: string } {
  if (!value.sessionId || value.sessionId.length > 1024 || !isAbsolute(value.sourcePath) || value.sourcePath.length > 4096) return fail();
  return { sessionKey: digest(`pi-session-v1\0${value.sessionId}`), sourceKey: digest(`pi-jsonl-v1\0${value.sourcePath}`) };
}
export function sessionRolloutPath(directory: string, value: SessionRolloutIdentity): string {
  const keys = identity(value);
  if (!isAbsolute(directory) || resolve(directory) !== directory) return fail();
...
async function safeDirectory(directory: string): Promise<void> {
  ...
  for (const part of directory.split("/").filter(Boolean)) {
    ...
    if (!s.isDirectory() || s.isSymbolicLink() || ![0, process.getuid?.()].includes(s.uid) ||
      ((s.mode & 0o022) !== 0 && !(s.uid === 0 && (s.mode & 0o1000)))) fail();
  }
  const s = await lstat(directory);
  if (s.uid !== process.getuid?.() || (s.mode & 0o777) !== 0o700 || await realpath(directory) !== directory) fail();
}
/** One owner-only record for this exact session and source, never history I/O. */
export async function readSessionRollout(directory: string, value: SessionRolloutIdentity): Promise<boolean | undefined> {
  ...
  try {
    ...
    try {
      ...
      if (!s.isFile() || s.uid !== process.getuid?.() || s.nlink !== 1 || (s.mode & 0o777) !== 0o600 || s.size > 4096) return fail();
      const buffer = Buffer.alloc(4097);
      const { bytesRead } = await file.read(buffer, 0, buffer.length, 0);
      if (bytesRead > 4096) return fail();
      ...
      if (Object.keys(record).sort().join(",") !== "enabled,schemaVersion,sessionKey,sourceKey" || record.schemaVersion !== 1 ||
        record.sessionKey !== keys.sessionKey || record.sourceKey !== keys.sourceKey || typeof record.enabled !== "boolean") return fail();
...
export async function writeSessionRollout(directory: string, value: SessionRolloutIdentity, enabled: boolean): Promise<void> {
  ...
  try { await safeDirectory(directory); await rename(temporary, path); }
