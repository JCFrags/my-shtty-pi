...
interface MemoryRow { readonly index: number; readonly id: string; readonly text: string; readonly importance: number }
...
export function composeBoundedMemory(input: {
...
}): { summary: string; receipt: { mode: "bounded-programmatic-fallback"; reason: string;
  inspectedEntries: number; selectedEntries: number; omittedEntries: number; earlierPrefixOmitted: boolean;
  reusedPreviousMemory: boolean; complete: false; renderedTokens: number; combinedTokens: number; combinedCeilingTokens: number } } {
  ...
  let priorTailStart: string | undefined;
