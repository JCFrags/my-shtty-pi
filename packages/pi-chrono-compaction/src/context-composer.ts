...
import type {
  ...
  EpisodeStateSelection,
  EpisodeStateSelectionItem,
  EpisodeStateSelectionMember,
...
export interface ComposerSelectionRow {
  readonly id: string;
  readonly text: string;
  readonly startSeq: number;
  readonly endSeq: number;
  readonly recovery: string;
  readonly kind: ComposerRowKind;
...
export interface ShadowComposerInput {
  ...
  readonly mandatoryCoverage: {
    readonly protectedComplete: boolean;
    readonly openWorkComplete: boolean;
  };
  readonly selected: {
    readonly protected: readonly ComposerSelectionRow[];
    readonly openWork: readonly ComposerSelectionRow[];
    readonly recent: readonly ComposerSelectionRow[];
    readonly older: readonly ComposerSelectionRow[];
  };
  readonly delta: {
    readonly records: readonly ComposerSelectionRow[];
    readonly completeThroughCut: boolean;
  };
  readonly limitations?: {
    readonly unsupportedExtraction?: readonly string[];
    readonly selectionLoss?: readonly string[];
    readonly lag?: readonly string[];
  };
}

export type ShadowDegradationLevel =
  | "committed-plus-delta"
  | "committed-without-delta"
  | "last-good-state-and-recent"
  | "last-good-state-summary-tail"
  | "pi-summary-and-tail"
  | "pi-default-required";

export interface ShadowCompositionEnvelope {
  readonly schemaVersion: 1;
  readonly sourceCutEntryId: string;
  readonly sourceCutSeq: number;
  readonly firstKeptEntryId: string;
  readonly memoryGeneration: string;
  readonly memoryRepresentedRange: readonly [number, number];
  readonly memoryLag: number;
  readonly rollupGeneration?: string;
  readonly rollupRepresentedRange?: readonly [number, number];
  readonly rollupLag?: number;
  ...
  /** Hash of the composition payload before a preview adds comparison data. */
  readonly payloadHash: string;
  /** Hash of the artifact represented by this envelope. A preview replaces it with the final comparison-artifact hash. */
  readonly artifactHash: string;
}

export interface ShadowCompositionArtifact {
  readonly schemaVersion: 1;
  ...
  readonly selectedRows: readonly {
    readonly section: "protected" | "open-work" | "recent" | "older" | "delta";
...
export interface ShadowCompositionResult {
  readonly status: "composed" | "pi-default-required";
  readonly text: string;
  readonly firstKeptEntryId: string;
  readonly degradation: ShadowDegradationLevel;
  readonly degradationReasons: readonly string[];
  readonly envelope: ShadowCompositionEnvelope;
  readonly artifact: ShadowCompositionArtifact;
}

export interface PersistedCompositionArtifact {
  readonly artifactHash: string;
...
export function composeStoredSelection(
  ...
  type ExtendedSelection = EpisodeStateSelection & {
  ...
  const extended = selection as ExtendedSelection;
  const protectedRows: ComposerSelectionRow[] = [], openWork: ComposerSelectionRow[] = [];
  const recent: ComposerSelectionRow[] = [], older: ComposerSelectionRow[] = [], deltaRows: ComposerSelectionRow[] = [];
  const unsupportedProtected: string[] = [], unsupportedOpenWork: string[] = [], unsupportedOptional: string[] = [];

  const exactRow = (item: EpisodeStateSelectionItem, maximumCut: number): ComposerSelectionRow | undefined => {
    const evidence = item.evidence as { source?: unknown; decodedUtf16?: unknown; exactText?: unknown;
      omissions?: unknown; contextComplete?: unknown } | null;
    const decoded = evidence?.decodedUtf16 as { start?: unknown; end?: unknown } | undefined;
    const omissions = Array.isArray(evidence?.omissions) ? evidence.omissions : undefined;
    const omission = omissions?.length === 1
      ? omissions[0] as { beforeUtf16?: unknown; afterUtf16?: unknown } : undefined;
    if (!evidence || !isScopedBodySourceRef(evidence.source) || !sourceRefWithinViewBounds(evidence.source, selection.sourceView)
      ...
      || (decoded.end as number) - (decoded.start as number) !== evidence.exactText.length
      || (decoded.start as number) < evidence.source.decodedUtf16.start || (decoded.end as number) > evidence.source.decodedUtf16.end
      || !omissions || omissions.length > 1
      || omissions.length === 0 && ((decoded.start as number) !== evidence.source.decodedUtf16.start
        || (decoded.end as number) !== evidence.source.decodedUtf16.end)
      || omissions.length === 1 && (!omission || !Number.isSafeInteger(omission.beforeUtf16) || !Number.isSafeInteger(omission.afterUtf16)
        || omission.beforeUtf16 !== (decoded.start as number) - evidence.source.decodedUtf16.start
  ...
  for (const item of selection.protected) addState(item, selection.processedCut, false);
  ...
  if (delta && Number.isSafeInteger(delta.throughCut) && delta.throughCut >= selection.processedCut
    ...
    for (const item of delta.protected) addState(item, delta.throughCut, verifiedDelta);
    for (const item of delta.current) addState(item, delta.throughCut, verifiedDelta);
    ...
    for (const member of delta.recent) addMember(member, verifiedDelta ? delta.throughCut : selection.requestedCut,
      verifiedDelta ? deltaRows : recent, "episode");
  }
  ...
  const dedupeRows = (rows: readonly ComposerSelectionRow[]): ComposerSelectionRow[] => {
  ...
  const uniqueOptional = (rows: readonly ComposerSelectionRow[]): ComposerSelectionRow[] => rows.filter(row => {
    const key = `${row.startSeq}\n${row.recovery}`;
...
function assertRows(rows: readonly ComposerSelectionRow[], delta: boolean): void {
  const limit = delta ? SHADOW_COMPOSER_LIMITS.maxDeltaRows : SHADOW_COMPOSER_LIMITS.maxRows;
  if (rows.length > limit) throw new Error(`bounded composer row count exceeds ${limit}`);
  for (const row of rows) {
    if (!row.id || !row.text || !row.recovery) throw new Error("composer rows require id, text, and opaque recovery reference");
    assertInteger("row.startSeq", row.startSeq);
    assertInteger("row.endSeq", row.endSeq);
    if (row.endSeq < row.startSeq) throw new Error(`composer row ${row.id} has an inverted source range`);
    ...
    if (byteCount(row.text) > SHADOW_COMPOSER_LIMITS.maxRowBytes) throw new Error(`composer row ${row.id} text exceeds its byte cap`);
    if (byteCount(row.recovery) > SHADOW_COMPOSER_LIMITS.maxRecoveryBytes) throw new Error(`composer row ${row.id} recovery reference exceeds its byte cap`);
  }
}
...
function validateInput(input: ShadowComposerInput): void {
  ...
  if (!input.cut.sourceCutEntryId || !input.cut.firstKeptEntryId) throw new Error("composer cut requires source and retained-tail entry IDs");
  if (input.cut.firstKeptSeq <= input.cut.sourceCutSeq) throw new Error("retained tail must begin after the summarized prefix cut");
  ...
  if (input.memory.representedEndSeq < input.memory.representedStartSeq) throw new Error("memory represented range is inverted");
  if (input.memory.representedEndSeq > input.cut.sourceCutSeq) throw new Error("memory generation is beyond the source cut");
  if (input.rollups) {
    ...
    if (input.rollups.representedEndSeq < input.rollups.representedStartSeq) throw new Error("rollup represented range is inverted");
    if (input.rollups.representedEndSeq > input.cut.sourceCutSeq) throw new Error("rollup generation is beyond the source cut");
  }
  const groups = [input.selected.protected, input.selected.openWork, input.selected.recent, input.selected.older];
  for (const rows of groups) assertRows(rows, false);
  ...
  for (const row of groups.flatMap((rows) => rows).concat(input.delta.records)) {
    if (row.endSeq > input.cut.sourceCutSeq) throw new Error(`composer row ${row.id} extends beyond the summarized prefix cut`);
  }
  const totalRows = groups.reduce((sum, rows) => sum + rows.length, 0) + input.delta.records.length;
  if (totalRows > SHADOW_COMPOSER_LIMITS.maxRows) throw new Error(`combined composer row count exceeds ${SHADOW_COMPOSER_LIMITS.maxRows}`);
  if (byteCount(stableStringify(input)) > SHADOW_COMPOSER_LIMITS.maxInputBytes) throw new Error("bounded composer input exceeds its total byte cap");
}

function chronological(rows: readonly ComposerSelectionRow[]): ComposerSelectionRow[] {
...
function degradationFor(input: ShadowComposerInput): { level: ShadowDegradationLevel; reasons: string[] } {
  ...
  if (!input.cut.toolPairSafe) return { level: "pi-default-required", reasons: [...reasons, "the supplied retained-tail cut failed tool-pair validation"] };
...
function replayText(input: ShadowComposerInput, level: ShadowDegradationLevel, reasons: readonly string[], rows: readonly SectionRow[]): string {
  const memoryLag = Math.max(0, input.cut.sourceCutSeq - input.memory.representedEndSeq);
...
export function composeShadowContext(input: ShadowComposerInput): ShadowCompositionResult {
  validateInput(input);
  let { level, reasons } = degradationFor(input);
  let rows = sectionsFor(input, level);
  const omittedRowIds: string[] = [];
  ...
  for (const drop of optionalDropOrder(rows)) {
    if (renderedTokens + input.cut.rawTailTokens <= input.combinedCeilingTokens) break;
  ...
  const envelope: ShadowCompositionEnvelope = {
    schemaVersion: 1,
    ...
    ...(input.rollups ? {
      ...
      rollupRepresentedRange: [input.rollups.representedStartSeq, input.rollups.representedEndSeq] as const,
...
async function validatePrivateDirectory(directory: string): Promise<void> {
  ...
  if (uid === undefined) throw new Error("artifact persistence requires an owner-aware filesystem");
  let current = "/";
...
function artifactStringify(value: unknown, space = 0): string {
...
export async function persistPrivateCompositionArtifact(
  directory: string,
  artifact: ShadowCompositionArtifact,
): Promise<PersistedCompositionArtifact> {
  const canonical = artifactStringify(artifact);
  const serialized = `${artifactStringify(artifact, 2)}\n`;
  if (byteCount(serialized) > SHADOW_COMPOSER_LIMITS.maxInputBytes) throw new Error("composition artifact exceeds its persistence byte cap");
