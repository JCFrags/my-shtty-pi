import type { CompressionPlan, PlannedUnit, RepresentationCandidate, SourceRef } from "./types.js";
import { estimateTokensFromText, formatSourceRef, unique } from "./utils.js";

const PATH = /(?:\.?\.?\/|\/)[\w@.+\-~]+(?:\/[\w@.+\-~]+)+/g;

function firstAndLastRefs(unit: PlannedUnit): { first?: SourceRef; last?: SourceRef } {
...
function treatment(candidate: RepresentationCandidate): string {
  if (candidate.level === "raw") return candidate.lossy ? "text; non-text source recoverable" : "exact";
  if (candidate.level === "normalized") return `lossless ${candidate.rawTokens}→${candidate.tokens}`;
  if (candidate.level === "absent") return "absent";
  return `${candidate.reducer ?? candidate.level} ${candidate.rawTokens}→${candidate.tokens}`;
}

function header(unit: PlannedUnit): string {
  const first = unit.sourceRefs[0];
...
function recoveryLine(unit: PlannedUnit): string | undefined {
  if (!unit.selected.lossy && unit.selected.level !== "normalized") return undefined;
  const { first, last } = firstAndLastRefs(unit);
  if (!first) return undefined;
  const entries = unique(unit.sourceRefs.map((source) => source.entryId));
  if (unit.kind === "episode" || entries.length > 1) return `Exact range: history_range("${first.entryId}", "${last?.entryId ?? first.entryId}")`;
...
function commonDirectory(paths: readonly string[]): string | undefined {
  ...
  const first = split[0]!;
  let count = 0;
  while (count < first.length - 1 && split.every((parts) => parts[count] === first[count])) count += 1;
  if (count < 2) return undefined;
  const prefix = first.slice(0, count).join("/") || "/";
  return prefix.length >= 8 ? prefix : undefined;
}

function factorPathPrefix(text: string): { text: string; prefix?: string } {
...
function renderUnit(unit: PlannedUnit): string | undefined {
  if (unit.selected.level === "absent") return undefined;
  ...
  if (factored.prefix) sections.push(`Path base: ${factored.prefix}`);
...
export function renderCompressionPlan(plan: CompressionPlan, generationHash: string, includeHeader = true): RenderedPlan {
  ...
  if (absentCount > 0) sections.push(`${absentCount} low-information control block(s) omitted. Exact history remains searchable.`);
  if (plan.warnings.length > 0) sections.push(`Warnings:\n${plan.warnings.map((warning) => `- ${warning}`).join("\n")}`);
