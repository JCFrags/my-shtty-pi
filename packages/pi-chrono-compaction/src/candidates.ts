...
const PRECOMPUTED_REDUCER_VERSIONS: Readonly<Record<string, string>> = Object.freeze({
...
function candidateRecordIntegrityHash(
  blockId: string,
  key: string,
...
const BASE_IMPORTANCE: Record<HistoricalBlock["kind"], number> = {
  user: 125,
...
function recordedExitCode(block: HistoricalBlock): number | undefined {
...
function hasDecisiveFailureEvidence(block: HistoricalBlock): boolean {
  ...
  if (block.kind === "bash_execution" || (block.kind === "tool_result" && /^(?:bash|shell|terminal|exec)$/i.test(block.toolName ?? ""))) {
    ...
    if (block.isError === false) return false;
  ...
  ) && hasFailureLanguage(block.exactText);
}

function hasUnresolvedTaskEvidence(block: HistoricalBlock): boolean {
  return (
    block.unresolved &&
    (block.kind === "user" ||
      block.kind === "custom_message" ||
...
function scoreImportance(
  ...
  total: number,
  retentionHints = "",
  historyAnalysis?: BlockHistoryAnalysis,
): number {
  ...
  if (block.protectedExact) score += 220;
  if (hasUnresolvedTaskEvidence(block)) score += 90;
  if (hasDecisiveFailureEvidence(block)) score += 85;
  if (block.reproducible && !block.isError) score -= 16;
  if (block.kind === "assistant_text") {
    const phase = getString(block.attributes.phase);
    if (phase === "preamble") score -= 28;
    if (phase === "final") score += 20;
  }
  if (block.kind === "tool_result" && block.rawTokens > 2_000 && !block.isError) score -= 10;
  if (block.exactIdentifiers.length > 0) score += Math.min(20, block.exactIdentifiers.length * 2);
  if (retentionHints.trim()) {
    ...
    if (hintTerms.length > 0) score += Math.min(180, 55 + hintTerms.length * 15);
  }
...
function candidate(
  block: HistoricalBlock,
  level: RepresentationCandidate["level"],
  text: string,
  utility: number,
  options: {
    lossy?: boolean;
    ...
    omissions?: readonly OmissionNotice[];
...
function reducedBudget(block: HistoricalBlock, config: CompactorConfig): number {
  switch (block.kind) {
...
function looksLikeGeneratedOutput(block: HistoricalBlock): boolean {
  const command = getString(block.toolArguments?.command) ?? getString(block.attributes.command) ?? "";
  if (/(?:\bsource[-_.]?map\b|\.map\b|\.min\.(?:js|css)\b)/i.test(command)) return true;
...
function markerText(block: HistoricalBlock): string {
  ...
  if (block.kind === "tool_call") {
    ...
    const importantArguments = Object.fromEntries(
      importantArgumentNames.flatMap((name) => {
...
function compactUnknown(value: unknown, depth: number, omissions: { count: number }): unknown {
  if (depth > 5) {
    omissions.count += 1;
  ...
  if (Array.isArray(value)) {
    if (value.length <= 8) return value.map((item) => compactUnknown(item, depth + 1, omissions));
  ...
  if (value !== null && typeof value === "object") {
    ...
    for (const key of Object.keys(record).sort()) {
      const critical = /^(?:command|path|file|file_path|query|pattern|id|url|method|cwd|offset|limit|line|start|end|revision|ref)$/i.test(
        key,
      );
      const child = record[key];
      if (critical || Object.keys(record).length <= 12) result[key] = compactUnknown(child, depth + 1, omissions);
      else if (typeof child === "string" && child.length <= 240) result[key] = child;
      else omissions.count += 1;
    }
    return result;
  }
  return value;
...
function reduceToolCall(block: HistoricalBlock): RepresentationCandidate | undefined {
  if (!block.toolArguments || block.rawTokens < 280) return undefined;
  ...
  if (estimateTokensFromText(text) >= block.rawTokens * 0.9) return undefined;
...
function dedupeCandidates(candidates: readonly RepresentationCandidate[]): RepresentationCandidate[] {
...
export function candidatePrecomputeKey(
  block: HistoricalBlock,
  laterText: string,
...
function sourceRefsMatchBlock(candidate: RepresentationCandidate, block: HistoricalBlock): boolean {
  if (!Array.isArray(candidate.sourceRefs) || candidate.sourceRefs.length !== block.sourceRefs.length) return false;
...
function expectedCachedCandidateShape(
  block: HistoricalBlock,
  config: CompactorConfig,
  reducer: string,
): { level: RepresentationCandidate["level"]; utility: number } | undefined {
  if (reducer === "tool-arguments" && block.kind === "tool_call") return { level: "reduced", utility: 0.9 };
  if (reducer === "tool-call-marker" && block.kind === "tool_call") return { level: "marker", utility: 0.34 };
  ...
  if (["terminal", "test-output", "file-read", "git-diff", "search-results", "structured-json"].includes(reducer)) {
    if (block.kind !== "tool_result" && block.kind !== "bash_execution") return undefined;
 ...
 */
function validatedPrecomputedRecord(
  precomputed: CandidatePrecomputeRecord | undefined,
  block: HistoricalBlock,
  expectedKey: string,
  config: CompactorConfig,
  ...
  for (const item of precomputed.candidates) {
    ...
    if (typeof item.text !== "string" || item.text.length > MAX_PRECOMPUTED_CANDIDATE_TEXT_CHARS) return undefined;
    if (!Number.isSafeInteger(item.tokens) || item.tokens < 0 || item.tokens !== estimateTokensFromText(item.text)) return undefined;
    if (!Number.isSafeInteger(item.rawTokens) || item.rawTokens !== block.rawTokens) return undefined;
    if (!Number.isFinite(item.utility) || item.lossy !== true) return undefined;
    if (typeof item.reducer !== "string" || typeof item.reducerVersion !== "string") return undefined;
    ...
    if (!shape || item.level !== shape.level || item.utility !== shape.utility || item.reducerVersion !== expectedVersion) return undefined;
    const expectedId = `${block.id}:${item.level}:${item.reducer}`;
    if (item.id !== expectedId || candidateIds.has(item.id)) return undefined;
    candidateIds.add(item.id);
    if (!sourceRefsMatchBlock(item, block)) return undefined;
  }
  return precomputed;
}

async function candidatesForBlock(
  block: HistoricalBlock,
  laterText: string,
  config: CompactorConfig,
  ...
  const candidates: RepresentationCandidate[] = [
    candidate(block, "raw", block.exactText, 1,
      containsOpaqueImage
        ? {
            lossy: true,
            ...
            omissions: [
              {
                description:
                  "Original image bytes are not embedded in the text-only replay; an explicit image marker is retained and exact JSONL remains recoverable",
              },
            ],
  ...
  if (block.protectedExact) {
    ...
    if (directText !== block.exactText && estimateTokensFromText(directText) < block.rawTokens * 0.9) {
      candidates.push(
        candidate(block, "reduced", directText, 0.97, {
  ...
  if (normalized && estimateTokensFromText(normalized.text) <= block.rawTokens) {
    candidates.push(
      candidate(block, "normalized", normalized.text, 0.995, {
  ...
  if (block.kind === "tool_call") {
    ...
    if (reducedCall) candidates.push(reducedCall);
    candidates.push(
      candidate(block, "marker", markerText(block), 0.34, {
  ...
  if (
    ...
    if (response?.text) {
      ...
      if (estimateTokensFromText(semanticText) < block.rawTokens * 0.95) {
        candidates.push(
          candidate(block, "semantic", semanticText, 0.8, {
  ...
  if (block.kind !== "user" && block.kind !== "custom_message") {
    candidates.push(
      candidate(block, "marker", markerText(block), block.kind === "tool_result" && block.isError ? 0.5 : 0.28, {
...
function laterTextForBlock(
  block: HistoricalBlock,
  fallbackIndex: number,
  analysisBlocks: readonly HistoricalBlock[],
  analysisIndexById: ReadonlyMap<string, number>,
): string {
...
function cacheSafeCandidate(candidate: RepresentationCandidate): RepresentationCandidate {
  const metadata: Record<string, unknown> = {};
  for (const [key, value] of Object.entries(candidate.metadata)) {
    if (value === null || typeof value === "boolean" || typeof value === "number") metadata[key] = value;
    else if (typeof value === "string" && value.length <= 512) metadata[key] = value;
  }
 ...
 */
export async function precomputeCandidateRepresentations(
  blocks: readonly HistoricalBlock[],
  config: CompactorConfig,
  previous: ReadonlyMap<string, CandidatePrecomputeRecord> = new Map(),
  signal?: AbortSignal,
): Promise<CandidatePrecomputeResult> {
  ...
  for (let index = 0; index < blocks.length; index += 1) {
    if (signal?.aborted) throw signal.reason instanceof Error ? signal.reason : new Error("Incremental precompute aborted");
...
export async function buildCandidateUnits(
  ...
  signal?: AbortSignal,
  retentionHints = "",
  analysisBlocks: readonly HistoricalBlock[] = blocks,
  precomputedCandidates: ReadonlyMap<string, CandidatePrecomputeRecord> = new Map(),
  ...
  for (let index = 0; index < blocks.length; index += 1) {
    if (signal?.aborted) throw signal.reason instanceof Error ? signal.reason : new Error("Compaction aborted");
    const block = blocks[index];
    if (!block) continue;
    const laterText = laterTextForBlock(block, index, analysisBlocks, analysisIndexById);
    const representations = await candidatesForBlock(
      block,
      laterText,
      config,
      semanticCompressor,
      signal,
      precomputedCandidates.get(block.id),
    );
...
export function sourceIdentifiersForUnit(unit: CandidateUnit, blocksById: ReadonlyMap<string, HistoricalBlock>): string[] {
  return unique(
    unit.sourceRefs.flatMap((ref) => {
      const block = [...blocksById.values()].find(
        (candidateBlock) => candidateBlock.entryId === ref.entryId && candidateBlock.blockIndex === ref.blockIndex,
      );
