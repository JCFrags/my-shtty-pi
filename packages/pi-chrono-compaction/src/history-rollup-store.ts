...
import { acquireHistoryRollupLock, type HistoryRollupLockOptions } from "./history-rollup-lock.js";
import {
  extractHistoryValues,
  fullHistoryHash,
  historyStaticValue,
  type HistoryRelation,
  type HistoryValueCategory,
  type HistoryValueRecord,
} from "./history-value.js";
import { updateSourceLedger, type SourceLedger, type SourceLedgerEntry } from "./source-ledger.js";
...
export type HistoryRollupTransition =
  | "new"
  | "exact-hit"
  | "append"
  | "append-open-leaf"
  | "branch-switch-reuse"
  | "branch-switch-rebuild-tail"
  | "source-replacement"
  | "source-truncation"
  | "source-tail-rewrite"
...
export const DEFAULT_HISTORY_ROLLUP_CONFIG: HistoryRollupConfig = {
...
export interface HistoryNodeQueryIndex {
  ...
  readonly sourceOrderRange: { readonly start: number; readonly end: number };
...
interface NodeBase {
  ...
  readonly branchOrderRange: { readonly start: number; readonly end: number };
...
export interface HistoryLeafNode extends NodeBase {
  ...
  readonly sourceByteRange: { readonly start: number; readonly end: number };
...
export interface HistoryBranchManifest {
  ...
  readonly openLeafState: { readonly nodeId: string; readonly start: number; readonly end: number } | null;
...
export interface HistoryRollupRuntime {
  ...
  ledger?: SourceLedger;
  readonly cache: Map<string, { node: HistoryNode; bytes: number }>;
  cacheBytes: number;
  cacheHits: number;
  cacheMisses: number;
...
function verifyIntegrity(value: Record<string, unknown>): boolean {
  if (typeof value.integrityHash !== "string" || value.integrityHash.length !== 64) return false;
  const copy = { ...value };
  delete copy.integrityHash;
  return fullHash(stableStringify(copy)) === value.integrityHash;
}

function nodeIdentity(value: Record<string, unknown>): { nodeId: string; contentHash: string } {
...
export function historyRollupStorePath(sessionPath: string): string {
  return `${sessionPath}${HISTORY_ROLLUP_SUFFIX}`;
}

export function historyRollupV1StorePath(sessionPath: string): string {
  return `${sessionPath}${HISTORY_ROLLUP_V1_SUFFIX}`;
}

export function createHistoryRollupRuntime(
  sessionPath: string,
  config: Partial<HistoryRollupConfig> = {},
): HistoryRollupRuntime {
  return {
    sessionPath,
...
function configurationHash(config: HistoryRollupConfig): string {
  return fullHash(stableStringify(config));
}

async function loadJson(path: string): Promise<Record<string, unknown>> {
  const value = JSON.parse(await readFile(path, "utf8")) as Record<string, unknown>;
  if (value.schemaVersion !== 2 || !verifyIntegrity(value)) throw new Error("history-rollup-integrity");
  return value;
}

export async function loadHistoryRollupManifest(
  runtime: HistoryRollupRuntime,
): Promise<HistoryMainManifest | undefined> {
  try {
    const manifest = await loadJson(join(runtime.directory, "manifest.json")) as unknown as HistoryMainManifest;
...
export async function loadHistoryBranchManifest(
  runtime: HistoryRollupRuntime,
  reference?: string,
): Promise<HistoryBranchManifest | undefined> {
  const ref = reference ?? runtime.manifest?.activeBranchManifestReference;
  if (!ref) return undefined;
  try {
    const branch = await loadJson(join(runtime.directory, "branches", ref)) as unknown as HistoryBranchManifest;
    if (!reference || reference === runtime.manifest?.activeBranchManifestReference) runtime.branchManifest = branch;
    return branch;
  } catch {
    if (!reference) runtime.branchManifest = undefined;
    return undefined;
  }
}

export async function loadHistoryNode(runtime: HistoryRollupRuntime, nodeId: string): Promise<HistoryNode> {
  ...
  const node = JSON.parse(text) as HistoryNode;
  ...
  while (runtime.cacheBytes > runtime.config.nodeCacheBytes && runtime.cache.size > 1) {
    const oldest = runtime.cache.entries().next().value as [string, { node: HistoryNode; bytes: number }] | undefined;
    if (!oldest) break;
...
async function writeNode(
  ...
  try {
    ...
    const existing = JSON.parse(existingText) as HistoryNode;
    if (
      existing.schemaVersion !== 2 ||
      existing.nodeId !== node.nodeId ||
      existing.contentHash !== node.contentHash ||
      !verifyIntegrity(existing as unknown as Record<string, unknown>) ||
      existingText !== text
...
function queryTermHashes(records: readonly HistoryValueRecord[], querySalt: string): string[] {
  const terms = new Set<string>();
  for (const record of records) {
    if (record.exactSourceRequired) continue;
...
function makeQueryIndex(
  records: readonly HistoryValueRecord[],
  start: number,
  end: number,
  querySalt: string,
): HistoryNodeQueryIndex {
  ...
  const base = {
    ...
    maximumPriority: records.map(record => record.priority).sort(
...
function archiveRecord(records: readonly HistoryValueRecord[], omitted: number): HistoryValueRecord | undefined {
  if (!omitted || !records.length) return undefined;
...
function selectRecords(
  ...
  nodeFraction = 0.6,
): { kept: HistoryValueRecord[]; omitted: Record<string, number>; archives: HistoryValueRecord[] } {
  const important = (record: HistoryValueRecord) =>
    record.priority === "A" ||
  ...
  for (const record of sorted) {
    ...
    if (fits) {
      ...
      recordBytes += bytes;
    } else omitted[record.category] = (omitted[record.category] ?? 0) + 1;
  }
...
function relationAuthority(authority: HistoryValueRecord["sourceAuthority"]): number {
  return { derived: 0, assistant: 1, tool: 2, user: 3, project: 4 }[authority];
}

function explicitRelation(record: HistoryValueRecord, kind: HistoryRelation["kind"], targetId: string): boolean {
  return record.relations.some(relation => relation.kind === kind && relation.targetRecordId === targetId);
}

export function aggregateHistoryState(records: readonly HistoryValueRecord[]): {
  current: HistoryValueRecord[];
  conflicts: HistoryValueRecord[];
} {
  const ordered = [...records].sort((a, b) => a.sourceOrder.start - b.sourceOrder.start || a.id.localeCompare(b.id));
  const current: HistoryValueRecord[] = [];
  const conflicts: HistoryValueRecord[] = [];
  const byKey = new Map<string, HistoryValueRecord>();
  for (const record of ordered) {
    ...
    conflicts.push(
      { ...prior, lifecycle: "conflict", conflictSources: sources, relations: [...prior.relations, { kind: "conflict", targetRecordId: record.id, basis: "same-state-key-no-supersession" }] },
      { ...record, lifecycle: "conflict", conflictSources: sources, relations: [...record.relations, { kind: "conflict", targetRecordId: prior.id, basis: "same-state-key-no-supersession" }] },
    );
    byKey.delete(key);
  }
...
export function resolveHistoryLifecycles(records: readonly HistoryValueRecord[]): HistoryValueRecord[] {
  const ordered = [...records].sort((a, b) => a.sourceOrder.start - b.sourceOrder.start || a.id.localeCompare(b.id));
  const signatures = new Map<string, HistoryValueRecord[]>();
  const commandResources = new Map<string, HistoryValueRecord[]>();
  const explicitTasks = new Map<string, HistoryValueRecord[]>();
  const explicitRelations = new Map<string, HistoryValueRecord[]>();
  const taskClosers = new Map<string, HistoryValueRecord[]>();
  const add = (map: Map<string, HistoryValueRecord[]>, key: string | undefined, record: HistoryValueRecord): void => {
    if (!key) return;
  ...
  for (const candidate of ordered) {
    if (candidate.successEvidence === true) {
      ...
      add(commandResources, candidate.commandIdentity && candidate.resourceIdentity
        ...
        : undefined, candidate);
      if (/\b(resolve|correct|fix)\b/i.test(candidate.cue ?? "")) add(explicitTasks, candidate.taskIdentity, candidate);
      for (const relation of candidate.relations) {
        if (relation.kind === "resolution") add(explicitRelations, relation.targetRecordId, candidate);
      }
    }
    ...
    if (userAcceptance || recorded || validation) add(taskClosers, candidate.taskIdentity, candidate);
  }
  const firstAfter = (values: readonly HistoryValueRecord[] | undefined, sourceOrder: number): HistoryValueRecord | undefined => {
    if (!values?.length) return undefined;
    let low = 0;
    let high = values.length;
    while (low < high) {
      const middle = Math.floor((low + high) / 2);
      if (values[middle]!.sourceOrder.start <= sourceOrder) low = middle + 1;
      else high = middle;
    }
    return values[low];
  };
  const earliest = (values: readonly (HistoryValueRecord | undefined)[]): HistoryValueRecord | undefined =>
    values.filter((value): value is HistoryValueRecord => value !== undefined)
...
function combinedRecords(children: readonly HistoryNode[]): HistoryValueRecord[] {
...
function makeRollup(
  ...
  for (const child of children) {
    if (child.nodeType === "rollup") {
      ...
      for (const record of child.valueRecords) counts[record.category] = (counts[record.category] ?? 0) + 1;
    }
  }
  const base = {
    schemaVersion: 2 as const,
    nodeType: "rollup" as const,
    level,
  ...
  return withIntegrity({ ...base, ...ids }) as unknown as HistoryRollupNode;
}

function leafContexts(entries: readonly SessionEntryLike[]): Map<string, { toolName?: string; toolArguments?: Record<string, unknown> }> {
  const contexts = new Map<string, { toolName?: string; toolArguments?: Record<string, unknown> }>();
  for (const entry of entries) {
    ...
    for (const part of content) {
      ...
      const args = part.arguments && typeof part.arguments === "object" ? part.arguments as Record<string, unknown> : undefined;
...
function makeLeaf(
  ...
  ownEntries: readonly SessionEntryLike[],
  ledgerEntries: readonly SourceLedgerEntry[],
  branchStart: number,
  config: HistoryRollupConfig,
  querySalt: string,
): HistoryLeafNode {
  ...
  const selected = selectRecords(
    extractHistoryValues(ownBlocks, contexts).map(record => ({
      ...record,
      sourceOrder: {
  ...
  const ids = nodeIdentity(base);
  return withIntegrity({ ...base, ...ids }) as unknown as HistoryLeafNode;
}

function extendLeaf(
  old: HistoryLeafNode,
  added: HistoryLeafNode,
  config: HistoryRollupConfig,
  querySalt: string,
): HistoryLeafNode {
...
function initialGroups(entries: readonly SourceLedgerEntry[], start: number, config: HistoryRollupConfig): { start: number; end: number }[] {
  ...
  while (at < entries.length) {
    ...
    while (end < entries.length) {
      ...
      if (end > at && (end - at >= config.targetLeafEntries || nextBytes > config.targetLeafSourceBytes)) break;
      bytes = nextBytes;
      end++;
    }
...
async function boundedLeafGroups(
  runtime: HistoryRollupRuntime,
  ledger: SourceLedger,
  ...
  for (const group of initialGroups(entries, start, runtime.config)) {
    ...
    while (cursor < group.end) {
      ...
      while (end > cursor + 1) {
        ...
        if (blocks.length <= runtime.config.targetLeafBlocks) break;
...
function exactAppendSuffix(
  ledger: SourceLedger,
  previous: HistoryBranchManifest,
  requestedLeafId: string,
): SourceLedgerEntry[] | undefined {
  if (requestedLeafId === previous.branchLeafId) return [];
  ...
  if (!current || current.entryId !== previous.branchLeafId) return undefined;
  return reverse.reverse();
}

function emptyMetrics(transition: HistoryRollupTransition, previous?: HistoryBranchManifest): HistoryRollupMetrics {
  return {
    transition,
    sourceBytesRead: 0,
    entriesParsed: 0,
...
export async function updateHistoryRollupStore(
  runtime: HistoryRollupRuntime,
  branchLeafId: string,
  options: HistoryRollupUpdateOptions = {},
): Promise<HistoryRollupMetrics> {
  ...
  try {
    ...
    if (options.signal?.aborted) throw new Error("history-rollup-aborted");
    ...
    let transition: HistoryRollupTransition = oldMain ? "append" : manifestExists ? "manifest-corruption" : "new";
    let prior = previous;
    ...
    let branchEntries: readonly SourceLedgerEntry[];
    let appendedStart = 0;
    if (prior && suffix) {
      ...
      appendedStart = 0;
      if (prior) transition = "branch-switch-rebuild-tail";
    }
    const querySalt = oldMain?.querySalt ?? fullHash(`query:${ledger.sourceIdentity.deviceId}:${ledger.sourceIdentity.inodeId}`);
    const descriptors: HistoryLeafDescriptor[] = prior && suffix ? [...prior.leafNodes] : [];
    ...
    let cursor = 0;
    let activeOpenToolRefs: readonly SourceRef[] = prior?.openToolCallRefs ?? [];
    let allEntries: readonly SourceLedgerEntry[];
    if (prior && suffix) {
      allEntries = suffix;
      if (prior.openLeafState && descriptors.at(-1)?.sealed === false) {
        ...
        oldNodesLoaded++;
        if (oldNode.nodeType !== "leaf") throw new Error("history-rollup-node-type");
        let fill = 0;
        let bytes = oldDescriptor.sourceBytes;
        let count = oldDescriptor.entryCount;
        while (fill < suffix.length) {
          const next = suffix[fill]!;
          if (fill > 0 && (count >= runtime.config.targetLeafEntries || bytes + next.sourceByteLength > runtime.config.targetLeafSourceBytes)) break;
          bytes += next.sourceByteLength;
          count++;
          fill++;
          if (count >= runtime.config.targetLeafEntries || bytes >= runtime.config.targetLeafSourceBytes) break;
        }
        if (fill) {
    ...
    while (ids.length > 1) {
      ...
      for (let at = 0; at < ids.length; at += runtime.config.fanout) {
        ...
        const children = await Promise.all(childIds.map(async id => {
          ...
          if (prior && prior.levels.flat().includes(id)) oldNodesLoaded++;
          else newNodesLoaded++;
          return node;
        }));
    ...
    const reachableBytesById = new Map<string, number>();
    for (const descriptor of descriptors) reachableBytesById.set(descriptor.nodeId, descriptor.nodeBytes);
    ...
    const branchBase = {
      ...
      sourceLedgerIntegrityState: "verified" as const,
    ...
    if (transition === "branch-switch-rebuild-tail" && nodesReused > 0) transition = "branch-switch-reuse";
    const metrics: HistoryRollupMetrics = {
      transition,
      sourceBytesRead,
      entriesParsed,
      blocksParsed,
...
export async function readCurrentHistorySnapshot(
  runtime: HistoryRollupRuntime,
): Promise<HistoryNode | undefined> {
  ...
  if (!manifest || !branch) return undefined;
  return loadHistoryNode(runtime, branch.rootRollupNodeId);
}

export async function loadRecentHistoryLeavesByTokens(
  runtime: HistoryRollupRuntime,
  sourceTokens: number,
): Promise<HistoryLeafNode[]> {
  const branch = runtime.branchManifest ?? await loadHistoryBranchManifest(runtime);
  if (!branch) return [];
  const selected: HistoryLeafDescriptor[] = [];
  let tokens = 0;
  for (let index = branch.leafNodes.length - 1; index >= 0 && selected.length < runtime.config.recentLeafSafetyLimit; index--) {
    ...
    if (tokens >= sourceTokens) break;
  }
  const nodes = await Promise.all(selected.reverse().map(descriptor => loadHistoryNode(runtime, descriptor.nodeId)));
  return nodes.map(node => {
    if (node.nodeType !== "leaf") throw new Error("history-rollup-node-type");
    return node;
  });
}

export async function loadRecentHistoryLeaves(
  runtime: HistoryRollupRuntime,
  limit = 2,
): Promise<HistoryLeafNode[]> {
  const branch = runtime.branchManifest ?? await loadHistoryBranchManifest(runtime);
...
export async function cleanupHistoryRollupStore(
  runtime: HistoryRollupRuntime,
): Promise<{ nodesRemoved: number; branchesRemoved: number }> {
  const release = await acquireHistoryRollupLock(runtime.directory);
  try {
    const manifest = runtime.manifest ?? await loadHistoryRollupManifest(runtime);
    if (!manifest) return { nodesRemoved: 0, branchesRemoved: 0 };
    ...
    for (const ref of keepBranches) {
      ...
      if (!branch) continue;
      for (const leaf of branch.leafNodes) keepNodes.add(leaf.nodeId);
      for (const id of branch.levels.flat()) keepNodes.add(id);
    }
