...
export function extractIdentifiers(text: string): string[] {
  ...
  for (const pattern of patterns) {
    for (const match of text.matchAll(pattern)) {
      ...
      if (candidate.length >= 3 && candidate.length <= 240) identifiers.push(candidate);
      if (identifiers.length >= 80) return unique(identifiers);
    }
  }
  return unique(identifiers);
}

function omitCuedLongQuotes(text: string, pattern: RegExp): string {
  return text.replace(pattern, (match: string, _body: string, offset: number, source: string) => {
...
export function directInstructionText(text: string): string {
  const explicitlyDelimited = text.replace(
    /"([\s\S]{400,}?)"\s*\*{3}/g,
    " [long quoted reference omitted for policy scan] ***",
  );
  return omitCuedLongQuotes(
    omitCuedLongQuotes(explicitlyDelimited, /"([\s\S]{400,}?)"/g),
    /“([\s\S]{400,}?)”/g,
  );
}

export function hasRestrictionLanguage(text: string): boolean {
  return /\b(?:must|must not|never|do not|don't|should not|shouldn't|cannot|can't|without changing|only|no longer|avoid|prohibit|forbid|required|restriction|correction|instead|not that|wrong)\b/i.test(
    text,
  );
}

export function hasUnresolvedLanguage(text: string): boolean {
  return /\b(?:unresolved|uncertain|unknown|still failing|still broken|not fixed|not yet|todo|tbd|open question|remaining|needs investigation|follow[- ]?up)\b/i.test(
    text,
  );
}

export function hasFailureLanguage(text: string): boolean {
  return /\b(?:error|failed|failure|exception|panic|fatal|timeout|assertion|expected\b.*\breceived|exit code\s*[1-9]|segfault|traceback)\b/i.test(
    text,
  );
}

export function hasSuccessLanguage(text: string): boolean {
...
export function orderedIncludes(haystack: readonly string[], needles: readonly string[]): boolean {
  let at = 0;
...
export function formatSourceRef(entryId: string, blockIndex?: number): string {
