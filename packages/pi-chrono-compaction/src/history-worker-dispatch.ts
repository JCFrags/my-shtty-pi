...
function finite(value: unknown): boolean { return typeof value === "number" && Number.isFinite(value) && value >= 0 && value <= Number.MAX_SAFE_INTEGER; }
function parseResponse(wire: string): HistoryWorkerResponse {
  if (typeof wire !== "string" || wire.length > HISTORY_WORKER_CAPS.responseBytes || Buffer.byteLength(wire) > HISTORY_WORKER_CAPS.responseBytes) return historyRefusal("history-output-limit");
  let value: unknown;
...
export async function dispatchHistoryWorker(
  path: string | undefined, operation: HistoryOperation, transport?: HistoryWorkerTransport, signal?: AbortSignal,
): Promise<HistoryWorkerResponse> {
  if (!path) return historyRefusal("history-source-unpersisted");
  ...
  if (!reservation) return historyRefusal("history-load-memory-limit");
  try {
    if (signal?.aborted) return historyRefusal("history-worker-aborted");
    if (!boundedHistoryValue(operation) || typeof path !== "string" || path.length > 4096 || !path.startsWith("/")) return historyRefusal("history-request-limit");
    const stat = await lstat(path);
    if (!stat.isFile()) return historyRefusal("history-source-unsafe-type");
    const refusal = historySourceAdmission(stat.size, operation);
    if (refusal) return historyRefusal(refusal);
    if (!transport || transport.isolation !== "os-bounded-child-v1") return historyRefusal("history-worker-unavailable");
    const request: HistoryWorkerRequest = { version: 1, path, source: { deviceId: String(stat.dev), inodeId: String(stat.ino), size: stat.size, mtimeMs: stat.mtimeMs }, operation };
    const wire = JSON.stringify(request);
    if (Buffer.byteLength(wire) > HISTORY_WORKER_CAPS.requestBytes) return historyRefusal("history-request-limit");
    dispatched++;
...
export function historyWorkerToolResult(response: HistoryWorkerResponse, exact = false) {
  if (exact && response.status === "refused" && response.code === "legacy-history-size-limit") return {
    content: [{ type: "text" as const, text: "History unavailable: this session exceeds the legacy-load limit; exact retrieval requires an existing verified source ledger." }],
...
export function validateHistoryWorkerWire(wire: unknown): string | undefined {
  if (typeof wire !== "string" || wire.length > HISTORY_WORKER_CAPS.responseBytes || Buffer.byteLength(wire) > HISTORY_WORKER_CAPS.responseBytes) throw new Error("history-output-limit");
  let value: unknown;
  try { value = JSON.parse(wire); } catch { throw new Error("history-response-invalid"); }
  if (plain(value) && value.status === "stage") {
    if (Object.keys(value).length !== 2 || !HISTORY_WORKER_STAGES.includes(value.stage)) throw new Error("history-response-invalid");
    return undefined;
  }
  const response = parseResponse(wire);
  if (JSON.stringify(response) !== JSON.stringify(value)) throw new Error("history-response-invalid");
  return wire;
}
...
export function createHistoryFeedbackAdmission() {
  ...
  return {
    ...
    release(): void { reservation?.release(); reservation = undefined; },
  };
}
