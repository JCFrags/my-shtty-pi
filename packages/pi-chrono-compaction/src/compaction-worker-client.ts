...
import { acquireHostWorkerSlot, defaultSchedulerDirectory, type SchedulerLease, type WorkerPriority } from "./host-worker-scheduler.js";
import { MAX_WORKER_REQUEST_BYTES, MAX_WORKER_RESPONSE_BYTES, MAX_WORKER_STDERR_BYTES, validateWorkerRequest, validateWorkerResponse, validateWorkerProgress, type CompactionWorkerRequest, type CompactionWorkerResponse, type WorkerFailureCode } from "./compaction-worker-protocol.js";
...
async function runSingle(request: CompactionWorkerRequest, options: WorkerClientOptions, progress: (stage: string) => void = () => {}): Promise<WorkerClientResult> {
  ...
  const failure = (code: WorkerFailureCode) => {
    const result = emptyMetrics(request, options.slots ?? 1, safeFailure(request, code, "scheduler-wait"));
    try { writePrivateDiagnostic(diagnosticPath, result.response, performance.now() - started, "scheduler-wait", entry, requestBytes, 0, Buffer.alloc(0), 0); } catch {}
    return result;
  };
  if (request.expectedSource.size > WORKER_LIMITS.sourceBytes) return failure("worker-source-limit");
  let lease: SchedulerLease | undefined;
  let child: ContainedWorker | undefined;
  let stageWrites = Promise.resolve();
  const stage = (label: string) => { if (!lease) return; const slot = lease.slot; stageWrites = stageWrites.then(() => publishRuntimeStage(directory, slot, runtimeCategory(request.jobType), label)).catch(() => {}); progress(label); };
  try {
    ...
    if (!statSync(entry).isFile()) return failure("worker-entrypoint-unavailable");
    for (;;) {
      if (options.signal?.aborted) return failure("worker-aborted");
      const remaining = Math.min(options.workerTimeoutMs ?? 900_000, request.deadlineMs - Date.now());
      if (remaining <= 0) return failure("worker-timeout");
      try { const start = () => startContainedWorker(directory, lease!.slot, lease!.slots, remaining, undefined, MAX_WORKER_RESPONSE_BYTES, undefined, options.signal); child = options.schedulerDirectory === undefined ? await withVerifiedLegacyAdmission(directory, start) : await start(); break; }
      catch (error) {
        if ((error as Error).message !== "runtime-slot-busy") throw error;
    ...
    let childExitCode: number | null | undefined, childSignal: NodeJS.Signals | null | undefined;
    const probe = setInterval(() => { const now = performance.now(); maxDelay = Math.max(maxDelay, now - expected); expected = now + 10; }, 10);
    let abort = () => {};
    let timer: ReturnType<typeof setTimeout> | undefined;
    try {
      const response = await new Promise<CompactionWorkerResponse>(resolve => {
        let settled = false;
        const finish = (value: CompactionWorkerResponse) => { if (!settled) { settled = true; resolve(value); } };
        ...
        running.stderr.on("data", (bytes: Buffer) => {
          stderrBytes += bytes.length; stderrTail = Buffer.concat([stderrTail, bytes]).subarray(-MAX_WORKER_STDERR_BYTES);
          if (stderrBytes > MAX_WORKER_STDERR_BYTES) finish(safeFailure(request, "worker-response-too-large", latestStage));
        });
        running.on("limit", (code: WorkerFailureCode) => finish(safeFailure(request, code, "response-validation")));
        running.on("message", value => {
          if (value && typeof value === "object" && value.kind === "worker-stage") { try { stage(validateWorkerProgress(value, request)); } catch { finish(safeFailure(request, "worker-protocol-error", "response-validation")); } return; }
        ...
        else running.send(request, entry, error => { if (error) finish(safeFailure(request, "worker-crashed", "child-start")); });
      });
      ...
      try { writePrivateDiagnostic(diagnosticPath, response, elapsedMs, latestStage, entry, requestBytes, responseBytes, stderrTail, stderrBytes, childExitCode, childSignal); } catch {}
    ...
    } finally { clearInterval(probe); if (timer) clearTimeout(timer); options.signal?.removeEventListener("abort", abort); }
  } catch (error) {
    const message = (error as Error).message;
    return failure(options.signal?.aborted ? "worker-aborted" : message === "scheduler-timeout" ? "scheduler-timeout" : message === "scheduler-queue-full" ? "scheduler-queue-full" : message === "scheduler-policy-mismatch" ? "scheduler-policy-mismatch" : (error as NodeJS.ErrnoException).code === "ENOENT" ? "worker-entrypoint-unavailable" : "worker-containment-unavailable");
  } finally {
...
export function workerJobIdentity(request: CompactionWorkerRequest, options: WorkerClientOptions = {}): string {
...
export async function runCompactionWorker(requestValue: unknown, options: WorkerClientOptions = {}): Promise<WorkerClientResult> {
  ...
  if (Buffer.byteLength(JSON.stringify(request)) > MAX_WORKER_REQUEST_BYTES) throw new Error("worker-protocol-error");
  const fail = (code: WorkerFailureCode) => emptyMetrics(request, options.slots ?? 1, safeFailure(request, code, "scheduler-wait"));
  if (options.signal?.aborted) return fail("worker-aborted");
  if (!options.schedulerDirectory && !await verifyLegacyAdmissionGate()) return fail("worker-legacy-transition-required");
  const directory = await prepareRuntimeNamespace(options.schedulerDirectory ?? defaultSchedulerDirectory());
