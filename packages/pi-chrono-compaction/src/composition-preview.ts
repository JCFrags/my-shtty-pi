...
export interface NormalCompositionAdapterResult {
  ...
  readonly firstKeptEntryId: string;
  readonly envelope: ShadowCompositionEnvelope & { readonly artifactRef?: string; readonly artifactStored?: boolean };
}
...
export async function composeStoredCompactionForNormalReturn(
  ...
  const parentId = first && getString((first as unknown as Record<string, unknown>).parentId);
  ...
  const result = composeStoredSelection({ regularPiSummary: input.regularPiSummary, combinedCeilingTokens,
    cut: { sourceCutEntryId: input.sourceCutEntryId, sourceCutSeq: selection.requestedCut,
      ...
      rawTailTokens: input.rawTailTokens, toolPairSafe: input.toolPairSafe } },
    selection, source => reader.recovery(selection.sourceView, source));
