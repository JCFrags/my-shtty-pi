...
export function validateSearchV3Response(value: unknown): SearchV3Response {
  if (!value || typeof value !== "object" || Array.isArray(value)) throw new Error("search-v3-response-invalid");
  ...
  if (response.v !== 1 || typeof response.ok !== "boolean" || !Number.isSafeInteger(response.sourceBytes) || Number(response.sourceBytes) < 0
    ...
    || Buffer.byteLength(JSON.stringify(response)) > SEARCH_V3_LIMITS.responseBytes) throw new Error("search-v3-response-invalid");
  if (response.ok) {
    if (!response.result || typeof response.result !== "object" || Array.isArray(response.result)
      || Object.keys(response).sort().join(",") !== "ok,result,sourceBytes,sqliteNativeLimitBytes,v") throw new Error("search-v3-response-invalid");
