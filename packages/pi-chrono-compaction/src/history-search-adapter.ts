...
export class HistorySearchAdapter {
  ...
  private rawCatalogScope(signal?: AbortSignal): { scope: CatalogHistoryScope; execute: CatalogHistoryExecutor } {
    const target = this.target, source = this.sourceTarget, key = this.key;
    if (!this.enabled || !source || !target || this.progress.catalog !== "ready") return fail("search-v3-index-not-ready");
    if (this.expectedLogicalCut && (target.view.storeKey !== this.expectedLogicalCut.catalogStoreKey
      ...
      || target.view.eventCut !== this.expectedLogicalCut.eventCut)) return fail("logical-session-route-scope-mismatch");
    const scope: CatalogHistoryScope = { catalogDirectory: target.catalogDirectory, sessionKey: target.view.sessionKey,
      shardKey: source.shardKey, view: { ...target.view, segments: target.view.segments.map(segment => ({ ...segment })) } };
    const valid = (): void => {
      if (signal?.aborted || !this.enabled || this.key !== key || this.target !== target) fail("search-v3-worker-aborted");
    };
    const execute: CatalogHistoryExecutor = async request => {
      valid();
      const response = await runCatalogWorker(request, { ...this.options, signal });
