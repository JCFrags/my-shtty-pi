...
export type WorkerFailureCode = "worker-legacy-transition-required" | "worker-containment-unavailable" | "worker-source-limit" | "scheduler-queue-full" | "scheduler-policy-mismatch" | "worker-disabled" | "no-session-file" | "branch-not-persisted" | "branch-parent-missing" | "branch-cycle" | "branch-source-order" | "invalid-cut" | "source-changed" | "scheduler-timeout" | "worker-timeout" | "worker-aborted" | "worker-crashed" | "worker-entrypoint-unavailable" | "worker-resource-limit" | "worker-internal-error" | "worker-protocol-error" | "worker-response-too-large" | "candidate-store-unavailable" | "replay-validation-rejected" | RollupShadowFailureCode;
export type WorkerJobType = "replay-compaction" | "candidate-store-update" | "rollup-shadow";
export interface WorkerSourceExpectation { readonly deviceId: string; readonly inodeId: string; readonly size: number; readonly mtimeMs: number; readonly prefixHash?: string; readonly prefixBytes?: number; }
...
function validConfig(value:unknown):value is CompactorConfig{if(!object(value)||!exactKeys(value,CONFIG_KEYS))return false;for(const key of CONFIG_KEYS){const item=value[key];if(["mergeEpisodes","enableSemanticCompression","includeHeader","emergencyAllowAbsent"].includes(key)){if(typeof item!=="boolean")return false;}else if(key==="minMarginalUtilityPerToken"){if(!inRange(item,WORKER_LIMITS.minMarginalUtilityPerToken,false))return false;}else if(["recentExactBiasFraction","mergeBeforeFraction"].includes(key)){if(typeof item!=="number"||!Number.isFinite(item)||item<0||item>1)return false;}else if(!integer(item,0,1_000_000))return false;}return true;}
...
export function validateWorkerProgress(value: unknown, request: CompactionWorkerRequest): string {
  if (!object(value) || !exactKeys(value, ["kind", "schemaVersion", "jobId", "jobType", "stage"]) || value.kind !== "worker-stage" || value.schemaVersion !== 1 || value.jobId !== request.jobId || value.jobType !== request.jobType || !["source-bind", "replay", "candidate-update", "response-validation"].includes(String(value.stage))) throw new Error("worker-protocol-error");
  return value.stage as string;
}
function validRetrievalFeedback(value: unknown): boolean {
...
function validDynamicContext(value: unknown): boolean {
...
export function validateWorkerRequest(value: unknown): CompactionWorkerRequest {
  ...
  if (!exactKeys(value, allowed) || !boundedId(value.jobId, 256) || !boundedId(value.sessionPath, 4096) || !validExpectation(value.expectedSource) || !integer(value.deadlineMs, Date.now() - 1, Date.now() + WORKER_LIMITS.timeoutSeconds.max * 1000) || !inRange(value.niceLevel, WORKER_LIMITS.nice)) throw new Error("worker-protocol-error");
  ...
  if (value.jobType==="replay-compaction") {
    ...
    if (value.retrievalFeedback!==undefined && !validRetrievalFeedback(value.retrievalFeedback)) throw new Error("worker-protocol-error");
    if (value.deterministicRebase!==undefined && (!object(value.deterministicRebase)||!exactKeys(value.deterministicRebase,["targetTokens","combinedTargetTokens","historicalCeilingTokens"])||!integer(value.deterministicRebase.targetTokens,256,16_000)||!integer(value.deterministicRebase.combinedTargetTokens,256,MAX_CONTEXT_TOKENS)||!integer(value.deterministicRebase.historicalCeilingTokens,256,MAX_CONTEXT_TOKENS))) throw new Error("worker-protocol-error");
...
export function validateWorkerResponse(value: unknown, expectedJobId?: string, hardOutputTokens = MAX_CONTEXT_TOKENS): CompactionWorkerResponse {
  if (
    !object(value) ||
