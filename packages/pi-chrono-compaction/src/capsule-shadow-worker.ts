 ...
 */
export function createContainedCapsuleShadow(options: { slots?: number; schedulerDirectory?: string } = {}): CapsuleShadowScheduler {
  ...
  return new CapsuleShadowScheduler(async (target, signal) => {
    ...
    if (!response.ok) throw Object.assign(new Error("capsule-shadow-failed"), { code: response.code });
