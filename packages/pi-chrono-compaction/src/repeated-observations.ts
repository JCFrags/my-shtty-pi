...
function sourceBlock(unit: CandidateUnit, blocks: readonly HistoricalBlock[]): HistoricalBlock | undefined {
  if (unit.sourceRefs.length !== 1) return undefined;
...
function refText(ref: SourceRef): string {
...
function hasStructuredFailure(record: Readonly<Record<string, unknown>> | undefined): boolean {
  ...
  for (const key of ["cancelled", "canceled", "aborted"] as const) {
    if (record[key] === true) return true;
  }
  for (const key of ["status", "state", "reason"] as const) {
    const value = record[key];
    if (typeof value === "string" && /^(?:abort(?:ed)?|cancel(?:led|ed)?)$/i.test(value.trim())) return true;
  }
  return false;
}

function hasStructuredSuccess(record: Readonly<Record<string, unknown>> | undefined): boolean {
  if (!record) return false;
  for (const key of ["exitCode", "exit_code", "code"] as const) {
    if (numericCode(record[key]) === 0) return true;
  }
  return false;
}

function exactExitCodes(text: string): number[] {
  const pattern = /(?:^|\n)\s*(?:exit(?:ed)?[ _-]*(?:code|status)|exit_code)\s*[:=]\s*(-?\d+)\b/gim;
  return [...text.matchAll(pattern)].map((match) => Number(match[1]));
}

function hasExactNonzeroExitCode(text: string): boolean {
  return exactExitCodes(text).some((code) => code !== 0);
}

function hasExactZeroExitCode(text: string): boolean {
  return exactExitCodes(text).some((code) => code === 0);
}

function isTerminalObservation(block: HistoricalBlock): boolean {
  if (block.kind === "bash_execution") return true;
 ...
 */
export function isFailedObservation(block: HistoricalBlock): boolean {
  if (block.kind !== "tool_result" && block.kind !== "bash_execution") return false;
  ...
  if (isTerminalObservation(block)) return !hasPositiveExit;
  if (block.isError === false || hasPositiveExit) return false;
  return true;
}

function repeatCandidate(
  unit: CandidateUnit,
  canonical: CandidateUnit,
  block: HistoricalBlock,
): RepresentationCandidate | undefined {
  if (isFailedObservation(block)) return undefined;
...
function commonEdges(previous: readonly string[], current: readonly string[]): { prefix: number; suffix: number } {
  const maximum = Math.min(previous.length, current.length);
  let prefix = 0;
  while (prefix < maximum && previous[prefix] === current[prefix]) prefix += 1;
  let suffix = 0;
  while (
...
function deltaCandidate(
  unit: CandidateUnit,
  previousUnit: CandidateUnit,
  previousBlock: HistoricalBlock,
  block: HistoricalBlock,
): RepresentationCandidate | undefined {
  if (block.kind !== "tool_result" && block.kind !== "bash_execution") return undefined;
  if (previousBlock.kind !== block.kind || isFailedObservation(block) || isFailedObservation(previousBlock)) return undefined;
  if (block.unresolved || previousBlock.unresolved || !unit.resource || unit.resource.key !== previousUnit.resource?.key) return undefined;

  const previousLines = previousBlock.exactText.split("\n");
  const currentLines = block.exactText.split("\n");
  if (previousLines.length < MIN_DELTA_LINES || currentLines.length < MIN_DELTA_LINES) return undefined;
  const { prefix, suffix } = commonEdges(previousLines, currentLines);
  const stableLines = prefix + suffix;
  if (stableLines < MIN_DELTA_LINES || stableLines / Math.max(previousLines.length, currentLines.length) < 0.72) return undefined;

  const currentChanged = currentLines.slice(prefix, currentLines.length - suffix);
  if (currentChanged.length === 0) return undefined;
  const changedText = currentChanged.join("\n");
  const changedTokens = estimateTokensFromText(changedText);
  if (changedTokens > MAX_DELTA_TOKENS || changedTokens >= block.rawTokens * 0.28) return undefined;
  ...
  if (tokens >= block.rawTokens * 0.8) return undefined;
  return {
 ...
 */
export function addRepeatedObservationCandidates(
  ...
  const firstExact = new Map<string, CandidateUnit>();
  const latestResource = new Map<string, { unit: CandidateUnit; block: HistoricalBlock }>();
  return units.map((unit) => {
    ...
    const additions: RepresentationCandidate[] = [];
    ...
    if (canonical) {
      ...
      if (repeated) additions.push(repeated);
    } else firstExact.set(exactKey, unit);
    ...
    if (!canonical && unit.resource) {
      ...
      if (previous) {
        ...
        if (delta) additions.push(delta);
      }
    }
    if (unit.resource) latestResource.set(unit.resource.key, { unit: canonicalUnit, block });
    if (additions.length === 0) return canonicalUnit;
