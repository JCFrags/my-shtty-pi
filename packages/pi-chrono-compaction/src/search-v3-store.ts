...
interface IndexedWindow { cueBefore: number; rawBefore: number; rawLexical: boolean }
function indexedWindow(store: Store, request: Extract<SearchV3Request, { op: "query" }>): IndexedWindow {
  if (request.cursor) {
    const value = cursorDecode(request.cursor).indexed as IndexedWindow | undefined;
  ...
  const before = (table: "cue_fts" | "raw_fts"): number => {
...
function query(request: Extract<SearchV3Request, { op: "query" }>, store: Store): Record<string, unknown> {
  ...
  let regexIsBounded = false, indexed: IndexedWindow | undefined, nextIndexed: IndexedWindow | undefined, postingsVisited = 0;
  if (match) {
    indexed = indexedWindow(store, request); nextIndexed = { ...indexed };
    const perIndex = mode === "ranked" ? Math.floor(maximum / 2) : maximum;
    const window = (table: "cue_fts" | "raw_fts", expression: string, before: number): { rows: SqlRow[]; nextBefore: number; matched: number } => {
      if (!before) return { rows: [], nextBefore: 0, matched: 0 };
      ...
      const rows = store.rows(`WITH matches AS MATERIALIZED (SELECT rowid AS matchRowid,sourceKey${raw ? ",chunkIndex" : ""} FROM ${table}
        ...
        ORDER BY matches.matchRowid DESC`, perIndex + 1, expression, before, perIndex + 1,
        request.view.eventCut, pin.generation, ...bounds.values, ...filter.values, lineage);
