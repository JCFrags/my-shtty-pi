import type { CandidateUnit, HistoricalBlock, RepresentationCandidate } from "./types.js";
import { compactWhitespace, estimateTokensFromText, getNumber, getRecord, getString, hashText, unique } from "./utils.js";

export type ResourceKind = "file" | "url" | "command" | "process" | "service" | "setting" | "package" | "test" | "agent" | "evidence";
...
export interface ResourceObservation {
  ...
  readonly symbolHashes: Readonly<Record<string, string>>;
  readonly relation: "read" | "write" | "edit" | "diff" | "run" | "observe";
...
export interface ResourceVersion {
  ...
  readonly unionRanges: readonly ResourceRange[];
...
export interface ResourceLineageIndex {
  readonly resources: ReadonlyMap<string, ResourceLineage>;
  readonly observationByBlockId: ReadonlyMap<string, ResourceObservation>;
...
function toolPath(block: HistoricalBlock): string | undefined {
...
function declaredRevision(block: HistoricalBlock): string | undefined {
  const details = getRecord(block.attributes.details);
...
function requestedRange(block: HistoricalBlock, lines: number): ResourceRange | undefined {
  ...
  if (start === undefined && end === undefined && limit === undefined) return undefined;
...
function rollingChunkHashes(text: string, size = 8): string[] {
  const lines = text.replace(/\r\n?/g, "\n").split("\n");
  if (lines.length <= size) return [hashText(lines.map((line) => line.trimEnd()).join("\n"))];
  const hashes: string[] = [];
...
function symbolHashes(text: string): Readonly<Record<string, string>> {
...
function associatedCall(block: HistoricalBlock, byToolCallId: ReadonlyMap<string, HistoricalBlock[]>): HistoricalBlock | undefined {
...
function relationFor(name: string, text: string): ResourceObservation["relation"] {
  if (/diff|patch/i.test(name) || /^(?:diff --git |@@ )/m.test(text)) return "diff";
  if (/edit|replace|patch/i.test(name)) return "edit";
  if (/write|save|create/i.test(name)) return "write";
  if (/read|cat|head|tail|view|open/i.test(name)) return "read";
  if (/bash|shell|exec|run|test/i.test(name)) return "run";
  return "observe";
}

function resourceIdentity(call: HistoricalBlock, result: HistoricalBlock): { kind: ResourceKind; key: string; display: string } | undefined {
  const name = call.toolName ?? result.toolName ?? "unknown";
  const path = toolPath(call) ?? toolPath(result);
  if (path) {
    const kind: ResourceKind = /evidence|report|manifest|custody/i.test(path) ? "evidence" : "file";
    return { kind, key: `${kind}:${path}`, display: path };
  }
  const url = getString(call.toolArguments?.url);
  if (url) return { kind: "url", key: `url:${url}`, display: url };
  ...
  if (command) {
    ...
    const kind: ResourceKind = /(?:^|\s)(?:npm|pnpm|yarn)\s+(?:test|run test)|\bnode --test\b/.test(normalized)
      ? "test"
  ...
  if (/process|agent/i.test(name)) {
    const kind: ResourceKind = /agent/i.test(name) ? "agent" : "process";
...
function unionRanges(ranges: readonly ResourceRange[]): ResourceRange[] {
  const sorted = [...ranges].sort((a, b) => a.start - b.start || a.end - b.end);
  const output: ResourceRange[] = [];
  for (const range of sorted) {
    const previous = output[output.length - 1];
    if (!previous || range.start > previous.end + 1) output.push({ ...range });
    else output[output.length - 1] = { start: previous.start, end: Math.max(previous.end, range.end) };
  }
  return output;
}

interface InferredFileVersion {
  readonly generation: number;
  readonly versionHash: string;
  readonly lineHashes: Map<number, string>;
}

function observedLineHashes(text: string, range: ResourceRange | undefined): Map<number, string> {
  const lines = text.replace(/\r\n?/g, "\n").split("\n");
  const start = range?.start ?? 1;
  return new Map(lines.map((line, index) => [start + index, hashText(line.trimEnd())]));
}

function inferVersionHash(
  identity: { kind: ResourceKind; key: string },
  revision: string | undefined,
  relation: ResourceObservation["relation"],
  failed: boolean,
  text: string,
  range: ResourceRange | undefined,
  inferred: Map<string, InferredFileVersion>,
): string {
  if (revision) return hashText(`${identity.key}\n${revision}`);
  if (identity.kind !== "file" && identity.kind !== "evidence") return hashText(text);
  if (failed) return hashText(`${identity.key}\nfailed\n${text}`);
  ...
  const merged = changed || previous === undefined ? new Map<number, string>() : new Map(previous.lineHashes);
  for (const [line, digest] of lines) merged.set(line, digest);
  inferred.set(identity.key, { generation, versionHash, lineHashes: merged });
  return versionHash;
}

export function buildResourceLineage(blocks: readonly HistoricalBlock[]): ResourceLineageIndex {
  const byToolCallId = new Map<string, HistoricalBlock[]>();
  ...
  const inferredVersions = new Map<string, InferredFileVersion>();
  for (const block of blocks) {
    if (block.kind !== "tool_result" && block.kind !== "bash_execution") continue;
  ...
  const resources = new Map<string, ResourceLineage>();
  for (const [key, group] of grouped) {
    const versionGroups = new Map<string, ResourceObservation[]>();
    ...
    const versions: ResourceVersion[] = ordered.map((version) => ({ ...version, superseded: version.versionHash !== current.versionHash }));
    resources.set(key, {
...
export function chunkSimilarity(left: ResourceObservation, right: ResourceObservation): number {
...
export interface FactoredTemplate {
...
export function factorNearDuplicateTemplate(texts: readonly string[]): FactoredTemplate | undefined {
  if (texts.length < 2 || texts.some((text) => text.length === 0)) return undefined;
  const shortest = Math.min(...texts.map((text) => text.length));
  let prefix = 0;
  while (prefix < shortest && texts.every((text) => text[prefix] === texts[0]?.[prefix])) prefix += 1;
  let suffix = 0;
  while (suffix < shortest - prefix && texts.every((text) => text[text.length - 1 - suffix] === texts[0]?.[texts[0]!.length - 1 - suffix])) suffix += 1;
  ...
  if (!reconstructed.every((text, index) => text === texts[index])) return undefined;
...
function factoringSignature(block: HistoricalBlock): string {
...
export function addNearDuplicateFactoringCandidates(
  ...
  for (const block of blocks) {
    if (block.protectedExact || block.unresolved || block.isError || !block.reproducible || block.exactText.length < 400) continue;
    if (block.kind !== "tool_result" && block.kind !== "bash_execution") continue;
    const unit = unitById.get(block.id);
    if (!unit?.candidates.some((candidate) => !candidate.lossy)) continue;
    const key = factoringSignature(block);
    const group = groups.get(key) ?? [];
    group.push(block);
    groups.set(key, group);
  }
  const candidateById = new Map<string, RepresentationCandidate>();
  for (const group of groups.values()) {
    ...
    group.forEach((block, index) => {
      ...
      if (tokens >= unit.rawTokens) return;
...
function ref(observation: ResourceObservation): string {
...
function laterObservations(lineage: ResourceLineage, observation: ResourceObservation): ResourceObservation[] {
...
function rangeCovered(range: ResourceRange, by: ResourceRange): boolean {
 ...
 */
export function applyResourceEvolutionCandidates(
  units: readonly CandidateUnit[],
  blocks: readonly HistoricalBlock[],
  index = buildResourceLineage(blocks),
): readonly CandidateUnit[] {
  const blockById = new Map(blocks.map((block) => [block.id, block]));
  return units.map((unit) => {
    const observation = index.observationByBlockId.get(unit.id);
    const block = blockById.get(unit.id);
    if (!observation || !block || observation.failed || observation.unresolved || observation.protectedExact) return unit;
    if (observation.resourceKind !== "file" && observation.resourceKind !== "evidence") return unit;
    const lineage = index.resources.get(observation.resourceKey);
    if (!lineage) return unit;
    const later = laterObservations(lineage, observation);
    if (later.length === 0) return unit;
    ...
    if (laterVersion) {
      ...
      const marker = markerCandidate(
        ...
        observation,
        text,
        "superseded-resource-version",
        Math.min(0.9, 0.74 + lineage.volatility * 0.12),
      );
      return {
        ...unit,
