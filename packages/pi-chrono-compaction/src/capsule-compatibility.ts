...
import type { CatalogResponse, CatalogView } from "./catalog-contract.js";
import type { CatalogStoreRequest } from "./catalog-store-contract.js";
...
import {
  ...
  type ScopedSourceRef,
} from "./capsule-contract.js";
import {
  candidateDependency,
  persistentCandidateKey,
  type CandidatePrecomputeRecord,
} from "./candidates.js";
import { REDUCER_VERSIONS } from "./reducers/index.js";
import type { CompactorConfig, HistoricalBlock, RepresentationCandidate } from "./types.js";
...
export type CapsuleCatalogExecutor = (
  request: CatalogStoreRequest,
  options?: { readonly signal?: AbortSignal },
) => Promise<CatalogResponse>;

export interface CapsuleBindingAssociation {
  readonly block: HistoricalBlock;
  readonly envelope: ReducerEnvelope;
}

export interface CreateVerifiedCapsuleBindingsInput {
  readonly catalogDirectory: string;
  readonly view: CapsuleCatalogView;
  readonly associations: readonly CapsuleBindingAssociation[];
...
/** Nominal result created only after live M04 pinned blocks/raw reads succeed. */
export interface M04VerifiedCapsuleBinding {
  readonly block: HistoricalBlock;
  readonly envelope: ReducerEnvelope;
  readonly view: CapsuleCatalogView;
...
export interface CapsuleBindingRejection {
  readonly blockId: string;
  readonly reason: string;
}

export interface CreateVerifiedCapsuleBindingsResult {
  readonly bindings: readonly M04VerifiedCapsuleBinding[];
  readonly rejected: readonly CapsuleBindingRejection[];
  /** Sum reported by the M04 executor for calls made by this bounded batch. */
  readonly sourceBytes: number;
  /** Unique exact raw ranges requested; checked before any catalog call. */
  readonly requestedRawBytes: number;
}

export interface CapsuleCompatibilityResult {
  readonly records: ReadonlyMap<string, CandidatePrecomputeRecord>;
...
function catalogView(view: CapsuleCatalogView): CatalogView {
  return {
...
function sourceShape(source: ScopedSourceRef): RecordValue {
...
function sourceKey(source: ScopedSourceRef): string {
  return stableStringify(sourceShape(source));
}

function collectSourceRefs(envelope: ReducerEnvelope): ScopedSourceRef[] {
  const refs: ScopedSourceRef[] = [envelope.source];
  if (envelope.pair) refs.push(envelope.pair.call, envelope.pair.result);
  for (const alternative of envelope.alternatives) {
    refs.push(...alternative.sourceRefs);
    for (const fact of alternative.facts) refs.push(fact.source);
    for (const cue of alternative.protectedCues) refs.push(cue.source);
    for (const omission of alternative.omissions) refs.push(omission.kind === "exact-range" ? omission.source : omission.affectedSource);
  }
  const unique = new Map<string, ScopedSourceRef>();
  for (const ref of refs) unique.set(sourceKey(ref), ref);
  return [...unique.values()];
}

function segmentForEvent(view: CapsuleCatalogView, eventSeq: number): number | undefined {
...
function expectedBlockKind(envelope: ReducerEnvelope, block: HistoricalBlock): boolean {
  return envelope.source.entryId === block.entryId
    && (block.blockIndex === undefined || envelope.source.blockIndex === block.blockIndex);
}

function bindingDigest(block: HistoricalBlock, envelope: ReducerEnvelope, view: CapsuleCatalogView, refs: readonly ScopedSourceRef[]): string {
  return createHash("sha256").update(stableStringify({ block, envelope, view, refs: refs.map(sourceShape) })).digest("hex");
}

function decodeBody(bytes: Buffer): string | undefined {
  try {
...
async function successfulResult(
  execute: CapsuleCatalogExecutor,
  request: CatalogStoreRequest,
...
async function verifyOneSource(
  source: ScopedSourceRef,
  input: CreateVerifiedCapsuleBindingsInput,
  execute: CapsuleCatalogExecutor,
  signal: AbortSignal | undefined,
  charge: (bytes: number) => void,
): Promise<{ readonly descriptor: RecordValue; readonly bodyText?: string }> {
  ...
  const base = { v: 1 as const, catalogDirectory: input.catalogDirectory, sessionKey: input.view.sessionKey };
  ...
  if (source.entryId !== undefined && metadata?.id !== source.entryId) throw new Error("catalog-entry-mismatch");
  ...
  if (!object(row) || row.index !== source.descriptor || !object(row.metadata)) throw new Error("catalog-descriptor-mismatch");
  ...
  if (length < 0 || length > CAPSULE_COMPATIBILITY_LIMITS.rawBytesPerSource) throw new Error("raw-range-too-large");
  ...
  if (bytes.length !== length || bytes.toString("base64") !== raw.data) throw new Error("catalog-raw-bytes-invalid");
  if (source.coordinateKind === "raw-json") {
    const digest = createHash("sha256").update(bytes).digest("hex");
    if (source.rawHashAlgorithm !== SEGMENT_CONTENT_HASH || digest !== source.rawHash) throw new Error("raw-hash-mismatch");
    return { descriptor };
  }
...
function pairMatchesBlock(binding: CapsuleBindingAssociation): boolean {
  ...
  if (typeof pairedEntryId !== "string") return envelope.pair === undefined;
  if (!envelope.pair || envelope.pair.call.entryId !== pairedEntryId) return false;
 ...
 */
export async function createVerifiedCapsuleBindings(
  input: CreateVerifiedCapsuleBindingsInput,
  ...
  const windowRefs = new Map<string, ScopedSourceRef>();
  for (const association of input.associations) {
    for (const ref of collectSourceRefs(association.envelope)) windowRefs.set(sourceKey(ref), ref);
  }
  if (windowRefs.size > CAPSULE_COMPATIBILITY_LIMITS.sourceRefs) throw new Error("capsule-source-ref-window-limit");
  ...
  const charge = (bytes: number): void => {
    sourceBytes += bytes;
    if (sourceBytes > maximum) { exhausted = true; throw new Error("total-source-read-limit"); }
  };
  const verifiedWindow = new Map<string, Promise<{ readonly descriptor: RecordValue; readonly bodyText?: string }>>();
  ...
  for (const association of input.associations) {
    ...
    try {
      if (options.signal?.aborted) throw options.signal.reason instanceof Error ? options.signal.reason : new Error("binding-aborted");
      if (!isReducerEnvelope(association.envelope) || !expectedBlockKind(association.envelope, association.block)
        ...
        || ["data"].includes(association.envelope.source.field)) throw new Error("unsupported-envelope");
      const refs = collectSourceRefs(association.envelope);
      if (refs.length > CAPSULE_COMPATIBILITY_LIMITS.sourceRefs) throw new Error("source-ref-limit");
      let mainText: string | undefined;
      const verifiedDescriptors = new Map<string, RecordValue>();
      for (const ref of refs) {
        ...
        if (sourceKey(ref) === sourceKey(association.envelope.source)) {
          ...
          if (verified.descriptor.provenance !== association.envelope.provenance) throw new Error("provenance-mismatch");
        }
      }
      if (mainText === undefined || mainText !== association.block.exactText) throw new Error("block-body-mismatch");
      ...
      if (association.envelope.pair) {
        ...
        if (callDescriptor?.kind !== "block" || callDescriptor.type !== "toolCall" || callDescriptor.id !== association.block.toolCallId
          || callDescriptor.name !== association.block.toolName) throw new Error("paired-call-mismatch");
      }
      ...
      bindings.push(Object.freeze({ block: association.block, envelope: association.envelope, view: input.view,
        sourceRefs: frozenRefs, [VERIFIED]: true as const,
...
const reducerMapping: Readonly<Record<string, { reducer: string; version: string }>> = Object.freeze({
...
function candidateShape(block: HistoricalBlock, reducer: string): { readonly level: RepresentationCandidate["level"]; readonly utility: number } | undefined {
  if (reducer === "assistant-extractive" || reducer === "assistant-cleanup") {
    if (!["assistant_reasoning", "assistant_text", "branch_summary", "custom_message", "user"].includes(block.kind)) return undefined;
...
function recordIntegrityHash(blockId: string, key: string, candidates: readonly RepresentationCandidate[], block: HistoricalBlock): string {
...
export function adaptVerifiedCapsulesToPrecomputedCandidates(
  blocks: readonly HistoricalBlock[],
  config: CompactorConfig,
  bindings: readonly M04VerifiedCapsuleBinding[],
  ...
  const byBlock = new Map<string, RepresentationCandidate[]>();
  const rejected: CapsuleBindingRejection[] = [];
  let accepted = 0;
  for (const binding of bindings) {
    ...
    try {
      if (binding[VERIFIED] !== true || block !== binding.block || binding.block.protectedExact
        ...
        || !isReducerEnvelope(binding.envelope)) throw new Error("unverified-binding");
      const mapping = reducerMapping[binding.envelope.family];
      const shape = mapping && candidateShape(block, mapping.reducer);
      if (!mapping || !shape || binding.envelope.familyVersion !== mapping.version) throw new Error("unsupported-reducer");
      const candidates = byBlock.get(block.id) ?? [];
      if (binding.envelope.alternatives.length !== 1) throw new Error("unsupported-alternative-count");
      const alternative = binding.envelope.alternatives[0];
      if (!alternative || alternative.lossy !== true || alternative.omissions.length < 1
        || alternative.protectedCues.some(cue => !alternative.text.includes(cue.exactText))) throw new Error("loss-or-cue-mismatch");
      if (candidates.some(candidate => candidate.reducer === mapping.reducer)) throw new Error("duplicate-reducer");
      ...
      if (alternative.text.length > CAPSULE_COMPATIBILITY_LIMITS.candidateTextUnits || combined > CAPSULE_COMPATIBILITY_LIMITS.candidateTextUnits
        || candidates.length >= CAPSULE_COMPATIBILITY_LIMITS.candidatesPerBlock) throw new Error("candidate-ceiling");
      const candidate: RepresentationCandidate = Object.freeze({
  ...
  const records = new Map<string, CandidatePrecomputeRecord>();
