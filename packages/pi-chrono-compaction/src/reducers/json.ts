import type { JsonValue, OmissionNotice } from "../types.js";
...
export function parseStructuredJson(text: string): JsonValue | undefined {
...
interface CompactJsonStats {
  arraysReduced: number;
...
function recordIsImportant(record: { [key: string]: JsonValue }): boolean {
  return Object.entries(record).some(([key, value]) => {
    if (/error|fail|warn|outlier|invalid/i.test(key)) return true;
    if (typeof value === "string" && /error|fail|warn|timeout|invalid|denied/i.test(value)) return true;
    return false;
  });
}

function compactJson(value: JsonValue, depth: number, stats: CompactJsonStats): JsonValue {
  if (depth > 6) return "[nested value omitted]";
  if (Array.isArray(value)) {
    if (value.length <= 8) return value.map((item) => compactJson(item, depth + 1, stats));
    const important = value.filter((item) => isJsonRecord(item) && recordIsImportant(item)).slice(0, 4);
    const samples = [value[0], value[1], value[value.length - 2], value[value.length - 1]].filter(
      (item): item is JsonValue => item !== undefined,
    );
    const selected: JsonValue[] = [];
    for (const item of [...important, ...samples]) {
      if (!selected.some((existing) => stableStringify(existing) === stableStringify(item))) selected.push(item);
    }
  ...
  if (isJsonRecord(value)) {
    ...
    if (Object.keys(result).length === 0 && entries.length > 0) {
      const [key, child] = entries[0] as [string, JsonValue];
      result[key] = compactJson(child, depth + 1, stats);
    }
    if (stats.fieldsOmitted > 0 && depth === 0) result.__omittedFields__ = stats.fieldsOmitted;
    return result;
  }
  if (typeof value === "string" && value.length > 2_000) return `${value.slice(0, 1_500)}…[string truncated]`;
  return value;
}

function renderPlainJson(value: JsonValue, path = "root"): string[] {
  if (Array.isArray(value)) {
...
export function reduceStructuredJson(context: ReducerContext, parsed = parseStructuredJson(context.block.exactText)): ReducerResult | undefined {
  if (parsed === undefined) return undefined;
  const stats: CompactJsonStats = { arraysReduced: 0, recordsOmitted: 0, fieldsOmitted: 0 };
  ...
  if (stats.fieldsOmitted > 0) omissions.push({ description: `${stats.fieldsOmitted} low-priority structured field(s) omitted` });
  return {
    text,
