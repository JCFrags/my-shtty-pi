...
function parseMatch(raw: string): SearchMatch | undefined {
  ...
  if (pathOnly?.[1] && pathOnly[2]) return { file: pathOnly[1], text: pathOnly[2], raw };
  return undefined;
}
...
export function reduceSearchOutput(context: ReducerContext): ReducerResult {
  ...
  const matches = lines.map(parseMatch).filter((match): match is SearchMatch => match !== undefined);
  ...
  if (truncated !== undefined) sections.push(`Original result exhaustive: ${String(!truncated)}`);
  sections.push(
