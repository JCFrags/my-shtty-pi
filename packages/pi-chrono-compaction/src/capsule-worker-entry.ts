...
import { CAPSULE_LIMITS, isCapsuleWorkerRequest, type CapsuleWorkerResponse } from "./capsule-contract.js";
...
process.once("message", async (request: unknown) => {
  ...
  try {
    ...
    if (Buffer.byteLength(JSON.stringify(response)) > CAPSULE_LIMITS.responseBytes) response = failure("capsule-response-limit", response.sourceBytes);
  } catch { response = failure("capsule-worker-failed"); }
