...
import { CAPSULE_LIMITS, isCapsuleWorkerRequest, type CapsuleWorkerRequest, type CapsuleWorkerResponse } from "./capsule-contract.js";
...
export function validateCapsuleResponse(value: unknown): CapsuleWorkerResponse {
  if (!value || typeof value !== "object" || Array.isArray(value)) throw new Error("capsule-response-invalid");
  ...
  if (response.v !== 1 || typeof response.ok !== "boolean"
    ...
    || Buffer.byteLength(JSON.stringify(response)) > CAPSULE_WORKER_CAPS.responseBytes) throw new Error("capsule-response-invalid");
  if (response.ok) {
    if (!response.result || typeof response.result !== "object" || Array.isArray(response.result)
      || Object.keys(response).sort().join(",") !== "ok,result,sourceBytes,sqliteNativeLimitBytes,v") throw new Error("capsule-response-invalid");
  } else if (typeof response.code !== "string" || !/^capsule-[a-z0-9-]{1,80}$/.test(response.code)
    || typeof response.resumable !== "boolean"
    || Object.keys(response).sort().join(",") !== "code,ok,resumable,sourceBytes,sqliteNativeLimitBytes,v") throw new Error("capsule-response-invalid");
  return value as CapsuleWorkerResponse;
}

export async function runCapsuleWorker(request: CapsuleWorkerRequest, options: {
  signal?: AbortSignal; slots?: number; schedulerDirectory?: string;
} = {}): Promise<CapsuleWorkerResponse> {
  try {
    const validateRequest = (value: unknown): CapsuleWorkerRequest => {
      if (!isCapsuleWorkerRequest(value) || Buffer.byteLength(JSON.stringify(value)) > CAPSULE_WORKER_CAPS.requestBytes) throw new Error("capsule-request-invalid");
      return value;
    };
    ...
    const { value } = await runBoundedWorker<CapsuleWorkerRequest, CapsuleWorkerResponse>({
