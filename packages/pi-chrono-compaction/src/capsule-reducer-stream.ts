...
function span(start: number, text: string): { readonly decodedUtf16: CoordinateRange; readonly text: string } {
...
export function beginCapsuleReduction(
  base: SourceBlockReducerBaseInput,
  options: CapsuleReducerOptions,
): CapsuleReducerStreamState {
  const emptyWindow = {
    ...base,
    window: {
...
export function feedCapsuleReduction(
  state: CapsuleReducerStreamState,
...
function complementOmissions(state: CapsuleReducerStreamState, selected: readonly CoordinateRange[]): CapsuleOmission[] {
...
export function finalizeCapsuleReduction(state: CapsuleReducerStreamState): ReducerEnvelope {
  ...
  const selected: CapsuleSelectedInput = {
    text: parts.join(""),
