...
async function materialize(request: Extract<EpisodeStateRequest, { op: "materializeRollup" }>, store: Store,
  ...
  for (let count = 0; count < maximum; count++) {
    ...
    stateGeneration = page.stateGeneration; knownThroughCut = page.knownThroughCut;
    if (!page.episode) { exhausted = true; break; }
  ...
  store.transaction(() => {
    ...
    if (root) store.run("INSERT INTO publications VALUES(?,?,?,?,?,?,?)", generation, line, request.view.branchKey, knownThroughCut,
...
function status(request: Extract<EpisodeStateRequest, { op: "rollupStatus" }>, store: Store): Record<string, unknown> {
  ...
  if (!head || !head.rootNodeId) return { readiness: "missing", rollupGeneration: num(store.get("SELECT generation FROM meta WHERE singleton=1")!, "generation"),
  ...
  if (!publication) fail("search-v3-rollup-publication-missing");
