export interface TriggerDecisionInput {
  ...
  readonly minimumGrowthTokens: number;
  readonly lastAttemptTokens?: number;
...
export function decideCompactionTrigger(input: TriggerDecisionInput): TriggerDecision {
  ...
  if (input.pending) return { trigger: false, reason: "a compaction request is already pending" };
