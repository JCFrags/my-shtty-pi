...
const n = (r: SqlRow, k: string): number => Number(r[k]);
const text = (r: SqlRow, k: string): string => String(r[k]);
const json = <T>(r: SqlRow, k: string): T => JSON.parse(text(r, k)) as T;
const schema = [
...
function prepareDirectory(directory: string): void {
  if (resolve(directory) !== directory) fail("catalog-storage-unsafe");
  const parts = directory.split("/").filter(Boolean);
  if (parts.length > 64) fail("catalog-storage-unsafe");
  const missing: string[] = [];
  let path = "";
  const check = (path: string, final = false): void => {
    const st = lstatSync(path);
    if (!st.isDirectory() || st.isSymbolicLink() || (st.uid !== 0 && st.uid !== process.getuid!()) ||
      ((st.mode & 0o022) !== 0 && !(st.uid === 0 && (st.mode & 0o1000) !== 0)) ||
      (final && (st.uid !== process.getuid!() || (st.mode & 0o777) !== 0o700))) fail("catalog-storage-unsafe");
  };
  try {
    for (const component of parts) {
      path += `/${component}`;
      if (missing.length) { missing.push(path); continue; }
      try { check(path, path === directory); }
      catch (error) { if ((error as NodeJS.ErrnoException).code !== "ENOENT") throw error; missing.push(path); }
    }
    if (missing.length > 8) fail("catalog-storage-unsafe");
  ...
  } catch { fail("catalog-storage-unsafe"); }
}
/** One instance per contained job. No source content or lifetime identity maps. */
class Engine {
  bytes = 0; statements = 0; readonly started = Date.now();
  constructor(readonly db: CatalogSqlite, readonly request: CatalogRequest) {}
  get(sql: string, ...v: SqlValue[]): SqlRow | undefined { this.statements++; return this.db.prepare(sql).get(...v); }
  run(sql: string, ...v: SqlValue[]): void { this.statements++; this.db.prepare(sql).run(...v); }
  rows(sql: string, limit: number, ...v: SqlValue[]): SqlRow[] { this.statements++; return [...this.db.prepare(sql).iterate(limit, ...v)]; }
  transaction<T>(fn: () => T): T {
    let domainError: unknown;
    try {
      return this.db.transaction(() => {
        try { return fn(); }
        catch (error) { if (/^catalog-[a-z0-9-]+$/.test(String((error as { code?: string })?.code))) domainError = error; throw error; }
      });
  ...
  generation(requested?: number): number {
    ...
    if (!this.get("SELECT g FROM generations WHERE g=?", g)) fail("catalog-generation-missing");
    return g;
  }
  shard(g: number, shard: string): SqlRow { return this.get("SELECT * FROM shards WHERE g=? AND shard=?", g, shard) ?? fail("catalog-shard-missing"); }
  checkpoint(s: SqlRow): CatalogParserState {
    const checkpoint = text(s, "checkpoint");
    if (Buffer.byteLength(checkpoint) > L.checkpointBytes || hash(checkpoint) !== text(s, "checkpointHash")) fail("catalog-checkpoint-corrupt");
    const state = JSON.parse(checkpoint) as CatalogParserState;
    if (state.version !== 1 || !Number.isSafeInteger(state.byteOffset) || state.byteOffset < 0 || (state.error && !/^catalog-[a-z0-9-]+$/.test(state.error.code))) fail("catalog-checkpoint-corrupt");
    return state;
  }
  event(g: number, seq: number): SqlRow { return this.get("SELECT * FROM events WHERE g=? AND seq=?", g, seq) ?? fail("catalog-event-missing"); }
  resolve(g: number, shard: string, id: string): SqlRow {
    const rows = this.rows("SELECT * FROM events WHERE g=? AND shard=? AND id=? LIMIT 2", 2, g, shard, JSON.stringify(id));
    if (rows.length !== 1) fail(rows.length ? "catalog-reference-ambiguous" : "catalog-parent-missing");
    return rows[0]!;
  }
  source<T>(s: SqlRow, fn: (source: CatalogSource) => T): T {
    const source = new CatalogSource(text(s, "path"));
    try {
      if (source.size < n(s, "observed")) fail("catalog-source-changed");
      source.verify(json<CatalogSourceSnapshot>(s, "snapshot")); return fn(source);
    }
    finally { this.bytes += source.bytesRead; source.close(); }
  }
  saveRecord(g: number, s: SqlRow, record: CatalogRecordMetadata): void {
    const shard = text(s, "shard");
    let parent: SqlRow | undefined;
    if (record.parentId !== undefined && record.parentId !== null) {
      ...
      if (local.length > 1) fail("catalog-reference-ambiguous");
      parent = local[0];
      if (!parent) {
        const declaration = json<{ shardKey?: string; eventId?: string }>(s, "parent");
        if (!declaration.shardKey || declaration.eventId !== record.parentId) fail("catalog-parent-missing");
        parent = this.resolve(g, declaration.shardKey!, record.parentId);
      }
    }
    let toolCallSource: Record<string, unknown> | undefined;
    if (record.role === "toolResult" && record.toolName === undefined) {
      if (record.toolCallId === undefined || !parent) fail("catalog-tool-call-missing");
      const scope = this.pin(g, text(parent!, "branch"), n(parent!, "seq"));
      let matched: SqlRow | undefined;
      for (const segment of scope.segments) {
        ...
        if (calls.length > 1 || (matched && calls.length)) fail("catalog-reference-ambiguous");
        if (calls.length) matched = calls[0];
      }
      if (!matched) fail("catalog-tool-call-missing");
      if (matched) {
        record.toolName = json<string>(matched, "name");
        ...
        if (text(matched, "provenance") === "generated") {
          ...
          for (const block of record.blocks) block.provenance = "generated";
        }
      }
    }
    ...
    if (parent && text(parent, "branch") === text(s, "branch")) {
      ...
      if (n(previous, "lastSeq") === n(parent, "seq")) segment = n(parent, "segment");
    }
    if (segment === seq) this.run("INSERT INTO segments VALUES(?,?,?,?)", g, segment, parent ? n(parent, "seq") : null, seq);
    else this.run("UPDATE segments SET lastSeq=? WHERE g=? AND segment=?", seq, g, segment);
    ...
    let idx = 0;
    for (const body of bodies) this.run("INSERT INTO blocks VALUES(?,?,?,?)", g, seq, idx++, JSON.stringify({ kind: "body", provenance: record.provenance, ...body }));
    for (const block of blocks) {
      const { bodies: blockBodies, ...header } = block;
      if (record.role === "assistant" && block.type === "toolCall" && block.id !== undefined && block.name !== undefined) this.run("INSERT INTO tool_calls VALUES(?,?,?,?,?,?,?,?,?)", g, segment, JSON.stringify(block.id), seq, shard, ordinal, block.index, JSON.stringify(block.name), block.provenance);
      this.run("INSERT INTO blocks VALUES(?,?,?,?)", g, seq, idx++, JSON.stringify({ kind: "block", ...header }));
      for (const body of blockBodies) this.run("INSERT INTO blocks VALUES(?,?,?,?)", g, seq, idx++, JSON.stringify({ kind: "body", blockIndex: block.index, provenance: block.provenance, ...body }));
    }
    s.count = ordinal; s.committed = record.endByte;
  }
  ingest(r: Extract<CatalogRequest, { op: "ingestStep" }>): Record<string, unknown> {
    return this.transaction(() => {
      ...
      if (text(generation, "state") === "retired") fail("catalog-generation-retired");
      ...
      if (!s) {
        ...
        if (latest && (r.shardOrdinal <= n(latest, "ordinal") || n(latest, "caught") !== 1)) fail("catalog-shard-order");
        if (n(this.get("SELECT COUNT(*) AS count FROM shards WHERE g=?", g)!, "count") >= 1024) fail("catalog-shard-limit");
        const source = new CatalogSource(r.sourcePath);
        let snapshot: CatalogSourceSnapshot;
        try { snapshot = source.snapshot(0); } finally { this.bytes += source.bytesRead; source.close(); }
      ...
      if (text(s, "path") !== r.sourcePath || text(s, "branch") !== r.branchKey || n(s, "ordinal") !== r.shardOrdinal || text(s, "parent") !== parentJson) fail("catalog-source-declaration-mismatch");
      const shard = s;
      return this.source(shard, source => {
        const state = this.checkpoint(shard);
        if (source.size > state.byteOffset && this.get("SELECT ordinal FROM shards WHERE g=? AND ordinal>? LIMIT 1", g, r.shardOrdinal)) fail("catalog-shard-order");
        if (state.error) return { generation: g, records: 0, offset: state.byteOffset, committed: n(shard, "committed"), error: state.error.code, caughtUp: false };
  ...
  pin(g: number, branch: string, seq: number): CatalogView {
    const leaf = this.event(g, seq);
    if (text(leaf, "branch") !== branch) fail("catalog-branch-mismatch");
    const segments: CatalogView["segments"] = [];
    let event: SqlRow | undefined = leaf;
    while (event) {
      if (segments.length === L.ancestry) fail("catalog-ancestry-limit");
  ...
  validateView(view: CatalogView): void {
    // Reconstruct only bounded indexed segments, never the complete event chain.
    if (view.sessionKey !== this.request.sessionKey || view.storeKey !== text(this.get("SELECT store FROM meta WHERE singleton=1")!, "store")) fail("catalog-view-invalid");
    const expected = this.pin(view.generation, view.branchKey, view.eventCut);
    if (expected.segments.length !== view.segments.length || expected.segments.some((s, i) => s.segment !== view.segments[i]!.segment || s.cut !== view.segments[i]!.cut)) fail("catalog-view-invalid");
  }
  selected(view: CatalogView, seq: number): SqlRow {
    this.validateView(view);
    const e = this.event(view.generation, seq);
    if (!view.segments.some(s => s.segment === n(e, "segment") && seq <= s.cut)) fail("catalog-event-outside-view");
    return e;
  }
  execute(): Record<string, unknown> {
    ...
    if (r.op === "rebuildStep") return this.transaction(() => {
      ...
      if (r.action === "start") {
        ...
        if (existing) return { generation: n(existing, "g") };
      ...
      if (n(counts, "count") !== r.expectedShards || n(counts, "caught") !== 1) fail("catalog-rebuild-incomplete");
      const state = text(this.get("SELECT state FROM generations WHERE g=?", g)!, "state");
      if (state === "retired") fail("catalog-generation-retired");
    ...
    if (r.op === "status") {
      const g = this.generation(r.generation);
      if (!r.shardKey) return { generation: g, storeKey: text(this.get("SELECT store FROM meta WHERE singleton=1")!, "store") };
    ...
    if (r.op === "page") {
      ...
      pageLoop: for (const segment of r.view.segments) {
        if (events.length >= (r.limit ?? L.page)) break;
        for (const row of this.rows("SELECT * FROM events WHERE g=? AND segment=? AND seq>? AND seq<=? ORDER BY seq LIMIT ?", r.limit ?? L.page, r.view.generation, segment.segment, r.after ?? 0, segment.cut, (r.limit ?? L.page) - events.length)) {
          ...
          if (pageBytes + size > 192 * 1024) break pageLoop;
          events.push(item); pageBytes += size;
        }
      }
    ...
    if (r.op === "blocks") {
      ...
      const blocks: { index: number; metadata: unknown }[] = [];
      let pageBytes = 0;
      for (const row of this.rows("SELECT idx,metadata FROM blocks WHERE g=? AND seq=? AND idx>=? ORDER BY idx LIMIT ?", r.limit ?? L.page, r.view.generation, r.eventSeq, r.after ?? 0, r.limit ?? L.page)) {
        ...
        const size = Buffer.byteLength(JSON.stringify(item));
        if (pageBytes + size > 192 * 1024) break;
        blocks.push(item); pageBytes += size;
      }
      return { blocks, after: blocks.length ? blocks.at(-1)!.index + 1 : r.after ?? 0 };
    }
    if (r.op === "raw") {
      const event = this.selected(r.view, r.eventSeq);
      if (r.offset < n(event, "rawStart") || r.length > n(event, "endByte") - r.offset || r.offset > n(event, "endByte")) fail("catalog-source-range");
      return this.source(this.shard(r.view.generation, text(event, "shard")), source => {
        ...
        while (at < r.offset + r.length) {
          ...
          if (at >= start + length) fail("catalog-span-missing");
          ...
          bytes.copy(output, at - r.offset, at - start, at - start + count); at += count;
          if (this.statements > L.statements) fail("catalog-job-limit");
        }
        source.assertCurrent();
    ...
    return this.source(s, source => {
      ...
      const size = json<CatalogSourceSnapshot>(s, "snapshot").size;
      if (after > size) fail("catalog-integrity-cursor");
      ...
      for (const span of spans) {
        if (n(span, "offset") !== after) fail("catalog-integrity-cursor");
        source.verifyRange({ offset: after, length: n(span, "length"), sha256: text(span, "hash") });
        after += n(span, "length"); verified++;
      }
      source.assertCurrent();
      if (!spans.length && after !== size) fail("catalog-integrity-cursor");
...
export function executeCatalogRequest(request: unknown, options?: CatalogOpenOptions): CatalogResponse {
  ...
  try {
    ...
    try { db.checkpoint(); } catch { /* retry maintenance on a later bounded job */ }
    return response;
  } catch (error) {
  ...
  } finally { try { db?.close(); } catch { /* original bounded result remains authoritative */ } }
}
