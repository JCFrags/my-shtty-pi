...
async function execute(request, store, options, executor, budget) {
    ...
    if (request.op === "chunkSourcePage") {
        ...
        while (sources.length < limit && scanned < maximum && visitedEvents < CAPSULE_LIMITS.deriveEvents) {
            ...
            if (afterDescriptor > 0 && event.seq !== afterEventSeq)
                fail("capsule-cursor-invalid");
            ...
            scanned++;
            afterEventSeq = event.seq;
            afterDescriptor = block.index + 1;
            const source = bodySource(request.identity, request.view, event, block);
            if (!source || store.get("SELECT state FROM markers WHERE eventSeq=? AND descriptor=? AND layer='chunks'", event.seq, block.index)?.state !== "ready")
                continue;
            const item = { source, provenance: provenance(block) };
            ...
            if (capsuleRow && string(capsuleRow, "source") === canonicalJson(source)) {
                ...
                if (canonicalJson(capsule) !== string(capsuleRow, "record") || canonicalJson(capsule.source) !== canonicalJson(source))
                    fail("capsule-content-corrupt");
                if (Buffer.byteLength(JSON.stringify({ sources: [...sources, { ...item, capsule }] })) <= CAPSULE_LIMITS.responseBytes - 4096)
