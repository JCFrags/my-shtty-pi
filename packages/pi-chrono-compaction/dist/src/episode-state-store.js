...
async function selectionContext(request, selection, options, budget) {
    ...
    const result = { ...selection, protected: [], current: [...selection.current],
    ...
    for (let index = 0; index < result.current.length; index++)
        result.current[index] = await enrich(result.current[index]);
    if (result.delta) {
    ...
    while (Buffer.byteLength(JSON.stringify(result)) > EPISODE_STATE_LIMITS.composeUtf8Bytes) {
        ...
        if (work >= 0) {
            result.protected.splice(work, 1);
