...
function insertReduced(store, request, reduced, generation) {
    ...
    if (episode)
        store.run("INSERT OR IGNORE INTO episode_membership VALUES(?,?,?,?,?,?,?)", line, str(episode, "episodeKey"), eventSeq, descriptor, sourceKey(reduced.source), canonicalJson(reduced.source), generation);
    for (const item of reduced.states) {
        if (item.explicitResolution) {
            const old = store.rows("SELECT stableKey,evidence FROM state_items WHERE lineage=? AND subject=? AND revision=? AND authority=? AND (status='unresolved' OR kind=?) AND supersededGeneration IS NULL AND createdGeneration<? ORDER BY eventSeq,descriptor,stableKey LIMIT ?", EPISODE_STATE_LIMITS.page, line, item.subject, item.revision, item.authority, item.kind, generation, EPISODE_STATE_LIMITS.page);
        ...
        store.run("INSERT OR IGNORE INTO state_items VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)", line, item.stableKey, item.subject, item.revision, item.kind, item.authority, item.confidence, item.status, canonicalJson(item.evidence), eventSeq, descriptor, generation, null, null);
    ...
    for (const item of reduced.resources) {
        ...
        store.run("INSERT OR IGNORE INTO resources VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", line, item.stableKey, item.resourceKind, item.resourceKey, item.relation, item.revision, item.revisionBasis, item.currentRevision, item.knownThrough, item.failed === null ? null : item.failed ? 1 : 0, canonicalJson(item.evidence), eventSeq, descriptor, generation, null);
...
export async function executeEpisodeStateRequest(value, options = {}) {
    ...
    const request = value, create = request.op === "materializeState";
    let db;
    ...
    try {
        ...
        const action = async () => {
            const path = join(request.searchDirectory, "state-v1.sqlite"), validate = (candidate) => new Store(candidate, request).validate(create);
            ...
            const response = { v: 1, ok: true, result, sourceBytes: budget.bytes, sqliteNativeLimitBytes: EPISODE_STATE_LIMITS.nativeSqliteBytes };
