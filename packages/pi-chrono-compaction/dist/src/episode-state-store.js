...
async function exactStructural(request, envelope, executor, budget) {
    ...
    if (length < 1 || length > 64 * 1024)
        return Object.keys(catalogFacts()).length ? catalogFacts() : undefined;
...
function insertReduced(store, request, reduced, generation, stateOnly = false) {
    ...
    for (const item of reduced.states) {
        ...
        if (store.get("SELECT stableKey FROM state_items WHERE lineage=? AND stableKey=?", line, item.stableKey))
            continue;
        if (item.transition) {
...
function boundedCategoryRows(store, line, active, condition, values, stateGeneration, processedCut, maximumPages = EPISODE_STATE_LIMITS.composeScanPages) {
    const rows = [];
    let cursor;
    for (let page = 0; page < maximumPages; page++) {
      ...
      ORDER BY eventSeq DESC,descriptor DESC,stableKey DESC LIMIT ?`, EPISODE_STATE_LIMITS.composeScanPage, line, stateGeneration, processedCut, stateGeneration, processedCut, ...values, ...(cursor ? [cursor.eventSeq, cursor.eventSeq, cursor.descriptor, cursor.descriptor, cursor.stableKey] : []), EPISODE_STATE_LIMITS.composeScanPage);
        rows.push(...batch);
        if (batch.length < EPISODE_STATE_LIMITS.composeScanPage)
            return { rows, complete: true, pages: page + 1 };
...
function composeStateSelection(request, store) {
    ...
    for (const condition of workConditions) {
        ...
        workRows.push(...scan.rows);
        workPages += scan.pages;
        if (!scan.complete) {
            workComplete = false;
            break;
        }
...
async function selectionContext(request, selection, options, budget) {
    ...
    const enrich = async (item) => {
        ...
        if (!cache.has(key)) {
            ...
            if (bytes.length !== (readEnd - readStart) * 2 || text.length !== readEnd - readStart)
                fail("search-v3-state-source-invalid");
        ...
        if (window.text.slice(clauseStart, clauseEnd) !== evidence.exactText)
            fail("search-v3-state-source-invalid");
        const before = [...window.text.slice(0, clauseStart).matchAll(/\n[ \t]*\n/gu)].at(-1);
        const after = /\n[ \t]*\n/u.exec(window.text.slice(clauseEnd));
        if (!before && readStart !== source.decodedUtf16.start || !after && readEnd !== source.decodedUtf16.end)
            return item;
        ...
        if (Buffer.byteLength(context) > 16 * 1024)
            return item;
    ...
    const result = { ...selection, protected: [], current: [],
        recent: [], older: [], omissions: { ...selection.omissions },
    ...
    const workPriority = (item) => item.authority === "user" ? 0
    ...
    for (const original of ordered) {
        ...
        const source = original.evidence.source;
        ...
        if (counts[category].size >= limit && sourceId && !selectedSources[category].has(sourceId)) {
            if (category === "restriction")
                result.omissions.restrictionWorkExhausted = result.omissions.protectedAtLeastOne = true;
            else
                result.omissions.openWorkExhausted = result.omissions.openWorkAtLeastOne = true;
            continue;
        }
    ...
    for (let index = 0; index < optionalCurrent.length; index++)
        optionalCurrent[index] = await enrich(optionalCurrent[index]);
    if (result.delta) {
        for (let index = 0; index < result.delta.protected.length; index++)
            result.delta.protected[index] = await enrich(result.delta.protected[index]);
        for (let index = 0; index < result.delta.current.length; index++)
            result.delta.current[index] = await enrich(result.delta.current[index]);
    }
    ...
    while (Buffer.byteLength(JSON.stringify(result)) > EPISODE_STATE_LIMITS.composeUtf8Bytes) {
        ...
        result.omissions.renderedOverflowAtLeastOne = true;
        const removableWork = result.protected.reduce((selected, item, index, all) => {
            if (item.kind === "restriction")
                return selected;
            if (selected < 0 || workPriority(item) > workPriority(all[selected]))
                return index;
            if (workPriority(item) === workPriority(all[selected])
                && Buffer.byteLength(JSON.stringify(item)) > Buffer.byteLength(JSON.stringify(all[selected])))
                return index;
            return selected;
        }, -1);
        if (removableWork >= 0) {
            result.protected.splice(removableWork, 1);
            result.omissions.openWorkAtLeastOne = true;
            continue;
        }
        if (result.protected.length) {
            result.protected.pop();
            result.omissions.protectedAtLeastOne = true;
            continue;
        }
        fail("search-v3-state-response-limit");
    }
    const refill = (candidates, target, omission) => {
        for (const candidate of candidates) {
            target.push(candidate);
            if (Buffer.byteLength(JSON.stringify(result)) <= EPISODE_STATE_LIMITS.composeUtf8Bytes)
                continue;
            target.pop();
            result.omissions[omission] = true;
            result.omissions.responseBudgetAtLeastOne = true;
            result.omissions.renderedOverflowAtLeastOne = true;
        }
    };
...
export async function executeEpisodeStateRequest(value, options = {}) {
    ...
    try {
        ...
        const action = async () => {
            const path = join(request.searchDirectory, "state-v4.sqlite"), validate = (candidate) => new Store(candidate, request).validate(create);
