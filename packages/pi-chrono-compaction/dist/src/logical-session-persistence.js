...
export function logicalBootstrapBytes(header, entries) {
    ...
    for (const value of entries.slice(0, MAX_BOOTSTRAP_ENTRIES)) {
        ...
        if (!(value.type === "custom" && value.customType === LOGICAL_CHECKPOINT_TYPE)
            && !["model_change", "thinking_level_change", "session_info"].includes(String(value.type)))
            return 0;
    }
    return 0;
}
async function verifyExistingSource(path, expected) {
    ...
    try {
        ...
        if (!info.isFile() || info.uid !== process.getuid?.() || info.nlink !== 1 || (info.mode & 0o777) !== 0o600
            ...
            fail("logical-session-source-persistence-invalid");
        const actual = Buffer.alloc(info.size + 1);
        const { bytesRead } = await file.read(actual, 0, actual.length, 0);
        if (bytesRead !== info.size || !actual.subarray(0, bytesRead).equals(expected))
            fail("logical-session-source-persistence-invalid");
    }
    finally {
        await file.close();
...
export async function persistNewShardBootstrap(manager, expectedParentSession, continuationEntryId, expectedSourceSessionId) {
    ...
    let parentId = null, continuationCount = 0;
    for (const value of entries) {
        ...
        if (value.type === "custom_message" && value.customType === "chrono-logical-continuation") {
            continuationCount += 1;
            if (value.id !== continuationEntryId)
                return fail("logical-session-source-persistence-invalid");
        }
    ...
    if (continuationCount !== 1 || parentId !== continuationEntryId)
        return fail("logical-session-source-persistence-invalid");
    const continuation = entries.at(-1);
    ...
    try {
        ...
        let reloaded;
        try {
            reloaded = Buffer.from([reloadedHeader, ...reloadedEntries].map(value => JSON.stringify(value)).join("\n") + "\n");
        }
        catch {
            return fail("logical-session-source-persistence-invalid");
        }
        if (manager.getSessionId() !== header.id || !reloaded.equals(serialized))
            return fail("logical-session-source-persistence-invalid");
    }
    finally {
        await unlink(temporary).catch(error => { if (error.code !== "ENOENT")
...
//# sourceMappingURL=logical-session-persistence.js.map