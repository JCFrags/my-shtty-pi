...
async function withLoadedSession(ctx, maximumBytes, use) {
    const loaded = await loadSession(ctx, maximumBytes);
    try {
        return await use(loaded);
    }
    finally {
        loaded.reservation.release();
    }
}
...
function reserveHistoryLoad(charge) {
    ...
    if (!reservation)
        throw new Error("history-load-memory-limit");
    return reservation;
}
...
function acquireSearchIndex(entry) {
    entry.pins++;
    let released = false;
    return {
...
async function indexedSession(ctx) {
    ...
    if (path) {
        ...
        if (before.size > SEARCH_INDEX_SOURCE_MAX_BYTES)
            throw new Error("history-index-memory-limit");
        sessionKey = path;
    ...
    if (pending) {
        ...
        return acquireSearchIndex(await pending);
    }
    ...
    const promise = (async () => {
        try {
            ...
            if (!reservation.move({ pendingBuild: components.total }))
                throw new Error("history-index-memory-limit");
            await new Promise((resolve) => setImmediate(resolve));
            const index = buildLocalSearchIndex(session);
            if (!reservation.move(components.retained))
                throw new Error("history-index-memory-limit");
            const entry = { session, sessionKey, generationKey, sourceBytes, index, bytes: components.total, reservation, pins: 0, evicted: false };
            evictSearchIndex(sessionKey);
    ...
    try {
        return acquireSearchIndex(await promise);
    }
    finally {
    ...
    pi.registerTool({
        ...
        async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
            ...
            if (!indexed) {
                let loaded;
                try {
                    loaded = await loadSession(ctx, LEGACY_HISTORY_MAX_BYTES);
                }
                catch (error) {
                    ...
                    if (error.message === "history-load-memory-limit")
                        return toolText("History unavailable: the bounded load could not be admitted within the local memory budget.", { status: "refused", code: "history-load-memory-limit" });
                    throw error;
                }
                try {
            ...
            let indexedLoaded;
            try {
                indexedLoaded = await indexedSession(ctx);
            }
            catch (error) {
