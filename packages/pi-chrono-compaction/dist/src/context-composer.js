...
export function storedSelectionInput(input, selection, recovery) {
    ...
    const exactRow = (item, maximumCut) => {
        ...
        return { id: item.stableKey, text: evidence.exactText, startSeq: evidence.source.eventSeq, endSeq: evidence.source.eventSeq,
            ...
            ...(omission ? { sourceOmissions: { beforeUtf16: omission.beforeUtf16, afterUtf16: omission.afterUtf16 },
    ...
    const addState = (item, maximumCut, delta) => {
        ...
        if (item.kind === "restriction")
            ...
            (delta ? deltaRows : recent).push({ ...row, kind: item.kind === "decision" ? "decision" : "capsule" });
    };
    ...
    if (selection.rollups) {
        ...
        if (rollups.handle.ruleset !== "episode-rollup-exact-v3" || rollups.handle.branchKey !== selection.branchKey
            ...
            || rollups.representedEndSeq < rollups.representedStartSeq)
            throw new Error("stored rollup selection exceeds the pinned memory view");
...
function validateInput(input) {
    if (typeof input.regularPiSummary !== "string")
        throw new Error("regular Pi summary must be text or an empty unavailable input");
...
function renderComposerRow(section, row, pinnedSnapshot, wholeRecords = false) {
...
function sectionsFor(input, level) {
    if (level === "pi-summary-and-tail" || level === "pi-default-required")
        return [];
    const rows = [];
    const renderRow = (section, row, pinned) => renderComposerRow(section, row, pinned, input.wholeRecords);
...
function replayText(input, level, reasons, rows) {
    ...
    const represented = (items) => items.every(item => rows.some(kept => kept.row.id === item.id && kept.completeText));
