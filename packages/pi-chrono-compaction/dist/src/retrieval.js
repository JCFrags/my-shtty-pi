...
function sliceExact(text, startChar = 0, maxChars = MAX_EXACT_CHARACTERS) {
...
function getContinuation(entryId, blockIndex, nextStart) {
...
function entryRecords(session) {
    return session.records.filter((record) => record.data.type !== "session");
}
export function historyGet(session, entryId, options = {}) {
    ...
    const sections = [`Entry ${entryId} (${entrySummary(entry)}), JSONL line ${record.lineNumber}`];
    ...
    if (options.blockIndex !== undefined) {
        ...
        if (block === undefined)
            throw new Error(`Entry ${entryId} has no block at index ${options.blockIndex}`);
        const exact = sliceExact(block, options.startChar, maxChars);
        sections.push(`Exact block ${options.blockIndex}:\n${exact.text}`);
        if (exact.nextStart !== undefined)
            sections.push(getContinuation(entryId, options.blockIndex, exact.nextStart));
        ...
        if (exact.nextStart !== undefined)
            sections.push(getContinuation(entryId, undefined, exact.nextStart));
    }
    if (neighbors.length > 1) {
        sections.push(`Neighboring chronological file records:\n${neighbors
            .map((neighbor) => {
...
export function historySearch(session, query, options = {}) {
    if (query.length === 0)
    ...
    for (const record of entryRecords(session)) {
        ...
        if (!first)
            continue;
        total += 1;
        if (total <= startMatch || matches.length >= limit)
            continue;
        ...
        matches.push(`[${id} | line ${record.lineNumber} | ${entrySummary(entry)}]\n${start > 0 ? "…" : ""}${record.rawLine.slice(start, end)}${end < record.rawLine.length ? "…" : ""}`);
    }
...
export function historyRange(session, startEntryId, endEntryId, options = {}) {
    if (!session.entryById.has(startEntryId))
        throw new Error(`Unknown start entry: ${startEntryId}`);
    if (!session.entryById.has(endEntryId))
        throw new Error(`Unknown end entry: ${endEntryId}`);
    const maxEntries = Math.min(MAX_RANGE_ENTRIES, Math.max(1, options.maxEntries ?? 200));
    const reversePath = [];
    let cursor = endEntryId;
    ...
    if (foundAncestor) {
        ...
        if (end < start)
            throw new Error(`End entry ${endEntryId} occurs before start entry ${startEntryId} in JSONL file order`);
    ...
    for (const record of range.slice(0, maxEntries)) {
        ...
        const rendered = record.rawLine.length > MAX_RANGE_CHARACTERS
            ...
            : record.rawLine;
        if (renderedRecords.length > 0 && usedCharacters + rendered.length + 1 > MAX_RANGE_CHARACTERS)
            break;
...
export async function historyGetFromLedger(sessionPath, ledger, entryId, options = {}) {
    ...
    const sections = [`Entry ${entryId} (${entrySummary(entry)}), JSONL line ${indexed.lineNumber}`];
    const maxChars = Math.min(MAX_EXACT_CHARACTERS, Math.max(1, options.maxChars ?? MAX_EXACT_CHARACTERS));
    if (options.blockIndex !== undefined) {
        const block = exactBlockContent(entry, options.blockIndex);
        if (block === undefined)
        ...
        if (exact.nextStart !== undefined)
            sections.push(getContinuation(entryId, options.blockIndex, exact.nextStart));
        if (rawLine.length <= 4_000)
            sections.push(`Exact containing JSONL record:\n${rawLine}`);
        else
            sections.push(`Containing JSONL record has ${rawLine.length} characters. Use history_get("${entryId}") for exact record slices.`);
    }
    else {
...
function ledgerRangeEntries(ledger, startEntryId, endEntryId) {
    const startEntry = ledger.entryById.get(startEntryId);
    const endEntry = ledger.entryById.get(endEntryId);
    if (!startEntry)
        throw new Error(`Unknown start entry: ${startEntryId}`);
    if (!endEntry)
        throw new Error(`Unknown end entry: ${endEntryId}`);
    ...
    if (ancestorIndex >= 0)
        return { range: endBranch.slice(ancestorIndex), rangeKind: "parent-chain chronological path" };
...
export async function historyRangeFromLedger(sessionPath, ledger, startEntryId, endEntryId, options = {}) {
...
export async function historyGetFromPath(sessionPath, entryId, options = {}) {
    return historyGet(await readSessionJsonl(sessionPath), entryId, options);
}
export async function historySearchFromPath(sessionPath, query, options = {}) {
    return historySearch(await readSessionJsonl(sessionPath), query, options);
}
export async function historyRangeFromPath(sessionPath, startEntryId, endEntryId, options = {}) {
    return historyRange(await readSessionJsonl(sessionPath), startEntryId, endEntryId, options);
}
export function inMemorySessionJsonl(header, entries) {
