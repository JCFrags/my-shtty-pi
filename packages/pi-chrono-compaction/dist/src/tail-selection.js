...
export function isValidCompactionCut(entry) {
...
function safeCutIndexes(entries) {
    const safe = new Set();
    const callIndexById = new Map();
    const resultIndexByCallId = new Map();
    entries.forEach((entry, index) => {
        for (const callId of toolCallIds(entry))
            callIndexById.set(callId, index);
        const resultId = toolResultId(entry);
        if (resultId)
            resultIndexByCallId.set(resultId, index);
    });
    ...
    entries.forEach((entry, index) => {
        ...
        if (callIndex === undefined || callIndex >= index)
            lastOrphanResultIndex = index;
    });
    const crossingToolCalls = new Set();
    for (let cutIndex = 1; cutIndex < entries.length; cutIndex += 1) {
        ...
        if (previous) {
            for (const toolCallId of toolCallIds(previous)) {
                ...
                if (resultIndex !== undefined && resultIndex >= cutIndex)
                    crossingToolCalls.add(toolCallId);
            }
            const resultId = toolResultId(previous);
            if (resultId)
                crossingToolCalls.delete(resultId);
        }
...
export function isSafeCompactionCut(entries, cutIndex) {
    return safeCutIndexes(entries).has(cutIndex);
}
export function selectRawTail(entries, desiredTokens, estimateTokens) {
    const desired = Math.max(256, Math.floor(desiredTokens));
    const candidates = [];
    const safeCuts = safeCutIndexes(entries);
    for (let cutIndex = 1; cutIndex < entries.length; cutIndex += 1) {
        const entry = entries[cutIndex];
        if (!entry || typeof entry.id !== "string" || !safeCuts.has(cutIndex))
            continue;
    ...
    const largestAvailable = [...candidates].sort((a, b) => b.actualTokens - a.actualTokens || a.cutIndex - b.cutIndex)[0];
    return {
        ...largestAvailable,
        reason: "the branch has less valid raw-tail content than the configured target, so the largest available tail was selected",
    };
}
export function selectRawTailWithinMaximum(entries, maximumTokens, estimateTokens) {
...
export function selectDynamicRawTail(entries, minimumTokens, maximumTokens, estimateTokens) {
...
//# sourceMappingURL=tail-selection.js.map