...
export class CatalogSource {
    ...
    snapshot(size = this.size) {
        ...
        if (evidence && size !== evidence.size)
            throw new CatalogSourceError("catalog-source-range");
        ...
        if (evidence) {
            ...
            for (const anchor of evidence.prior.anchors)
                this.verifyRange(anchor);
        }
        this.assertCurrent();
    ...
    accept(offset, bytes) {
        const evidence = this.evidence;
        if (!evidence || offset !== evidence.size || bytes.length > CATALOG_SOURCE_CHUNK_BYTES || bytes.length > this.size - offset)
            throw new CatalogSourceError("catalog-source-range");
        const remaining = CATALOG_SOURCE_ANCHOR_BYTES - evidence.first.length;
        if (remaining > 0)
            evidence.first = Buffer.concat([evidence.first, bytes.subarray(0, remaining)]);
