...
process.once("message", async (request) => {
    ...
    let response;
    try {
        if (Buffer.byteLength(JSON.stringify(request) ?? "") > SEARCH_V3_LIMITS.requestBytes)
            response = failure("search-v3-request-invalid");
        else if (isEpisodeStateRequest(request))
            response = await executeEpisodeStateRequest(request);
        else
            response = isSearchV3Request(request) ? await executeSearchV3Request(request) : failure("search-v3-request-invalid");
        if (response.ok)
