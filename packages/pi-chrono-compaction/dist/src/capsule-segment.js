...
import { CAPSULE_LIMITS, CAPSULE_SCHEMA_VERSION, CHUNK_CONTENT_HASH, CHUNK_SCHEMA_VERSION, MANIFEST_SCHEMA_VERSION, SEGMENT_CONTENT_HASH, isDecodedChunkDescriptor, isDerivedManifest, isDerivedPublicationReceipt, isReducerEnvelope, } from "./capsule-contract.js";
    ...
    const visit = (item) => {
        ...
        if (typeof item === "number") {
            if (!Number.isSafeInteger(item) || item < 0)
                fail("capsule-canonical-invalid");
            return item;
        }
        if (Array.isArray(item))
            return item.map(visit);
        if (!item || typeof item !== "object" || item instanceof Uint8Array || Buffer.isBuffer(item))
            fail("capsule-canonical-invalid");
        const output = {};
...
export function encodeChunkSegment(input, payload) {
    const provisional = { ...input, segmentHash: "0".repeat(64), segmentOffset: 0 };
    if (!isDecodedChunkDescriptor(provisional) || payload.byteLength !== input.utf16leBytes || sha256(payload) !== input.contentHash
        || input.contentHashAlgorithm !== CHUNK_CONTENT_HASH)
        fail("capsule-chunk-invalid");
    ...
    const chunk = { ...input, segmentHash: hash, segmentOffset: prefix.length };
...
function parseLength(bytes, offset) {
    ...
    if (!/^[0-9a-f]{8}\n$/.test(text))
        fail("capsule-segment-invalid");
    return { length: Number.parseInt(text.slice(0, 8), 16), next: offset + 9 };
}
export function decodeCapsuleSegment(bytes) {
    ...
    if (frame.next + frame.length + 1 !== bytes.length || bytes.at(-1) !== 10)
        fail("capsule-segment-invalid");
    let value;
    try {
        value = JSON.parse(bytes.subarray(frame.next, frame.next + frame.length).toString("utf8"));
    }
    catch {
        fail("capsule-segment-invalid");
    }
    if (!isReducerEnvelope(value) || canonicalJson(value) !== bytes.subarray(frame.next, frame.next + frame.length).toString("utf8"))
        fail("capsule-segment-invalid");
    return value;
}
export function decodeChunkPayload(bytes, descriptor) {
    ...
    let encodedDescriptor;
    try {
        encodedDescriptor = JSON.parse(headerText);
    }
    catch {
        fail("capsule-segment-invalid");
    }
    ...
    if (!isDecodedChunkDescriptor(descriptor) || sha256(bytes) !== descriptor.segmentHash
        || canonicalJson(encodedDescriptor) !== headerText || canonicalJson(encodedDescriptor) !== canonicalJson(expectedHeader))
    ...
    if (bytes[header.next + header.length] !== 10 || payloadLength.next !== descriptor.segmentOffset
        || payloadLength.length !== descriptor.utf16leBytes || payloadLength.next + payloadLength.length + 1 !== bytes.length || bytes.at(-1) !== 10)
        fail("capsule-segment-invalid");
...
export function encodeManifest(identity, layer, segment) {
    const base = { v: 1, schemaVersion: MANIFEST_SCHEMA_VERSION, identity, layer, segment, hashAlgorithm: SEGMENT_CONTENT_HASH };
    const hash = sha256(Buffer.from(canonicalJson(base), "utf8"));
    const manifest = { ...base, hash };
    if (!isDerivedManifest(manifest))
        fail("capsule-manifest-invalid");
...
export function encodeReceipt(receipt) {
    ...
    const complete = { ...receipt, receiptHash: hash };
    if (!isDerivedPublicationReceipt(complete))
        fail("capsule-receipt-invalid");
    return { bytes: Buffer.from(canonicalJson(complete) + "\n", "utf8"), receipt: complete };
}
function privateRegular(path, expectedBytes) {
    let stat;
    try {
        stat = lstatSync(path);
    }
...
export function syncCapsuleDirectory(path) {
...
export function readExactNamed(directory, name, expectedBytes) {
    if (!HASH.test(name) || !Number.isSafeInteger(expectedBytes) || expectedBytes < 1)
        fail("capsule-storage-range");
    return readExactSafe(join(directory, name), expectedBytes);
}
function readExactSafe(path, expectedBytes) {
    ...
    try {
        ...
        while (done < output.length) {
            const count = readSync(fd, output, done, output.length - done, done);
            if (!count)
                fail("capsule-content-corrupt");
            done += count;
        }
...
function renameNoReplace(source, destination) {
    if (source.length > 4096 || destination.length > 4096 || dirname(source) !== dirname(destination)
        ...
        || !HASH.test(basename(destination)))
        fail("capsule-storage-range");
    ...
    if (result.status === 125)
        return fail("capsule-storage-capability");
...
export function publishImmutable(directory, expectedHash, bytes, kind, hooks = {}, completeBytesHash = true) {
    if (!HASH.test(expectedHash) || (completeBytesHash && sha256(bytes) !== expectedHash))
        fail("capsule-content-hash");
    ...
    try {
        ...
        if (!existing.equals(Buffer.from(bytes)))
            fail("capsule-content-corrupt");
        hooks.fault?.("before-directory-fsync", kind);
        syncCapsuleDirectory(directory);
        hooks.fault?.("after-directory-fsync", kind);
        return finalPath;
    }
    catch (error) {
        if (error.code !== "capsule-content-missing")
            throw error;
    }
    ...
    try {
        ...
        while (offset < bytes.byteLength) {
            ...
            if (!written)
                fail("capsule-storage-io");
            offset += written;
        }
        hooks.fault?.("before-file-fsync", kind);
        fsyncSync(fd);
        const opened = fstatSync(fd);
        if (!opened.isFile() || opened.uid !== process.getuid?.() || opened.nlink !== 1 || (opened.mode & 0o7777) !== 0o600 || opened.size !== bytes.byteLength)
            fail("capsule-storage-unsafe");
        ...
        if (publication === "exists") {
            ...
            if (!raced.equals(Buffer.from(bytes)))
                fail("capsule-publication-conflict");
        }
...
export function readVerifiedImmutable(path, expectedHash, expectedBytes, offset = 0, length = expectedBytes) {
    if (!HASH.test(expectedHash) || !Number.isSafeInteger(expectedBytes) || expectedBytes < 1 || !Number.isSafeInteger(offset) || offset < 0
        || !Number.isSafeInteger(length) || length < 0 || offset > expectedBytes || length > expectedBytes - offset)
        fail("capsule-storage-range");
    privateRegular(path, expectedBytes);
    const fd = openSync(path, F.O_RDONLY | F.O_NOFOLLOW | F.O_NONBLOCK);
    try {
        const opened = fstatSync(fd);
        if (!opened.isFile() || opened.uid !== process.getuid?.() || opened.nlink !== 1 || (opened.mode & 0o7777) !== 0o600 || opened.size !== expectedBytes)
            fail("capsule-storage-unsafe");
        ...
        if (sha256(complete) !== expectedHash)
            fail("capsule-content-corrupt");
...
//# sourceMappingURL=capsule-segment.js.map