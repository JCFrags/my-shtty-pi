...
function validConfig(value) { if (!object(value) || !exactKeys(value, CONFIG_KEYS))
    ...
    else if (["recentExactBiasFraction", "minMarginalUtilityPerToken", "mergeBeforeFraction"].includes(key)) {
...
export function validateWorkerRequest(value) {
    ...
    if (!exactKeys(value, allowed) || !boundedId(value.jobId, 256) || !boundedId(value.sessionPath, 4096) || !validExpectation(value.expectedSource) || !integer(value.deadlineMs, Date.now() - 1, Date.now() + 3_600_000) || !integer(value.niceLevel, 0, 19))
        throw new Error("worker-protocol-error");
    ...
    if (value.jobType === "replay-compaction") {
        if (!boundedId(value.branchLeafId, 1024) || !boundedId(value.firstKeptEntryId, 1024) || !integer(value.hardOutputTokens, 128, 30_000) || !boundedText(value.retentionHints) || !boundedText(value.pinnedMemoryText) || typeof value.candidateStoreEnabled !== "boolean" || typeof value.cacheEnabled !== "boolean" || (value.valueWorkerMode !== undefined && !["off", "shadow", "advisory"].includes(String(value.valueWorkerMode))) || (value.valueWorkerConfigurationHash !== undefined && (typeof value.valueWorkerConfigurationHash !== "string" || !/^[a-f0-9]{64}$/.test(value.valueWorkerConfigurationHash))))
            throw new Error("worker-protocol-error");
        if (value.retrievalFeedback !== undefined && (!object(value.retrievalFeedback) || Buffer.byteLength(JSON.stringify(value.retrievalFeedback)) > MAX_WORKER_TEXT_BYTES))
            throw new Error("worker-protocol-error");
