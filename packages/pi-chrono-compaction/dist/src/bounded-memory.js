...
export function prepareBoundedMemory(input) {
    ...
    let priorTailStart, previousEntryId;
...
export function renderBoundedMemory(selection, input) {
    validateContextCeiling(input.combinedCeilingTokens);
    const available = input.combinedCeilingTokens - input.rawTailTokens;
    ...
    if (estimateTokensFromText(summary) > available && prior) {
        prior = "";
        summary = render();
    }
    ...
    if (renderedTokens > available)
        throw new Error("bounded-memory-budget-unavailable");
    return { summary, receipt: { mode: "bounded-programmatic-fallback", reason: selection.reason,
            ...
            earlierPrefixOmitted: selection.earlierPrefixOmitted, reusedPreviousMemory: !!prior, complete: false,
