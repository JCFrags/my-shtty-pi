...
import { appendMemoryEvent, createMemoryEvent, memorySidecarPath, readMemoryEvents, searchMemories } from "./memory-store.js";
...
import { boundedHistoryValue, HISTORY_FEEDBACK_LIMIT, HISTORY_WORKER_CAPS, LEGACY_HISTORY_MAX_BYTES, SEARCH_INDEX_SOURCE_MAX_BYTES, historyRefusal, historySourceAdmission, validHistoryPromotionEvent, } from "./history-worker-contract.js";
function boundedIds(values) {
    const result = [];
    const seen = new Set();
    for (const value of values) {
        if (value.length > 512 || seen.has(value))
            continue;
        result.push(value);
        seen.add(value);
        if (result.length === HISTORY_FEEDBACK_LIMIT)
            break;
    }
    return result;
}
function encode(response) {
    ...
    if (response.status === "ok" && Buffer.byteLength(response.text) > 50 * 1024)
...
export async function handleHistoryWorkerRequest(wire, progress = () => { }) {
    ...
    try {
        ...
        if (typeof wire !== "string" || wire.length > HISTORY_WORKER_CAPS.requestBytes || Buffer.byteLength(wire) > HISTORY_WORKER_CAPS.requestBytes)
            return finish(historyRefusal("history-request-limit"));
        const request = JSON.parse(wire);
        if (!boundedHistoryValue(request) || request.version !== 1 || typeof request.path !== "string" || !request.path.startsWith("/") || !request.source || !request.operation || !["get", "range", "legacy-search", "search", "recall"].includes(request.operation.kind))
            return finish(historyRefusal("history-request-invalid"));
        const { operation: op, source } = request;
        if (typeof source.deviceId !== "string" || typeof source.inodeId !== "string" || !Number.isFinite(source.mtimeMs))
            return finish(historyRefusal("history-request-invalid"));
        progress("admit");
        const refusal = historySourceAdmission(source.size, op);
        if (refusal)
            return finish(historyRefusal(refusal));
        ...
        if (op.kind === "get") {
            ...
            if (op.kind === "search") {
                ...
                for (const document of index.documents) {
                    if (!keys.has(document.key) && !keys.has(document.block.entryId))
                        continue;
                    if (blocks.length < HISTORY_FEEDBACK_LIMIT && document.block.id.length <= 512)
                        blocks.push(document.block.id);
                    if (resources.length < HISTORY_FEEDBACK_LIMIT && document.resourceKey && document.resourceKey.length <= 512)
                        resources.push(document.resourceKey);
                    if (blocks.length >= HISTORY_FEEDBACK_LIMIT && resources.length >= HISTORY_FEEDBACK_LIMIT)
                        break;
                }
                response = {
        ...
        if (JSON.parse(encode(response)).status !== "ok")
            return finish(historyRefusal("history-output-limit"));
        if (op.kind === "recall" && op.promotion) {
            ...
            const checkSidecar = async () => {
                try {
                    ...
                    if (!state.isFile() || state.nlink !== 1 || (state.mode & 0o077) !== 0 || state.size > 256 * 1024)
                        throw new Error("history-promotion-unavailable");
                }
                catch (error) {
            ...
            if (memory.status !== "ready")
                return finish(historyRefusal("history-promotion-unavailable"));
            ...
            for (const candidate of candidates) {
                const input = { action: candidate.state === "demoted" ? "promote" : "touch",
                ...
                if (!validHistoryPromotionEvent(createMemoryEvent(memory.events, input)))
                    return finish(historyRefusal("history-promotion-unavailable"));
                await checkSidecar();
                const updated = await appendMemoryEvent(path, input, {
                    maxReadBytes: 256 * 1024,
                    validateAppendEvent(event) {
                        if (!validHistoryPromotionEvent(event))
                            throw new Error("history-promotion-unavailable");
                    },
                });
...
//# sourceMappingURL=history-worker-handler.js.map