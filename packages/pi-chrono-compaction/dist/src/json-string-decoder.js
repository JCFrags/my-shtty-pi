const INVALID_UTF8 = Object.freeze({ code: "json-string-invalid-utf8" });
const INVALID_ESCAPE = Object.freeze({ code: "json-string-invalid-escape" });
const INVALID_STRING = Object.freeze({ code: "json-string-invalid-string" });
export function createJsonStringDecoderState() {
    return { escape: false, unicodeLeft: 0, unicode: 0, utfLeft: 0, utfValue: 0, utfMin: 0 };
}
export function isJsonStringDecoderError(value) {
 ...
 */
export function decodeJsonStringByte(state, byte, sink, context) {
    if (state.utfLeft) {
        ...
        if (--state.utfLeft === 0) {
            ...
            if (codePoint < state.utfMin || codePoint > 0x10ffff || (codePoint >= 0xd800 && codePoint <= 0xdfff))
                throw INVALID_UTF8;
            if (codePoint > 0xffff) {
    ...
    if (state.unicodeLeft) {
        ...
        if (--state.unicodeLeft === 0)
            sink(context, state.unicode);
        return false;
    }
    if (state.escape) {
        state.escape = false;
...
export function createQuotedJsonStringState() {
    return { opened: false, done: false, rawBytes: 0, decodedUnits: 0, decoder: createJsonStringDecoderState() };
}
function driverSink(driver, unit) {
    if (driver.state.decodedUnits === Number.MAX_SAFE_INTEGER)
        throw new Error("json-string-coordinate-overflow");
 ...
 */
export function decodeQuotedJsonStringChunk(state, input, sink, context) {
    let consumedBytes = 0;
    if (state.error || state.done)
        return { state, consumedBytes, ...(state.error ? { error: state.error } : {}) };
    const driver = { state, sink, context: context };
    while (consumedBytes < input.length && !state.done) {
        ...
        if (!state.opened) {
            ...
            try {
                ...
                if (!isJsonStringDecoderError(error))
                    throw error;
...
//# sourceMappingURL=json-string-decoder.js.map