import { byteCount, estimateTokensFromText, extractIdentifiers, getNumber, getRecord, getString, lineCount, truncateToTokens, unique, } from "../utils.js";
...
function filePath(context) {
    return (getString(context.block.toolArguments?.path) ??
...
function requestedRange(context) {
    ...
    if (offset === undefined && limit === undefined && end === undefined)
        return undefined;
...
function collectSelectedLines(lines, laterText) {
    const symbols = [];
    const imports = [];
    ...
    lines.forEach((line, index) => {
        ...
        if (/\b(?:TODO|FIXME|HACK|XXX|throw new|panic!|return Err|console\.error)\b/.test(line))
            selectedIndexes.add(index);
    ...
    if (selectedIndexes.size === 0) {
        for (let index = 0; index < Math.min(lines.length, 28); index += 1)
            selectedIndexes.add(index);
        for (let index = Math.max(28, lines.length - 16); index < lines.length; index += 1)
            selectedIndexes.add(index);
    }
    const selected = [];
    let previous = -2;
    for (const index of [...selectedIndexes].sort((a, b) => a - b)) {
        if (index > previous + 1 && selected.length > 0)
            selected.push("…");
...
export function reduceFileRead(context) {
    ...
    const revision = getString(context.block.toolArguments?.revision) ??
    ...
    if (revision)
        sections.push(`Historical revision/hash: ${revision}`);
    if (range)
        sections.push(`Requested line range: ${range}`);
    sections.push(`Historical read length: ${lines.length} line(s)`);
    if (selected.symbols.length > 0)
        sections.push(`Symbols observed: ${selected.symbols.slice(0, 30).join(", ")}`);
    if (selected.imports.length > 0)
        sections.push(`Imports observed: ${selected.imports.slice(0, 12).join(" | ")}`);
