...
function scoreSentence(sentence, index, total) {
    ...
    if (index === total - 1)
        score += 4;
    if (HIGH_VALUE.test(sentence))
        score += 8;
    if (/`[^`]+`|\b[A-Za-z0-9_.-]+\.(?:ts|tsx|js|jsx|py|rs|go|java|json|yaml|yml|toml|md)\b/.test(sentence))
        score += 4;
    if (/\b\d+(?:\.\d+)?\b/.test(sentence))
        score += 2;
    if (/^\s*(?:[-*]|\d+\.)/.test(sentence))
        score += 2;
    if (LOW_VALUE.test(sentence))
        score -= 10;
    return score;
}
export function reduceAssistantProse(context) {
    ...
    for (const item of ranked) {
        ...
        if (selected.length > 0 && used + tokens > context.maxTokens)
            continue;
        selected.push(item);
        used += tokens;
        if (used >= context.maxTokens * 0.9)
            break;
    }
