...
export const CHRONO_VERSION = "2.0.30";
...
function readBounded(url, maximum) {
    ...
    try {
        ...
        if (!info.isFile() || info.size > maximum)
            throw new Error("identity-input-invalid");
        ...
        if (length > maximum || (info.size > 0 && length !== info.size))
            throw new Error("identity-input-changed");
 ...
 */
export function captureRuntimeIdentity(entrypoint) {
    ...
    try {
        ...
        if (!/^\d+$/.test(ticks ?? "") || !/^[0-9a-f-]{36}$/.test(boot))
            throw new Error("identity-process-invalid");
        return Object.freeze({ ...common, state: "captured", processStart: `${boot}:${ticks}`,
        ...
        return Object.freeze({ ...common, state: "unverified" });
    }
}
