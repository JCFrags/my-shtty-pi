...
function valueFor(block, reuseCount, novelty) {
    let value = 25;
    const reasons = [];
...
export function assignRetentionGradient(blocks, options = {}) {
    ...
    for (let index = blocks.length - 1; index >= 0; index -= 1) {
        ...
        const ageBand = sourceTokensFromNewest <= policy.hotSourceTokens
            ? "hot"
            : sourceTokensFromNewest <= policy.hotSourceTokens + policy.warmSourceTokens
                ? "warm"
                : "cold";
        const score = valueFor(block, options.reuseByBlockId?.get(block.id) ?? 0, options.noveltyByBlockId?.get(block.id) ?? 0.5);
        const override = block.protectedExact || block.unresolved;
        const band = override && ageBand === "cold" ? "warm" : ageBand;
...
function coldCue(unit, block, policy) {
    ...
    const text = truncateToTokens(`${outcome} ${block.label.toLowerCase()}: ${firstSentence}\nExact source: ${sourceRefText(unit.sourceRefs[0])}`, policy.coldCueTokens, "…");
    return {
...
function isAggressivelyLossy(candidate) {
 ...
 */
export function applyRetentionGradient(units, blocks, options = {}) {
    const policy = { ...DEFAULT_RETENTION_POLICY, ...options };
...
//# sourceMappingURL=retention-gradient.js.map