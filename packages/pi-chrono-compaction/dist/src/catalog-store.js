...
import { isCatalogStoreRequest, isStoreKey } from "./catalog-store-contract.js";
import { withRuntimeMutex } from "./worker-runtime-mutex.js";
const hash = (s) => createHash("sha256").update(s).digest("hex");
function fail(code) { throw Object.assign(new Error(code), { code }); }
const missing = (e) => e?.code === "ENOENT";
const same = (a, b) => a.dev === b.dev && a.ino === b.ino && a.size === b.size && a.mtimeMs === b.mtimeMs && a.ctimeMs === b.ctimeMs && a.mode === b.mode && a.nlink === b.nlink && a.uid === b.uid;
...
function privateDirectory(path) {
    ...
    if (!s.isDirectory() || s.isSymbolicLink() || s.uid !== process.getuid() || (s.mode & 0o7777) !== 0o700)
...
function prepare(path, create) {
    if (resolve(path) !== path || path === "/" || Buffer.from(path).toString() !== path)
        fail("catalog-storage-unsafe");
    ...
    for (const part of parts) {
        ...
        if (!s.isDirectory() || s.isSymbolicLink() || (s.uid !== 0 && s.uid !== process.getuid()) || ((s.mode & 0o022) !== 0 && !(s.uid === 0 && (s.mode & 0o1000) !== 0)))
            fail("catalog-storage-unsafe");
    }
    privateDirectory(path);
}
function record(path, valid) {
    ...
    const safe = (s) => {
        if (!s.isFile() || s.isSymbolicLink() || s.uid !== process.getuid() || (s.mode & 0o7777) !== 0o600 || s.nlink !== 1 || s.size < 2 || s.size > 4096)
            fail("catalog-pointer-unsafe");
    };
    safe(before);
    ...
    try {
        ...
        if (size !== opened.size || !same(opened, fstatSync(fd)) || !same(opened, lstatSync(path)) || !same(parent, privateDirectory(dirname(path))))
            fail("catalog-pointer-changed");
        let result;
        try {
            result = JSON.parse(bytes.subarray(0, size).toString("utf8"));
        }
        catch {
            fail("catalog-pointer-invalid");
        }
        if (!valid(result))
            fail("catalog-pointer-invalid");
        return result;
...
function atomic(path, value, hooks) {
    const parent = privateDirectory(dirname(path));
    const bytes = Buffer.from(JSON.stringify(value));
    if (bytes.length > 4096)
        fail("catalog-pointer-limit");
    ...
    try {
        ...
        hooks.fault?.("before-metadata-sync");
        fsyncSync(fd);
        const st = fstatSync(fd);
        if (!st.isFile() || st.nlink !== 1 || st.uid !== process.getuid() || (st.mode & 0o7777) !== 0o600 || !same(st, lstatSync(tmp)))
            fail("catalog-pointer-unsafe");
        ...
        if (now.dev !== parent.dev || now.ino !== parent.ino)
            fail("catalog-pointer-changed");
        hooks.fault?.("before-metadata-rename");
        renameSync(tmp, path);
        hooks.fault?.("after-metadata-rename");
...
export function createCatalogStoreExecutor(engine, hooks = {}) {
    return async (value) => {
        if (!isCatalogStoreRequest(value))
        ...
        try {
            ...
            prepare(root, create);
            for (const child of ["stores", "refs", "stages"])
                prepare(join(root, child), create);
            const activePath = join(root, "active.json");
            const lock = (fn) => withRuntimeMutex(join(root, "publication.lock"), async () => { prepare(root, false); return fn(); });
            const active = () => {
                const a = record(activePath, activeValid);
                if (a && a.sessionKey !== r.sessionKey)
            ...
            const refPath = (storeKey) => join(root, "refs", `${storeKey}.json`);
            const readRef = (storeKey) => {
                const ref = record(refPath(storeKey), refValid) ?? fail("catalog-store-missing");
                if (ref.storeKey !== storeKey || ref.sessionKey !== r.sessionKey || ref.folder !== (ref.recovery ? `recovery-${hash(`${r.sessionKey}\0${ref.recovery.rebuildKey}`)}` : `initial-${hash(r.sessionKey)}`))
            ...
            const commitRef = (ref) => {
                ...
                if (old) {
                    if (JSON.stringify(old) !== JSON.stringify(ref))
                        fail("catalog-store-mismatch");
            ...
            if (r.op === "recoverStart") {
                ...
                const stage = await lock(() => {
                    ...
                    if (prior) {
                        if (prior.sessionKey !== r.sessionKey || prior.folder !== folder || prior.rebuildKey !== r.rebuildKey)
                            fail("catalog-store-mismatch");
                ...
                if (!isStoreKey(s.storeKey) || !Number.isSafeInteger(result.generation) || Number(result.generation) < 1)
                    fail("catalog-store-mismatch");
                const ref = { v: 1, sessionKey: r.sessionKey, storeKey: s.storeKey, folder, recovery: { rebuildKey: r.rebuildKey, generation: Number(result.generation), expectedActiveStoreKey: stage.expectedActiveStoreKey } };
            ...
            if (r.op === "recoverPublish") {
                ...
                await lock(() => {
                    ...
                    if (current === ref.storeKey) {
                        syncDirectory(root);
                        return;
                    } // Finish durability on retry.
                    if (current !== r.expectedActiveStoreKey)
                        fail("catalog-publication-conflict");
            ...
            if (!storeKey) {
                ...
                if (!a) {
                    ...
                    if (!isStoreKey(s.storeKey))
                        fail("catalog-store-mismatch");
                    const ref = { v: 1, sessionKey: r.sessionKey, storeKey: s.storeKey, folder };
                    a = await lock(() => {
                        ...
                        commitRef(ref);
                        hooks.fault?.("before-active-publish");
                        const next = { v: 1, sessionKey: r.sessionKey, storeKey: ref.storeKey };
                        atomic(activePath, next, hooks);
                        hooks.fault?.("after-active-publish");
                        return next;
                    });
                }
