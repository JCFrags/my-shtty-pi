...
export function searchResultCacheStatus(index) {
...
function termFrequency(values, cap = 12) {
    const frequencies = new Map();
    for (const value of values)
        frequencies.set(value, Math.min(cap, (frequencies.get(value) ?? 0) + 1));
    return frequencies;
}
function normalizedForDuplicate(text) {
...
export function buildLocalSearchIndex(sessionOrBlocks, existingLineage) {
    const blocks = Array.isArray(sessionOrBlocks)
        ? sessionOrBlocks
        : parseHistoricalBlocks(sessionOrBlocks.entries, { includeHistoricalCompactions: false, includeMetadata: false });
...
function fuzzyPathScore(queryTerms, document) {
    if (queryTerms.length === 0 || document.pathTerms.size === 0)
        return 0;
    let score = 0;
    for (const query of queryTerms) {
        ...
        for (const path of document.pathTerms) {
            if (path === query)
                ...
                if (segments.some((segment) => query.length >= 5 && editDistanceAtMostOne(query, segment)))
...
function applies(document, filters) {
    if (!filters)
        return true;
    const block = document.block;
    if (filters.kinds && !filters.kinds.includes(block.kind))
        return false;
    if (filters.toolNames && !filters.toolNames.includes(block.toolName ?? ""))
        return false;
    if (filters.error !== undefined && block.isError !== filters.error)
        return false;
    if (filters.unresolved !== undefined && block.unresolved !== filters.unresolved)
        return false;
    if (filters.protected !== undefined && block.protectedExact !== filters.protected)
        return false;
    if (filters.resourceKinds && (!document.resourceKind || !filters.resourceKinds.includes(document.resourceKind)))
        return false;
    if (filters.resourceKey !== undefined && document.resourceKey !== filters.resourceKey)
        return false;
    if (filters.currentState && filters.currentState !== "any" && document.resourceState !== filters.currentState)
        return false;
    if (filters.fromTimestamp && (!block.timestamp || block.timestamp < filters.fromTimestamp))
        return false;
    if (filters.toTimestamp && (!block.timestamp || block.timestamp > filters.toTimestamp))
        return false;
    return true;
}
function bm25(index, document, queryTerms) {
    const k1 = 1.2;
    const b = 0.75;
    let score = 0;
...
function scoreDocuments(index, query, options) {
    ...
    for (const document of index.documents) {
        if (!applies(document, options.filters))
            continue;
        const block = document.block;
        ...
        if (block.kind === "user" || block.kind === "custom_message")
            score += Math.min(3, score * 0.2);
        if (block.unresolved)
            score += Math.min(4, score * 0.25);
        if (block.isError)
            score += Math.min(3, score * 0.2);
        if (document.resourceState === "current")
            score += Math.min(3, score * 0.15);
        if (document.resourceState === "superseded")
            score *= 0.78;
        if (block.kind === "branch_summary")
            score *= 0.6;
        if (score <= 0)
            continue;
...
function exactDocuments(index, query, options) {
    let pattern;
    try {
...
function snippet(text, query, stage, caseSensitive = false) {
    const clean = text.replace(/\x1b\[[0-?]*[ -/]*[@-~]/g, "");
...
function hitFrom(scored, query, stage, duplicateCount, context = false) {
...
function encodeCursor(generationHash, queryHash, offset) {
    return Buffer.from(stableStringify({ generationHash, queryHash, offset })).toString("base64url");
}
function decodeCursor(cursor, generationHash, queryHash) {
    try {
...
function addNeighbors(index, hits, query, stage) {
    ...
    for (const hit of hits) {
        ...
        if (!document)
            continue;
        const candidates = [index.documents[document.ordinal - 1], index.documents[document.ordinal + 1]].filter((value) => value !== undefined);
        ...
        for (const neighbor of [...candidates, ...(paired ? [paired] : [])]) {
            if (selectedKeys.has(neighbor.key))
                continue;
            ...
            if (output.length >= hits.length * 3)
                break;
        }
    }
    return output;
}
export function searchLocalHistory(index, query, options = {}) {
    if (!query.trim())
        throw new Error("history_search query must not be empty");
    ...
    const indexCache = cache.get(index) ?? new Map();
    ...
    for (const scored of ranked)
        duplicateCounts.set(scored.document.normalizedHash, (duplicateCounts.get(scored.document.normalizedHash) ?? 0) + 1);
    ...
    for (const scored of ranked) {
        ...
        if (seenHashes.has(document.normalizedHash))
            continue;
        if ((perEntry.get(document.block.entryId) ?? 0) >= 2)
            continue;
        if (document.resourceKey && (perResource.get(document.resourceKey) ?? 0) >= 3)
            continue;
        ...
        if (document.resourceKey)
            perResource.set(document.resourceKey, (perResource.get(document.resourceKey) ?? 0) + 1);
        diverse.push(scored);
    }
    const baseResult = (hits, nextOffset, includeNext = true) => ({
    ...
    for (const [relativeOffset, scored] of diverse.slice(offset, offset + limit).entries()) {
        ...
        if (!renderBoundedSearch(tentativeResult).complete)
            break;
        selected = tentative;
    }
    if (selected.length === 0 && offset < diverse.length) {
        ...
        if (!renderBoundedSearch(baseResult(selected, offset + 1)).complete) {
            ...
            if (!renderBoundedSearch(failClosed).complete)
                throw new Error("History search recovery cue cannot fit the supported response budget");
        }
    }
    ...
    if (options.includeNeighbors && selected.length > 0) {
        ...
        for (const context of contextCandidates) {
            ...
            if (!renderBoundedSearch(baseResult(tentative, nextOffset)).complete)
                break;
            bounded = tentative;
        }
    }
    let preliminary = baseResult(bounded, nextOffset);
    if (!renderBoundedSearch(preliminary).complete)
        preliminary = baseResult(bounded, nextOffset, false);
    const rendered = renderBoundedSearch(preliminary);
    if (!rendered.complete)
        throw new Error("History search response cannot fit the supported response budget");
    const result = { ...preliminary, returnedTokens: estimateTokensFromText(rendered.text) };
    indexCache.set(cacheKey, result);
    while (indexCache.size > MAX_SEARCH_RESULT_CACHE_ENTRIES)
        indexCache.delete(indexCache.keys().next().value);
    return result;
}
function exactRecovery(hit) {
...
function renderBoundedSearch(result) {
    ...
    let ordered = () => [...hitSections, ...(next ? [next] : [])];
    if (!fits(ordered())) {
    ...
    let generation = "";
    let instruction = "";
    const compose = () => [header, generation, ...hitSections, ...(next ? [next] : []), instruction].filter(Boolean);
...
export function renderRankedSearch(result) {
