...
export async function rendezvousDirectory(namespace) {
...
async function connect(path, local) {
    return new Promise((resolve, reject) => {
        ...
        socket.once("error", (error) => { socket.destroy(); if (error.code === "ENOENT" || error.code === "ECONNREFUSED")
            ...
            reject(new Error("worker-rendezvous-unavailable")); });
    });
}
...
async function coalesceHostJobInner(namespace, identity, signal, execute, maxResultBytes, waiter, local) {
    ...
    for (;;) {
        ...
        const socket = await withRuntimeMutex(join(directory, "lock"), async () => {
            ...
            try {
                ...
                if (!metadata.isSocket() || metadata.uid !== process.getuid?.())
                    throw new Error("unsafe-rendezvous-artifact");
                await rm(path);
            }
            catch (error) {
            ...
            const server = createServer(peer => {
                ...
                let expiration;
                const leave = () => { if (expiration)
                    clearTimeout(expiration); peers.delete(peer); if (joined && !finished && peers.size === 0)
                    controller.abort(); };
                const cancel = (code) => {
                    if (cancelled)
                        return;
                ...
                peer.on("data", (chunk) => {
                    ...
                    for (;;) {
                        ...
                        try {
                            ...
                            if (!joined) {
                                if (!value || Object.keys(value).sort().join(",") !== "deadlineMs,identity,schemaVersion" || value.schemaVersion !== 1 || value.identity !== identity || !Number.isSafeInteger(value.deadlineMs) || value.deadlineMs > Date.now() + WORKER_LIMITS.timeoutSeconds.max * 1000 || finished || controller.signal.aborted) {
                                    peer.end('{"kind":"retry"}\n');
                                    return;
                                }
                                if (peers.size >= WORKER_LIMITS.waitersPerJob) {
                                    peer.end('{"kind":"error","code":"scheduler-queue-full"}\n');
                                    return;
                                }
                                if (value.deadlineMs <= Date.now()) {
                                    peer.end('{"kind":"error","code":"worker-timeout"}\n');
                                    return;
                                }
                                joined = true;
                                peers.add(peer);
                                ...
                                if (first)
                                    void task.then(result => publish({ kind: "result", value: result }), error => publish({ kind: "error", code: error.message })).finally(shutdown).catch(() => { });
                            }
                            else {
                                if (!value || Object.keys(value).sort().join(",") !== "cancel,reason" || value.cancel !== true || !["worker-aborted", "worker-timeout"].includes(value.reason)) {
                                    peer.destroy();
                                    return;
                                }
                                cancel(value.reason);
                ...
                peer.setTimeout(10_000, () => { if (!joined)
                    peer.destroy(); });
            });
            const shutdown = () => shutdownPromise ??= withRuntimeMutex(join(directory, "lock"), async () => {
                finished = true;
            ...
            const progress = (stage) => {
                if (finished || progressCount >= 128 || !/^[a-z][a-z0-9-]{0,63}$/.test(stage))
                    return;
                progressCount++;
            ...
            const publish = (value) => {
                ...
                let line;
                try {
                    line = Buffer.from(JSON.stringify(value) + "\n");
                    if (line.length > maxResultBytes + 1024)
                        throw new Error();
                }
                catch {
                    line = Buffer.from('{"kind":"error","code":"worker-response-too-large"}\n');
                }
                for (const peer of peers) {
                    peer.end(line);
                    peer.setTimeout(1000, () => peer.destroy());
                }
                peers.clear();
            };
        ...
        const result = await new Promise((resolve, reject) => {
            ...
            let expiration;
            const finish = (error, value, retry = false) => {
                if (settled)
                    return;
                settled = true;
                if (expiration)
            ...
            socket.on("data", (chunk) => {
                bytes += chunk.length;
                if (bytes > maxResultBytes + 65536) {
                    finish(new Error("worker-response-too-large"));
                    return;
                }
                ...
                for (;;) {
                    ...
                    try {
                        ...
                        if (value.kind === "progress") {
                            if (Object.keys(value).sort().join(",") !== "kind,stage" || typeof value.stage !== "string" || !/^[a-z][a-z0-9-]{0,63}$/.test(value.stage))
                                throw new Error();
                            try {
                                waiter.onProgress?.(value.stage);
                            }
                            catch { }
                            continue;
                        }
                        ...
                        if (value.kind === "retry")
                            ...
                            finish(new Error(["worker-aborted", "scheduler-queue-full", "worker-response-too-large", "worker-containment-unavailable", "worker-source-limit", "worker-resource-limit", "worker-timeout", "scheduler-timeout", "scheduler-policy-mismatch", "worker-protocol-error", "worker-crashed"].includes(value.code) ? value.code : "worker-crashed"));
                        else if (value.kind === "result")
                            finish(undefined, value.value);
                        else
                            finish(new Error("worker-protocol-error"));
                    }
                    catch {
                        finish(new Error("worker-protocol-error"));
                    }
...
//# sourceMappingURL=worker-runtime-rendezvous.js.map