...
export function canonicalWorkerJson(value) {
    ...
    const normalize = (input) => {
        ...
        if (typeof input !== "object" || ancestors.has(input))
            throw new WorkerRuntimeError("worker-protocol-error");
        ancestors.add(input);
        try {
    ...
    if (Buffer.byteLength(text) > MAX_WORKER_REQUEST_BYTES)
        throw new WorkerRuntimeError("worker-protocol-error");
    return text;
}
...
export async function runBoundedWorker(options) {
    ...
    if (!identity || Object.keys(identity).sort().join(",") !== "kind,schemaVersion,sessionKey" || identity.schemaVersion !== 1 || !/^[a-z][a-z0-9-]{0,63}$/.test(identity.kind) || !/^[a-f0-9]{64}$/.test(identity.sessionKey) || !inRange(slots, WORKER_LIMITS.slots) || !caps || Object.keys(caps).some(key => !["deadlineMs", "sourceBytes", "responseBytes", "memoryBytes", "heapMiB"].includes(key)) || !inRange(caps.deadlineMs, { min: Date.now() + 1, max: Date.now() + WORKER_LIMITS.timeoutSeconds.max * 1000 }) || !inRange(caps.sourceBytes, { min: 1, max: WORKER_LIMITS.sourceBytes }) || !inRange(caps.responseBytes, { min: 1, max: MAX_WORKER_RESPONSE_BYTES }) || (caps.memoryBytes !== undefined && !inRange(caps.memoryBytes, { min: 64 * 1024 * 1024, max: Math.floor(WORKER_LIMITS.hostMemoryBytes / slots) })) || (options.priority !== undefined && options.priority !== "high" && options.priority !== "low"))
        throw new WorkerRuntimeError("worker-protocol-error");
    if (options.entryTransport !== undefined && options.entryTransport !== "stdio" && options.entryTransport !== "ipc")
        throw new WorkerRuntimeError("worker-protocol-error");
    if (caps.heapMiB !== undefined && !inRange(caps.heapMiB, { min: 16, max: Math.floor((caps.memoryBytes ?? WORKER_LIMITS.hostMemoryBytes / slots) / (1024 * 1024)) }))
        throw new WorkerRuntimeError("worker-protocol-error");
    const production = options.schedulerDirectory === undefined;
    if (production && !await verifyLegacyAdmissionGate())
        throw new WorkerRuntimeError("worker-legacy-transition-required");
    const entryPath = await realpath(options.entryPath);
    const entryBytes = await readFile(entryPath);
    if (entryBytes.length > 1024 * 1024)
        throw new WorkerRuntimeError("worker-entrypoint-unavailable");
    ...
    const result = await coalesceHostJob(namespace, hash, options.signal, async (signal, progress) => {
        ...
        const stage = (label) => { stageWrites = stageWrites.then(() => publishRuntimeStage(namespace, lease.slot, runtimeCategory(identity.kind), label)).catch(() => { }); progress(label); };
        try {
            while (!worker) {
                if (signal.aborted)
                    throw new WorkerRuntimeError("worker-aborted");
                if (Date.now() >= hardDeadlineMs)
                    throw new WorkerRuntimeError("worker-timeout");
                try {
            ...
            const value = await new Promise((resolve, reject) => {
                ...
                running.on("message", frame => {
                    ...
                    if (responseBytes > caps.responseBytes) {
                        reject(new WorkerRuntimeError("worker-response-too-large"));
                        return;
                    }
                    try {
                        const checked = options.validateResponse(frame);
                        if (checked !== undefined)
                            resolve(checked);
                        else {
                            const label = runtimeStage(frame);
                            if (!label)
                                throw new Error();
                            stage(label);
                        }
                        ...
                        reject(new WorkerRuntimeError("worker-protocol-error"));
                    }
                });
                running.stderr.on("data", (bytes) => { stderrBytes += bytes.length; if (stderrBytes > MAX_WORKER_STDERR_BYTES)
                    reject(new WorkerRuntimeError("worker-response-too-large")); });
                ...
                if (signal.aborted)
                    ...
                    running.send(request, entryPath, error => { if (error)
                        reject(new WorkerRuntimeError("worker-crashed")); }, caps.sourceBytes, options.entryTransport);
            });
            await running.stop();
