...
export async function runWorkerRuntimeStartupEntry(argv = process.argv.slice(2)) {
    if (argv.length !== 4 || argv[0] !== "--authorization" || !argv[1] || argv[2] !== "--package" || !argv[3])
        throw new Error("invalid-arguments");
    for (const key of Object.keys(process.env)) {
        if (key.startsWith("PI_CHRONO_") || key === "TMPDIR" || key === "TMP" || key === "TEMP")
            throw new Error("environment-override-refused");
    }
...
//# sourceMappingURL=worker-runtime-startup-entry.js.map