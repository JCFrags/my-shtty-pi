...
export const EPISODE_ROLLUP_RULESET_VERSION = "episode-rollup-exact-v2";
...
export async function executeEpisodeRollupRequest(request, options = {}) {
    ...
    try {
        ...
        const action = async () => {
            const path = join(request.searchDirectory, "rollup-v2.sqlite"), validate = (candidate) => new Store(candidate, request).validate(create);
