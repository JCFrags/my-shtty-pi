...
export class HistorySearchAdapter {
    ...
    cancel() { this.scheduler.cancel(); this.key = undefined; this.target = undefined; }
    disable() { this.scheduler.disable(); this.key = undefined; this.target = undefined; }
    dispose() { this.scheduler.dispose(); this.key = undefined; this.target = undefined; }
    async step(t, signal) {
        ...
        if (this.progress.catalog !== "ready") {
            ...
            if (ingested.result.caughtUp !== true)
                return { ...this.progress, catalog: "lagging", waitingForAppend: ingested.result.incompleteTail === true };
    ...
    scoped(reference) {
        ...
        if (!current || this.progress.index !== "ready")
            return fail("search-v3-index-not-ready");
        if (!reference)
            return current;
        const view = reference.view;
        if (view.storeKey !== current.view.storeKey || view.generation !== current.view.generation || view.sessionKey !== current.view.sessionKey || view.branchKey !== current.view.branchKey || view.eventCut > current.view.eventCut || view.segments.some(segment => !current.view.segments.some(s => s.segment === segment.segment && segment.cut <= s.cut)))
            return fail("search-v3-reference-scope-mismatch");
        return { ...current, view };
    }
    async search(params, signal) {
        try {
            if (params.unresolved !== undefined || params.includeNeighbors === true || params.fuzzyPath === true || Number(params.startMatch ?? 0) !== 0)
