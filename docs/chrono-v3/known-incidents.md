...
| I-0007 | Synthetic verification timing sensitivity under CI load | corrected in R1 | The scheduler-capacity test now holds acquired leases behind an explicit barrier, and the synthetic abort probe allows bounded child startup time. These are test-only corrections; deployed runtime source/dist remain unchanged. |
