...
Deployment preparation found the accepted 2.0.4 package and earlier rollback intact, with persistent isolated workers enabled and shadow disabled. Both documented default admission directories were absent. Live deployment was initially held pending explicit verified boot-recovery authorization; no policy, gate, inhibitor or scheduler state was implicitly created. The asynchronous question provider refused to queue the authorization request; it was asked directly in chat while independent candidate work continued. Referenced general activation documentation was absent from both this branch and the canonical checkout, so the existing Chrono activation reference and retained rollback scripts are the available deployment references. These inspection failures did not mutate production. The accepted 2.0.4 read-only gate API also returned false without creating state.

Rollback preparation copied the complete retained 2.0.4 package and dependencies, configuration, settings, activation pointer and repository bundle into a new owner-only private activation root. A byte/ownership/permission check passed for 24,331 files and 13 internal symlinks; package-tree SHA-256 is `9a654d6b2b9dd56a654b264886eb662354c2a4ef6e74608eaa727c7f7041f341`. The rollback script passed syntax and `--check`; it has **not been applied**. It does not stop unrelated agents, rewrite configuration, migrate policy or remove inhibitors. Existing older recovery points remain retained. Deployment Node is 24.18.0, ABI 137; the fresh candidate's controlled native build passed before installation. This original backup remains retained, but its pre-COMMAND8 alias/pointer assumptions are historical; use the reconciled Chrono-only registration rollback below.

Deployment metadata is committed as `4e37a5b`: 2.0.5 package SHA-256 `6082b36dac835779ce35ea29a6ebc05e40bf9fcb5bcec3d4f49e68be7d150260`, lock SHA-256 `9f0d9218009cae48b2116edce63b99363defa90ed2aa064e91da0a0f22c56571`. The 96-row manifest and exact verifiers were updated together; source/dist remain byte-identical to the approved head. Isolated typecheck/build, all manifest rows, both existing-addon allocation probes, clean frozen verification and 29 verifier tests passed. The metadata worker's first ad hoc file-count check counted only 85 top-level files; the corrected recursive check found the expected 95 without output changes. Its first static-root scan exceeded a 120-second command lifetime; an unchanged 600-second invocation passed. This was a caller bound, not a verifier assertion or runtime-cap change. The deployment harness is integrated as `9eadeee`. Exact deployment candidate `dcd91924dbcfc0c02489e04c3e163b33e2b08e86` passed push CI [34188601273](https://github.com/JCFrags/my-shtty-pi/actions/runs/34188601273) and PR CI [34188607542](https://github.com/JCFrags/my-shtty-pi/actions/runs/34188607542), both attempt 1, all eight jobs. Local full-root verification passed both 535-test suites, replay, 512/1024 MiB lanes, native build/probe, privacy, 15 safe scripts, 17 package checks, 291 historical hashes and 16 Project Glance tests. The first local invocation failed 534/535 because the launcher's inherited `umask 077` changed an intentionally unsafe 0644 fixture to 0600; the runtime correctly rejected the resulting empty safe file as corrupt. One authorized corrected invocation used explicit `umask 022`, with private logs created separately, at identical clean HEAD and unchanged deadlines. Both attempts and the launcher diagnosis are retained. No runtime or test assertion changed.

The private candidate passed fresh locked installation, controlled native build, typecheck/build and all 96 manifest rows at Node 24.18.0/ABI 137. Native SHA-256 remains `baac38739b5e4c5137ea0514c451df58423c2e626f43cddcfb4542206f93d013`, with real allocation refusal. Initial native preparation omitted the strict builder's three required arguments and refused with usage. The corrected explicit node-gyp/header/version invocation passed. A later evidence read assumed a record-mode sidecar that strict mode does not create and returned ENOENT; the existing strict-build JSON was saved instead, without rebuilding or weakening provenance. Generated source maps were retained outside frozen dist; committed source and dist are unchanged from the approved code head. The exact candidate also passed a fresh locked-Pi 0.84.2 synthetic canary; all owned processes and admissions settled.

The harness's final exact bytes passed disposable runs with locked Pi 0.84.2 and unchanged global Pi 0.85.1. These are development checks, not installed-production evidence or expanded peer support. They cover the actual manifest entry, six full persisted tool results at the offline provider boundary, Pi's regular summarizer using a fixed offline fixture, contained replay, history, off restart without catalog writes, two independent catalog clients and fresh restarts, pinned chronology and exact sampled raw recovery. No external provider was called. Earlier attempts failed on import-only dependency resolution and two regular-summary checks caused by separate nested Pi-AI compatibility registries. The harness now registers the same synthetic API in each resolved registry; runtime files remain unchanged. Final runs sampled private WAL/SHM/temp permissions and confirmed task admissions settled. Evidence is retained, not securely erased. These initial runs did not test production mode; the subsequent installed runs below do. Production mode refuses absent admission state. It leaves the scheduler option undefined to preserve the default per-start gate, and preserves the original temporary-directory route because that route determines the legacy namespace.

## Guarded deployment completion

The user's later direction explicitly authorized completing all five deployment tasks with best judgment, resolving the separate boot-recovery hold. COMMAND8 first completed its independently approved registration-only cutover to exact main `21330169b25b24c282d6d097166c2bf6cafd8578`, preserving approved 2.0.4 bytes and old roots. After the browser owner's settings write completed, COMMAND8 granted a serialized Chrono-only write window. Its repository and this branch's shared Git are separate; neither active verifier nor parent/worker sessions were interrupted or reloaded.

The rollback was reconciled to the newly registered exact-main 2.0.4 source, not the obsolete alias/pointer assumption. A second complete owner-only package/dependency backup verified 24,331 files and 13 internal links, tree SHA-256 `d11aca1eb7ca174f129682b10655b1912dec4ffa1bd2d05aa05a593f5967c3c7`. It retains current settings/configuration and pointer metadata alongside the earlier repository bundle and recovery points. The scoped helper passed syntax, pre-install checks and post-install `--rollback-check`. It restores only the Chrono package-source slot and stable alias, retains object filters if present, uses fresh compare-before-write checks, requires settled canaries before application, and preserves other settings and all unified/historical pointers. **Rollback was verified, not exercised.** Do not apply the older whole-registration or M03 runtime-downgrade helpers to this new selection.

Production boot recovery first reverified both admission namespaces absent, all four fixed units inactive, unchanged required configuration and bounded double same-user old-worker quiescence scans. The reviewed helper pins accepted runtime files, reserves only new directories, writes exact canonical policy 2 with the unchanged configured one-slot/2 GiB host limit before enabling admission, and calls the accepted four-inhibitor/current-boot gate API. Apply and read-only verification passed. No occupied policy was migrated; no existing artifact was deleted, process killed, configuration rewritten or limit changed. Disposable tests proved absent/read-only, fresh apply, idempotent ready checks and unchanged-byte refusal for partial, foreign, unsafe and occupied-artifact cases. The first parent hash check used the evidence directory rather than the manifest's worktree-relative base and refused before execution; the corrected base verified both frozen hashes. This bootstrap helper is intentionally not a post-use health/repair tool: normal queue/rendezvous artifacts later make its strict pristine-state check refuse.

At the exact clean pushed candidate `dcd91924dbcfc0c02489e04c3e163b33e2b08e86`, the scoped install atomically replaced the Chrono alias and package-source slot individually, with rollback on a failed second step. It did not claim a cross-file atomic transaction. Fresh settings/configuration/pointer comparisons passed. Other repository packages, external browser/research registration, automatic extensions, shared core, unified pointers and historical M03 pointer were preserved. The selected immutable package is 2.0.5, entry SHA-256 `ab28b018a42137c030dc94dfe88c44e977dce8e9cf76ed9526da55b20258ace4`; source/dist remain unchanged from the code-approved head. Chrono is now an explicitly separate exception to COMMAND8's exact-main 2.0.4 registration, not evidence that main contains M04.

Both installed-entrypoint canaries passed through the stable alias: locked Pi 0.84.2 and unchanged global Pi 0.85.1. Each used fresh private synthetic sources/stores and normal jobs through the verified default production admission gate, never production fault injection. Each observed 41 attributable admissions, maximum one slot/two tickets, zero remaining task admissions, absent client processes and an inactive worker unit. Both retained policy/gate/inhibitor hashes, complete first persisted tool results at the offline provider boundary, the fixed offline regular-summary fixture, contained replay/history, off restart with no catalog writes, two independent catalog clients/restarts, bounded ingestion/append, pins, chronology and exact sampled raw recovery. WAL and SHM permissions were sampled in both; a transient temporary file was not observed in these installed runs and is not claimed observed. Earlier disposable checks cover that sample. Neither sampled permissions nor anchors prove every transient lifetime or arbitrary unsampled source immutability.

A fresh global offline, no-session Pi process passed production resource loading, doctor and worker-status commands with zero extension errors and no private path exposure. Persistent isolated workers remain enabled, catalog shadow remains globally off, and configuration bytes are unchanged. Existing parent/worker processes were not reloaded; only fresh tested processes establish loaded 2.0.5 identity. Global Pi 0.85.1 remains a compatibility observation, not expanded peer support. Native-inclusive accounting and retained scale/fault evidence retain their original limits; these small deployment fixtures do not replace the 50,000-record/276 MiB campaigns. No external provider, private archive, broad activation, package publication, M05 or merge was used. Evidence and old roots remain retained, without deletion or secure-erasure claims. COMMAND8 received the exact selection, scoped rollback and settlement receipt; the settings window was released. This report is delivery evidence for final M04 review, not self-acceptance.
