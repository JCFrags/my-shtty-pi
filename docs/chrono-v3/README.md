...
Version `3.0.3` keeps programmatic V3 memory and safe-idle physical rollover enabled by default. Earlier focused checks exercised composition without model calls, useful bounded recall, complete native tool-state transfer, and real Pi physical replacement with restart. This supersedes the disabled-by-default 2.x development configuration. The [patch corrections](../chrono-release-compatibility.md#current-303-release) retain failed-compaction continuation, tree navigation, and verified common-prefix reuse. Raw exact-entry recovery now uses the validated catalog without waiting for the final derived search branch. Ranked search, decoded blocks, ranges, state, and rollups still need their existing readiness. The 3.0.2 stopped-owner and admission-wait diagnostics remain read-only and do not recover blocked admission.
