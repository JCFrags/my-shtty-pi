...
- [`baseline-evidence.json`](./baseline-evidence.json) records the bounded hash and count projection.
...
- [`historical-test-inventory.md`](./historical-test-inventory.md) records the restored-test inventory.
- [`known-incidents.md`](./known-incidents.md) records bounded incident classifications.
- [`independent-review.md`](./independent-review.md) records the independent R1 review and the explicit acceptance boundary.
...
M00 is evidence and containment only. It does not change ChronoCompact runtime behavior, enable a new memory mode, alter Pi settings, or deploy a package. The repository is currently public under the separately recorded A-0003 visibility correction; public visibility is not a release or package publication. The independent R1 review found no blocking defect, but M00-R1 still awaits explicit project-lead/user acceptance; M01 must not begin until that decision is recorded.
