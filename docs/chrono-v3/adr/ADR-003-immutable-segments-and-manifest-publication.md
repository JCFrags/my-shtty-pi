...
Derived physical schema 2 adds the transactional `capsule_ancestry` membership index. Capsule pagination first authorizes the actual M04 view, then selects that lineage and event cut through the index before applying the bounded limit. Sibling artifacts cannot consume the page or poison an old pin. The cursor is an `(eventSeq, descriptor)` pair, not an event-only exclusion. The earlier unaccepted derived schema 1 is not migrated or relabeled; its directory and bytes remain untouched. Capsule, chunk, and segment encoding versions remain 1.
...
Publication requires Linux, `/usr/bin/python3` with standard-library `ctypes`, a libc `renameat2` symbol, and filesystem support for `RENAME_NOREPLACE`. The worker executes constant code with `-I -S`, explicitly typed syscall arguments, validated same-directory paths, a sanitized environment, and a five-second timeout. It never executes path-derived code. Missing capability, unsupported syscall or filesystem, timeout, and other unexpected errors fail closed. On `EEXIST`, it verifies and reuses only the exact safe destination bytes. There is no ordinary-rename, copy, or hard-link fallback. A shell `mv -n` frontend is not sufficient: some implementations fall back to a check followed by an overwriting rename. The capsule segment tests exercise the installed capability and raced matching, differing, and unsafe destinations; these tests must pass in local and CI environments.

Read-only operations acquire only an existing owner-only `publication.lock`, opened with `O_NOFOLLOW` and without `O_CREAT`. They validate the file and recheck its inode after acquiring the kernel lock. A missing lock refuses without creating it; status is not a repair or bootstrap operation. The shared M03 mutex implementation is unchanged.
