...
`CatalogSqlite.open(absolutePath, validate?)` is existing-only: a missing or zero-length DB refuses before native open. `CatalogSqlite.create(absolutePath, validate?)` explicitly authorizes fresh bootstrap or valid-store resume; an existing zero-byte file or orphan sidecars refuse, not permission to recreate. The engine validates schema, logical session and expected physical UUID through a read-only native connection before writable WAL setup. It repeats validation inside initialization. Only explicit creation can initialize a blank fresh store. Both APIs return `prepare(sql).get/run/iterate(maxRows, ...values)`, `transaction(fn)`, `checkpoint()`, `capabilities()`, and `close()`. Values are positional SQL bindings. Errors contain fixed codes, never SQL, DB paths, source text, or native messages. Busy/locked errors use `catalog-sqlite-busy`. No persistent schema or `all` API exists.

Native read-only preflight is not side-effect-free for a nonempty WAL-mode database. On this pinned Linux binding, refusing a checkpointed wrong-identity/schema database can create an empty WAL and a 32,768-byte SHM; SQLite may rebuild transient SHM bookkeeping. Tests prove main DB and existing committed WAL bytes remain unchanged on identity refusal, with no schema write, checkpoint or deletion. Missing/empty DB refusal preserves the complete orphan-sidecar set and bytes without native open. Do not describe nonempty preflight as preserving **all** filesystem artifacts. This binding lacks usable immutable URI support, and ignoring a valid WAL would be incorrect.
